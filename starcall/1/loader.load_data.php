...
include("conn_string.cfg.php");
include("path.cfg.php");

	echo "<font size=4>Таблица: ".$_SESSION['table_id']." (id:".$_SESSION['table_name'].")</font><br>"; 
...
Первая строка содержит названия полей
	...
	$q=OCIParse($c,"select id cnt from TABLES_LOAD_HIST t
where table_id='".$_SESSION['table_id']."' and status='Загружается...'");
	OCIExecute($q);
	if(OCIFetch($q)) {
		$disabled='y';
	} 
...
if (isset($preview)) {
	...
	$q=OCIParse($c, "select TABLES_LOAD_HIST_ID_SEQ.Nextval load_id, dbms_random.string('A',30) abort_pwd from dual");
	OCIExecute($q,OCI_DEFAULT);
	OCIFetch($q);
	$_SESSION['load_id']=OCIResult($q,"LOAD_ID");
	$abort_pwd=OCIResult($q,"ABORT_PWD");
	echo "<input type=hidden name=load_id value=".$_SESSION['load_id'].">";
	echo "<input type=hidden name=abort_pwd value='".$abort_pwd."'>";

	//получаем из БД уже существующие поля в массив base_fields
	$q=OCIParse($c,"select t.id,t.text_name,t.code_name,t.std_field_id,s.description,t.uniq,t.must,t.last_src_field_name
    from TABLES_FIELDS t,TABLES_STD_FIELDS s
    where s.id(+)=t.std_field_id
  and t.table_id='".$_SESSION['table_id']."'
  order by t.ord");
	OCIExecute($q, OCI_DEFAULT);
	$i=0; while(OCIFetch($q)) { //поля БД нумеруются с нуля
		...
		$base_fields[$i]['last_src_field_name']=OCIResult($q,"LAST_SRC_FIELD_NAME");
	...
	$q=OCIParse($c,"select avg_load_speed from TABLES_LOAD_STATISTIC t");
	...
	if($_FILES["new_file"]["size"] > 1024*400*1024) {
		echo ("</font color=red>Размер файла превышает 400 мегабайта!</font>"); exit();
	}
	else {
		...
		foreach ($base_fields as $bfkey => $bf) {
			foreach ($file_fields as $ffkey => $ff) {
				...
				or (isset($bf['last_src_field_name']) and strtoupper(trim($bf['last_src_field_name']))==strtoupper(trim($ff['text_name'])))
...
order by t.description");
		...
		foreach($base_fields as $key => $bf) {
			...
			<td style='cursor:s-resize' title='CTRL-двигать строку целиком' onMouseDown='fMD(this)' onMouseUp='fMU(this)' onMouseMove='return false'>
			...
			echo "<td>".($key+1)."</td>
			<th style='cursor:s-resize' title='CTRL-двигать строку целиком' onMouseDown='fMD(this)' onMouseUp='fMU(this)'><input type=hidden name=base_fields_id[] value='".$bf['id']."'>".$bf['id']."</th>";
			...
			if($bf['new_field']=='y') { //новые поля разрешаем редактировать
				...
				echo "</td>";
				echo "<td><input type=checkbox name=base_fields_uniq[".$bf['id']."]".($bf['uniq']<>''?' checked':NULL)."></td>"; //уникальное
				echo "<td><input type=checkbox name=base_fields_must[".$bf['id']."]".($bf['must']<>''?' checked':NULL)."></td>"; //обязательное
				echo "<td><input type=checkbox name=base_fields_quoted[".$bf['id']."]".($bf['quoted']<>''?' checked':NULL)."></td>"; //квотируемое
				echo "<td><input type=checkbox name=base_fields_idx[".$bf['id']."]".($bf['idx']<>''?' checked':NULL)."></td>"; //индексируемое
