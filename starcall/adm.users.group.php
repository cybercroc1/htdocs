...
echo "</td></tr><tr class=content_tr class=content_tr><td><div class=content_div>";
  ...
  where u.id=g.creator
	...
	<td style='cursor:pointer' colspan=4 onclick='select_group(this)'><b>ВСЕ ПОЛЬЗОВАТЕЛИ</b></td>";
	echo "</tr>";	
	...
	while (OCIFetch($q)) {
		...
		(($_SESSION['user']['rw_users']=='w' or $_SESSION['user']['rw_opers']=='w') and OCIResult($q,"MY_CHILD")=='y' )) {
			echo "<td style='cursor:pointer' ondblclick='del_old_group(this)' title='Удалить группу (двойной щелчок)'><img src=png/del.png></img></td>";
		...
		//редактирование запрещено	
			...
			echo "<td style='cursor:pointer' onclick='select_group(this)'><b>".(OCIResult($q,"DEFAULT_GROUP")=='y'?"Да":"Нет")."</b></td>";
