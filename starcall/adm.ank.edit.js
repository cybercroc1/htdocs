...
function fMU(cell) { //отпускание кнопки мыши
	...
	if(src_row_index!='x' & src_row_index!=dst_row_index) {
		...
		//глупая функция insertBefore не заботает если родителя указывать через переменную, поэтому только так:
		src_row.parentNode.insertBefore(src_row,dst_row);
		src_row_index='x';
	}
}

//поячеечный перенос строк
...
//notsaved();
	...
	with(obj.parentNode.parentNode) { //таблица
		new_row=insertRow(obj.parentNode.rowIndex+1)
		with(new_row) {//строка
			...
			with(appendChild(document.createElement('th'))) {
				...
				setAttribute("onclick","click_row(this.parentNode,'sel')");
			...
			with(appendChild(document.createElement('td'))) {
				...
				with(appendChild(document.createElement('select'))) {
					...
					opt=appendChild(document.createElement('option')); opt.value='q_sn_datetime'; opt.innerText='Вопрос - Дата и время';
