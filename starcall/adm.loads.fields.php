...
echo "</td></tr><tr class=content_tr class=content_tr><td><div class=content_div>";
		...
		foreach($base_fields as $key => $bf) {
			...
			echo "<th title='Удалить (двойной щелчок)' onDblClick='del_old_field(".$bf['id'].");del_field(this)'><font color=red><img src='png/del.png'></img></font></th>";
			echo "<th title='Добавить ниже' onClick=plus_field(this)><font color=blue><img src='png/plus.png'></img></font></th>";
			echo "<th style='cursor:s-resize' onMouseDown='fMD(this)' onMouseUp='fMU(this)'><input type=hidden name=base_fields_id[] value='".$bf['id']."'>".$bf['id']."</th>";


				echo "<td><input style='width:100%' type=text name=base_fields_text_name[".$bf['id']."] value='".$bf['text_name']."'></td>";
				echo "<td><input style='width:100%' type=text name=base_fields_code_name[".$bf['id']."] value='".$bf['code_name']."'></td>";
				echo "<td>";
				...
				echo "<td><input type=checkbox name=base_fields_ank_show[".$bf['id']."]".($bf['ank_show']<>''?' checked':NULL)."></td>"; //видно в анкете				
