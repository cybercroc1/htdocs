...
if(isset($frm_submit) and $frm_submit=='save') {
	...
	set	name=nvl('".$project_name."',name), status=nvl('".$set_status."',status), nedoz_interval=".$nedoz_interval.", from_time='".$from_time."', to_time='".$to_time."', quote='".$quote."', perez_policy='".$perez_policy."', nedoz_chance='".$nedoz_chance."', last_activity=sysdate
	where id=".$project_id);
	...
	//поменяли кол-во недозвонов
	if($nedoz_count<>$old_nedoz_count) {
		$upd=OCIParse($c,"begin STC_NEDOZ_CHANGE(".$_SESSION['adm']['project']['id'].",".$nedoz_count.",:changed_rows); end;");
		OCIBindByName($upd,":changed_rows",$changed_rows,16);
		OCIExecute($upd,OCI_DEFAULT);
		echo "Кол-во недозвонов: изменен статус ".$changed_rows." записей<hr>";
	}
...
OCIFetch($q);
	...
	echo "<font size=4>Настройки проекта \"".OCIResult($q,"NAME")."\" (id:$project_id)</font>";
...
echo "</td></tr><tr class=content_tr class=content_tr><td><div class=content_div>";
	...
	echo "<tr>";
	echo "<td align=center><b>Перезвоны: политика перезвонов:</b></td><td>";
	...
	echo "</td>";
	echo "<td><i><b>Общие.</b>Оператору бедут выводиться чужие перезвоны, если настало время перезвона, а их хозяин в данный момент не залогинен в систему и не работает по данному проекту.
	<hr>
	<b>Частные.</b> Оператор не будет иметь доступ к чужим перезвонам.
	</i></td>";
	echo "</tr>";
	...
	echo "<tr>";
	echo "<td align=center><b>Недозвоны: вероятность подачи недозвона</b></td><td><nobr><input type=text name=nedoz_chance value='".OCIResult($q,"NEDOZ_CHANCE")."'><b></b></td>";	
