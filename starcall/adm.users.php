...
function kill_sess(user_id) {
	if(confirm("Выбранный пользователь не сможет продолжить работу. Закрыть сессию?")) {
		parent.parent.logFrame.location='func.kill_sess.php?user_id='+user_id;
	}
	else return false;
}
...
if($_SESSION['user']['all_users']=='y') { //все пользователи
	...
	case when u1.last_activity > sysdate-2/1140 and nvl(u1.last_logout,u1.last_activity)<=u1.last_activity then 'y' end active, 
	...
	$i=1; $j=1; while (OCIFetch($q)) {
		...
		OCIResult($q,"IN_GROUP")=='y'?$tmp_class=' class=selected_row':$tmp_class=' class=selectable_row'; //подсвечиваем синим всех пользователей в выбранной группе
		
		echo "<tr data-in_group='".$in_group."' data-user_id='".OCIResult($q,"ID")."'".$tmp_class.">";
		...
		or ($_SESSION['user']['rw_opers']=='w' and OCIResult($q,"MY_CHILD")=='y' and OCIResult($q,"ROLE_ID")=='operator'))) {
			echo "<td style='cursor:pointer' ondblclick=del_old_user(this) title='Удалить пользователя (двойной щелчок)'><img src='png/del.png'></img></td>";
		}
		else {
			echo "<td></td>";
		}
		...
		or ($_SESSION['user']['rw_opers']=='w' and OCIResult($q,"ROLE_ID")=='operator'))) {		
			if(OCIResult($q,"IN_GROUP")=='y') {
				echo "<td style='cursor:pointer' onclick=minus(this) title='Исключить пользователя из группы'><img src='png/minus.png'></img></td>";	
			}
			else {
				echo "<td style='cursor:pointer' onclick=plus(this) title='Добавить пользователя в группу'><img src='png/plus.png'></img></td>";
			}				
		}
		else {
			echo "<td></td>";
		...
		or ($_SESSION['user']['rw_users']=='w' and OCIResult($q,"MY_CHILD")=='y') 
		or ($_SESSION['user']['rw_opers']=='w' and OCIResult($q,"MY_CHILD")=='y' and OCIResult($q,"ROLE_ID")=='operator')) {
			echo "<td><b>".OCIResult($q,"ID")." г: ".OCIResult($q,"IN_GROUP")." с: ".OCIResult($q,"MY_CHILD")."</b></td>";
			echo "<td><input type=text onkeyup='notsaved()' onchange='ch_user(".OCIResult($q,"ID").");notsaved()' onpaste='notsaved()' name=fio[".OCIResult($q,"ID")."] value='".OCIResult($q,"FIO")."'></input></td>";
			echo "<td><input type=text onkeyup='notsaved()' onchange='ch_user(".OCIResult($q,"ID").");notsaved()' onpaste='notsaved()' name=login[".OCIResult($q,"ID")."] value='".OCIResult($q,"LOGIN")."'></input></td>";
			echo "<td><input type=text onkeyup='notsaved()' onchange='ch_user(".OCIResult($q,"ID").");notsaved()' onpaste='notsaved()' name=pass[".OCIResult($q,"ID")."] value='".OCIResult($q,"PASS")."'></input></td>";
		}		
		else {	
			echo "<td><b>".OCIResult($q,"ID")." г: ".OCIResult($q,"IN_GROUP")." с: ".OCIResult($q,"MY_CHILD")."</b></td>";
			...
			if(OCIResult($q,"MY_CHILD")=='y') {
				echo "<td><b>".OCIResult($q,"PASS")."</b></td>";
			}
			else {
				echo "<td><b>******</b></td>";
			}
		...
		echo "<td><input type=hidden name=role[".OCIResult($q,"ID")."] value='".OCIResult($q,"ROLE_ID")."'><b>".OCIResult($q,"ROLE_NAME")."</b></td>";
		echo "<td><b>".OCIResult($q,"CREATE_DATE")."</b></td>";
		echo "<td>";
		...
		if(OCIResult($q,"ACTIVE")=='y') {
			echo "Активен"; 
			if(($_SESSION['user']['rw_users']=='w' or ($_SESSION['user']['rw_opers']=='w' and OCIResult($q,"ROLE_ID")=='operator')) and OCIResult($q,"ID")<>$_SESSION['user']['id']) 
				echo " <img src='png/exit.png' onclick=kill_sess('".OCIResult($q,"ID")."') title='Закрыть сессию'></img>";
		}
		echo "</tr>";
	}
	if($group_id<>'' and $j==1) {
