...
echo " | ";
echo "<a href='store.loads.loads.php'>Менеджер загрузок</a> | ";
//echo "<a href='store.loads.fields.php'>Настройка исх. полей</a> | ";
echo "<a href='store.loads.load_data.php'><font size=4>Загрузка из .CSV</font></a> | ";
echo "<font align=right><a href='help.store.loads.load_data.html' target='_blank'>Справка</a></font>";
echo "<hr>";

include("starcall/conn_string.cfg.php");
include("starcall/path.cfg.php");

	echo "<font size=4>Загрузка в хранилище</font><br>"; 
	...
	if($_FILES["new_file"]["size"] > 1024*100*1024) {
		echo ("</font color=red>Размер файла превышает 3 мегабайта!</font>"); exit();
	 	...
	 	if (!is_uploaded_file($_FILES['new_file']["tmp_name"])) {
			...
			foreach ($str as $num => $val) { //поля файла нумеруются с нуля
		...
		//присваиваем имена из файла новым полям бд 
		//if(!isset($first_load)) {
		//	foreach($file_fields as $ffkey => $ff) {
...
order by t.text_name");
		...
		foreach($file_fields as $ffnum => $ff) {
			...
			echo "<th style='cursor:pointer' title='Удалить (двойной щелчок)' onDblClick=del_file_field(this)><img src='png/del.png'></img></th>";
			echo "<td>".$ffnum."</td>";
