...
<body class="body_marign" onLoad="parent.admFrameset.rows=Math.round(document.getElementById('end_page').offsetTop)+8+',*,1'">
<?php 
extract($_POST);
...
if($_SESSION['user']['rw_src_bd']<>'' and $_SESSION['adm']['project']['id']<>'') echo "<a href='store.loads.loads.php' target='admBottomFrame'>Хранилище</a> | ";
...
if($_SESSION['user']['operator']=='y' and $_SESSION['adm']['project']['id']<>'') echo "<a href='survey.call.frame.php' target='admBottomFrame'>Начать опрос</a> | ";
...
if($_SESSION['adm']['project']['id']<>'') {
	...
	$q=OCIParse($c,"select count(*) cnt from STC_USERS where last_oper_prj_id=".$_SESSION['adm']['project']['id']." and nvl(last_logout,last_activity)<=last_activity and last_activity>=sysdate-5/1440");
