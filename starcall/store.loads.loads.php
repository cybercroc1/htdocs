...
echo " | ";
echo "<a href='store.loads.loads.php'><font size=4>Менеджер загрузок</font></a> | ";
echo "<a href='store.loads.fields.php'>Настройка исх. полей</a> | ";
echo "<a href='store.loads.load_data.php'>Загрузка из .CSV</a> | ";
echo "<font align=right><a href='help.store.loads.loads.html' target='_blank'>Справка</a></font>";
echo "<hr>";
...
echo "<font size=4>Загрузки.</font><br>"; 
...
order by ".$_SESSION['adm']['loads']['order_by']);
	...
	while (OCIFetch($q)) {
		...
		<td>
		<a style='cursor:pointer' onClick=window.open('store.loads.show_load_data.php?load_id=".OCIResult($q,"ID")."&show_rows='+frm.show_rows.value) title='показать данные'>".OCIResult($q,"STATUS")."</a>
