...
$commit_interval=1; //Количество строк в одной транзакции
$abort_interval=100;
...
/*foreach($base_fields_id as $key=>$id) {
	echo "base_fields_id:".$key."[".$id."]; key:".$key.";<br>";
}
foreach($file_fields_name as $key=>$id) {
	echo "file_fields_name:".$key."[".$id."]; key:".$key.";<br>";
}
exit;*/
...
$load_id=$_SESSION['adm']['load_id'];
//добавляем запись в историю загрузок
$upd=OCIParse($c,"update STC_LOAD_HISTORY 
set status='Загружается...', start_date=sysdate, file_name='".$file_name."', file_size_bytes='".$file_size."', abort_pwd='".$abort_pwd."', abort=''
where project_id=".$project_id." and id=".$load_id);
OCIExecute($upd,OCI_DEFAULT);
if(oci_num_rows($upd)==0) {
	$ins=OCIParse($c,"insert into STC_LOAD_HISTORY (ID,PROJECT_ID,STATUS,START_DATE,FILE_NAME,FILE_SIZE_BYTES,ABORT_PWD) 
	values (".$load_id.",'".$project_id."','Загружается...',sysdate,'".$file_name."','".$file_size."','".$abort_pwd."')");
	OCIExecute($ins,OCI_DEFAULT);
}	
//OCIBindByName($ins,":load_hist_id",$load_id,16);
//OCIBindByName($ins,":abort_pwd",$abort_pwd,256);
...
/*//унопка отмены загрузки
echo "<a href='adm.loads.cancel_load.php?load_id=".$load_id."&abort_pwd=".$abort_pwd."' target=_blank>отмена загрузки</a>";
echo "<script>
load_id='".$load_id."';
abort_pwd='".$abort_pwd."';
parent.admBottomFrame.frm_preview.cancel_load.style.display='none';
...
foreach($base_fields_id as $key=>$id) {
	...
	//номер и ID поля доб.номер
	if($base_fields_std_name[$id]=='PHONE_EXT') {
		$phone_ext_field_id=$id;
		$phone_ext_field_num=$file_fields_num[$key];
	}
...
$q_ins_phone=OCIParse($c,"insert into STC_PHONES (id,base_id,project_id,phone,ext,base_field_id,ord,allow,load_hist_id) 
values (SEQ_STC_PHONE_ID.nextval,:base_id,'".$project_id."',:phone,:ext,:base_field_id,:ord+1,'".$allow."','".$load_id."')");
...
if(count($idx_fields)>0 or count($quoted_fields)>0) {
	$q_idx_check=OCIParse($c,"select id, case when i.src_idx_quote-i.STAT_end_norm<=0 then 'y' else null end idx_lock from STC_SRC_INDEXES i where field_id=:field_id and value=:value");
	...
	if($allow=="y") $q_upd_idx=OCIParse($c,"update STC_SRC_INDEXES set STAT_new=STAT_new+1 where id=:index_id");
}
//
...
where cnt=".count($quoted_fields));
	...
	if($allow=="y") $q_upd_quote=OCIParse($c,"update STC_SRC_QUOTES set STAT_new=STAT_new+1 where id=:quote_id");
}
...
continue;
	...
	if($i/500==round($i/500)) echo "<script>
	
	parent.admBottomFrame.document.getElementById('load_status').innerHTML='<font color=black><b>Идет загрузка...</b></font> Строк. Прочитано: $read_row_count; Загружено: $load_rows_count; Пустых: $null_row_count; Дубликатов: $dublicate_count; Найдено телефонов: $found_phones_count; Загружено телефонов: $load_phones_count';</script>";
	$read_row_count++;
	
	if($read_row_count>0 and round($read_row_count/$abort_interval)==$read_row_count/$abort_interval) {
		//отмена загрузки.
		$q_abort_load=OCIParse($c,"select abort from STC_LOAD_HISTORY where id=".$load_id);
		OCIExecute($q_abort_load,OCI_DEFAULT);
		OCIFetch($q_abort_load);
		if(OCIResult($q_abort_load,"ABORT")=='y') {
			$info.="<font color=red>Загрузка прервана!</font><br>";
...
continue;
	...
	if($phone_field_id<>'') { //если есть поле Телефон
		...
		$p=0; $d=0; 
		$ins_phones_arr['phone']=array();
		$ins_phones_arr['ext']=array();
		//echo "<br>".str_replace($nlchar,';',$str[$phone_field_num])."<br>";
		$phones=phones_conv(str_replace($nlchar,';',$str[$phone_field_num]));
		foreach($phones as $phone) {
			//foreach ($phone as $kkk => $vvv) {echo $kkk."-".$vvv."<br>";}
			//не верный телефон
			if($phone['err']=='y') {
		...
		continue;
			...
			//слишком длинный экстеншн
			elseif (strlen($phone['ext'])>25) {
				echo "$i: слишком длинный добавочный (до 25 зн.): \"".$phone['ext']."\"<br>";
				fput_err($fp_err,$i,"слишком длинный добавочный (до 25 зн.):","\"\"".$phone['ext']."\"\"",implode('";"',str_replace('"','""',$str)));
				$wrong_phones_count++;			
		continue;
			}
			$p++;
			...
			if($base_fields_uniq[$phone_field_id]<>'') {
				...
				$d1=0;
				foreach($ins_phones_arr['phone'] as $phone1) {
		...
		continue;		
			...
			$ins_phones_arr['phone'][]=$phone['phone'];
			$ins_phones_arr['ext'][]=$phone['ext'];
		}
...
continue;			
		...
		//проверка экстеншна (стандартное(PHONE_EXT))(если несколько номеров, и есть экстеншн)
		if(count($ins_phones_arr['phone'])>0 and isset($phone_ext_field_id)) {
			if($str[$phone_ext_field_num]<>'' and count($ins_phones_arr['phone'])>1) {
				echo "$i: доб.номер \"".$str[$phone_ext_field_num]."\" не применим для нескольких телефонов<br>";
				fput_err($fp_err,$i,"доб.номер не применим для нескольких телефонов","\"\"".$str[$phone_ext_field_num]."\"\"",implode('";"',str_replace('"','""',$str)));
				$f_err++;
continue;			
			}
			elseif (count($ins_phones_arr['phone'])==1) {
				if(strlen($str[$phone_ext_field_num])>25) {
					echo "$i: слишком длинный доб.номер (до 25 зн.)\"".$str[$phone_ext_field_num]."\"<br>";
					fput_err($fp_err,$i,"слишком длинный доб.номер (до 25 зн.)","\"\"".$str[$phone_ext_field_num]."\"\"",implode('";"',str_replace('"','""',$str)));
					$f_err++;					
continue;
				}
			}
	...
	if($phone_field_id<>'') { //если есть поле Телефон
		foreach($ins_phones_arr['phone'] as $ord => $phone) {
			//если найден экстеншн в поле "телефон", то добавлянм его
			if($ins_phones_arr['ext'][$ord]<>'') $ext=$ins_phones_arr['ext'][$ord];
			//если нет, то из стандартного поля
			elseif(isset($phone_ext_field_id)) $ext=str_replace(array("\n","	",";",","),"&",$str[$phone_ext_field_num]); else $ext='';
			
			OCIBindByName($q_ins_phone,":base_id",$base_id);
			OCIBindByName($q_ins_phone,":phone",$phone);
			OCIBindByName($q_ins_phone,":ext",$ext);
...
if($allow=='y') {
	...
	OCIExecute(OCIParse($c,"begin STC_STAT_PROJECT_CALC(".$_SESSION['adm']['project']['id']."); end;"));
	echo "Пересчитана общая статистика по проекту<br>";
}
...
where status='Готово' and end_date-start_date>0 and t.file_size_bytes>0 and t.start_date>to_date('24.10.2016','DD.MM.YYYY')
)");
OCIExecute($upd,OCI_DEFAULT);	
OCICommit($c);
}
...
echo "<script>
parent.admBottomFrame.document.getElementById('load_status').innerHTML='".$info."<font color=green><b>Загрузка завершена за $dursec сек (".round($dursec/60)." мин). </b></font>Строк. Прочитано: $read_row_count; Загружено: $load_rows_count; Пустых: $null_row_count; Дубликатов: $dublicate_count; Найдено телефонов: $found_phones_count; Загружено телефонов: $load_phones_count';
parent.admBottomFrame.frm_preview.load_caption.value='';
parent.admBottomFrame.frm_preview.cancel_load.style.display='none';</script>";
echo "<hr>";
...
function my_error_handler($code, $msg, $file, $line) {
	global $load_id;
	include("starcall/conn_string.cfg.php");
	$upd=OCIParse($c,"update STC_LOAD_HISTORY set end_date=sysdate, status='Ошибка',abort=''
where id='".$load_id."'"); OCIExecute($upd); OCICommit($c);
