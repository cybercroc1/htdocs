...
if((isset($close) or isset($pause) or isset($open)) and isset($mark)) {
	...
	$upd=OCIParse($c,"update STC_PROJECTS set status='".$set_status."', status_date=sysdate, last_activity=sysdate where id=:id");
	foreach($mark as $id) {
	...
	echo " | <select name=show_closed onchange=frm_projects.submit()>
	 ...
	 <option value='-6'".($_SESSION['adm']['projects']['date_filter']==-6?" selected":NULL).">дата посл.активности 6 месяцев назад</option>
...
echo "</td></tr><tr class=content_tr><td><div class=content_div>";
	...
	<td align=center><a href='adm.projects.php?order_by=nvl(p.last_activity,p.create_date) desc'>".($_SESSION['adm']['projects']['order_by']=='nvl(p.last_activity,p.create_date) desc'?'<b>':NULL)."Посл.актив</b></a></td>
	...
	$q=OCIParse($c,"select p.id,p.name,to_char(p.create_date,'DD.MM.YYYY HH24:MI') create_date,to_char(p.last_activity,'DD.MM.YYYY HH24:MI') last_activity,p.status,decode(p.status,'Активен','green','Приостановлен','orange','Закрыт','red') color,
	u.fio creator
	...
	".(($_SESSION['adm']['projects']['date_filter']<>'' and $_SESSION['adm']['projects']['find_string']=='')?" and nvl(p.last_activity,p.create_date)>=add_months(sysdate,".$_SESSION['adm']['projects']['date_filter'].")":NULL)."
	...
	OCIExecute($q,OCI_DEFAULT);
	while (OCIFetch($q)) {
		OCIResult($q,"ID")==$_SESSION['adm']['project']['id']?$tmp_class=' class=selected_row':$tmp_class=' class=selectable_row';
		echo "<tr data-project_id='".OCIResult($q,"ID")."'".$tmp_class." onclick=click_row(this,'sel')>";
		echo "<td><input type=checkbox name=mark[] value='".OCIResult($q,"ID")."'></input></td>
		<td style='cursor:pointer' onclick='sel_project(this)'><b>".OCIResult($q,"ID")."</b></td>
		<td style='cursor:pointer' onclick='sel_project(this)'><b>".OCIResult($q,"NAME")."</b></td>
		<td style='cursor:pointer' onclick='sel_project(this)'><b>".OCIResult($q,"CREATE_DATE")."</b></td>
		<td style='cursor:pointer' onclick='sel_project(this)'><b>".OCIResult($q,"LAST_ACTIVITY")."</b></td>
		<td style='cursor:pointer' onclick='sel_project(this)'><b>".OCIResult($q,"CREATOR")."</b></td>
		<td style='cursor:pointer' onclick='sel_project(this)'>
