...
$q=OCIParse($c,"select decode(p.id,0,0,1) ord, p.id, p.name, p.tree_width,p.out_prefix
 ...
 order by ord, p.name");
	...
	if($_SESSION['allow_view_all_sc']<>1 and $_SESSION['allow_view_all_reports']<>1) $and_login="and r.login_id='".$_SESSION['login_id']."'"; else $and_login='';
	$q=OCIParse($c,"select 1 ord, p.id,p.name,p.tree_width,to_char(p.start_date,'DD.MM.YYYY HH24:MI:SS') start_date,p.out_prefix,
	".($_SESSION['allow_view_all_sc']==1?"'1' view_sc,":"r.view_sc,")."
	".($_SESSION['allow_view_all_reports']==1?"'1' view_rep,":"r.view_rep,")."
	".($_SESSION['allow_view_all_reports']==1?"'1' view_sms_log,":"r.view_sms_log,")."
	r.ch_sc,r.ch_form,r.ch_email,r.view_billing,r.vsr_billing,oir.view_okt_in_det 
	from sc_projects p
...
if(count($projects)>0) {
	...
	if (isset($_SESSION['project']['ch_sc']) and $_SESSION['project']['ch_sc']==1) {
		...
		if ($_SESSION['project']['id']>0 and $from_local_addr=='y' and ($_SESSION['project']['view_sc']==1 or $_SESSION['project']['ch_sc']==1)) 
			echo "(<a href='".$path_to_scenary."?project_id=".$_SESSION['project']['id']."&' target=_blank><img border=0 src=visible.gif title='посмотреть сценарий оператора'></a> )";
	...
	$sql_text="select count(*) cnt from SC_MODULES m
	...
	OCIExecute($q,OCI_DEFAULT);
	OCIFetch($q);
	if(OCIResult($q,"CNT")>0) echo "| <a href=modules_frame.php target=fr12>Модули </a>";
	else $_SESSION['last_url']='blank.htm';
