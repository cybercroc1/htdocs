...
if ($_SESSION['project']['view_billing']<>1) {echo "<font color=red>Страница недоступна!</font>"; exit();} 
?>
<?php
...
include("sc/sc_conn_string.php");
	...
	if (isset($report_go) or isset($xls_go)) {
		...
		if($admin=='y') {
			$q=OCIParse($c,"select ph.phone from SC_PHONES ph
			where ph.project_id='".$_SESSION['project']['id']."'");
		}
		else {
			$q=OCIParse($c,"select distinct ph.phone from SC_PHONES ph, SC_ACC_CDN ac
			where ac.project_id='".$_SESSION['project']['id']."' and ac.login_id='".$_SESSION['login_id']."' and ph.project_id=ac.project_id
			and (ac.phone=ph.phone or ac.phone='all')");
		}
	...
	echo "<nobr><font size=4> Биллинг - \"".$_SESSION['project']['name']."\"</font> ";
...
include("sc/ab_conn_string.php");
	...
	and a.offered_date >= to_date('".$_SESSION['project']['start_date']."','DD.MM.YYYY HH24:MI:SS')
	and 
 ...
 where a.originated_date BETWEEN to_date('".$_SESSION['start_bill_date']."', 'DD.MM.YYYY') AND
   ...
   and a.originated_date >= to_date('".$_SESSION['project']['start_date']."', 'DD.MM.YYYY HH24:MI:SS') 
