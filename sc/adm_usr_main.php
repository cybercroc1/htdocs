...
function new_pass_gen() {
	...
	//?rand='+Math.random() - случаная составляющая, что бы исключить кеширование
...
if($pass_check<>"OK") {
	echo "<font color=red><b>Ошибка: действие отменено. Причина: ".iconv("utf-8","windows-1251",$pass_check)."</b></font><br>";
}
else {
	if (isset($send) and $email_tmp<>'') {
		...
		$mess="Для доступа к системе зайдите на страничку: <a href=".$lk_url_tmp.">".$lk_url_tmp."</a><br>
	...
	'".$allow_record_full."','".$allow_nocall."','".$lk_type_tmp."','".trim($emails)."')");
		...
		echo "<font color=red>1ОШИБКА! Пользователь с таким именем и паролем уже существует!</font>";
		}
	}
	else {
	...
	where id='".$login_id."'");
		...
		echo "<font color=red>2ОШИБКА! Пользователь с таким именем и паролем уже существует!</font>";
		}
	}
}}
//
...
echo "<tr><td><font size=3><b>Пароль:</b></font></td><td><input type=text name=password value=\"".$password."\"> <input type=button value='Новый пароль' onclick='new_pass_gen()'></td></tr>";
echo "<tr><td colspan=2><hr></td></tr>";
echo "<tr><td><font size=3><b>Фамилия:</b></font></td><td><input type=text name=f value=\"".$_SESSION['edit_login']['f']."\"></td></tr>";
echo "<tr><td><font size=3><b>Имя:</b></font></td><td><input type=text name=i value=\"".$_SESSION['edit_login']['i']."\"></td></tr>";
echo "<tr><td colspan=2><hr></td></tr>";
echo "<tr><td><font size=3><b>EMAIL(ы):</b></font></td><td><textarea name=emails rows=2 cols=50>".htmlspecialchars($_SESSION['edit_login']['emails'])."</textarea>";
echo "<tr><td colspan=2><hr></td></tr>";
echo "<tr><td><font size=3><b>Описание:</b></font></td><td><textarea name=description rows=3 cols=50>".htmlspecialchars($_SESSION['edit_login']['desc'])."</textarea>
</td></tr>";
echo "<tr><td colspan=2><hr></td></tr>";
echo "<tr><td colspan=2><font size=3><b>Отправить доступ на E-Mail: </b></font><input type=text name=email_tmp value=\"".$email_tmp."\">
 <select name=lk_type_tmp>
 <option value=old_cabinet".($_SESSION['edit_login']['lk_type_tmp']=='old_cabinet'?' selected':'').">Старый кабинет</option>
 <option value=new_cabinet".($_SESSION['edit_login']['lk_type_tmp']=='new_cabinet'?' selected':'').">Новый кабинет</option>
 </select> 
...
echo "<tr><td colspan=2><font size=3><b>Разрешить прослушивать записи<input type=checkbox name=allow_records".($_SESSION['edit_login']['allow_records']==1?" checked":"").">
 доступ к полной информации по звонку <input type=checkbox name=allow_record_full".($_SESSION['edit_login']['allow_record_full']==1?" checked":"")."></b></font></td></tr>";
...
echo "<tr><td colspan=2><font size=3><b>Разрешить доступ к отчетам без звонка</b></font><input type=checkbox name=allow_nocall".($_SESSION['edit_login']['id']<>''?($_SESSION['edit_login']['allow_nocall']==1?" checked":""):' checked')."></td></tr>";
