...
if (isset($go_save)) {
$upd = OCIParse($c,"update sc_forms set post_url='".$post_url."',post_type='".$post_type."' where id='".$form_id."' and project_id='".$_SESSION['project']['id']."'");
OCIExecute($upd, OCI_DEFAULT);
OCICommit($c);
...
//Выбор формы
	...
	$q=OCIParse($c,"select f.id,f.post_url,f.post_type,f.name from sc_forms f, sc_projects p
	where f.project_id=".$_SESSION['project']['id']." and f.deleted is null and f.id>0
	and p.id=f.project_id order by f.name");
	OCIExecute($q,OCI_DEFAULT);
	while (OCIFetch($q)) {
		echo "<option value='".OCIResult($q,"ID")."'".($form_id==OCIResult($q,"ID")?' selected':'').">".OCIResult($q,"NAME")."</option>";
		if($form_id==OCIResult($q,"ID")) {$post_url=OCIResult($q,"POST_URL"); $post_type=OCIResult($q,"POST_TYPE");}
	}
	echo "</select>
...
if (isset($form_id) and $form_id<>'') {
	...
	echo "URL для отправки отчета методом POST <input type=text name=post_url value='".$post_url."' size='50'> 
	метод <select name=post_type>
	<option value=''>POST</option>
	<option value='array'".($post_type=='array'?' selected':'').">POST Массив</option>
	<option value='json'".($post_type=='json'?' selected':'').">JSON Массив</option>
	</select>
	<hr>";
