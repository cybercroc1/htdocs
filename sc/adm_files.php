...
if (isset($upload)) {
	if($_FILES["new_file"]["size"] > 1024*3*1024) {echo ("</font color=red>Размер файла превышает 3 мегабайта!</font>");}
	 	...
	 	if (!@is_uploaded_file($_FILES['new_file']["tmp_name"])) {echo "<font color=red>ОШИБКА ЗАГРУЗКИ ФАЙЛА!</font>";}
			...
			if (!@move_uploaded_file($_FILES['new_file']["tmp_name"],$path_to_project_files.$_FILES['new_file']["name"])) {echo "<font color=red>ОШИБКА ЗАГРУЗКИ ФАЙЛА!</font>";}
		}
	}
}	
...
echo "<form name=files_frm action=adm_files.php method=post enctype=\"multipart/form-data\">";
	echo "<font size=4>Файлы. </font>
	<a href='file://".$net_path_to_folders.$_SESSION['project']['name']."\\' target='_blank'>
	".$net_path_to_folders.$_SESSION['project']['name']."
	</a>
	<hr>";
	echo "<table id=tbl bgcolor=gray cellspacing=1 cellpadding=2>
	<tr>
	<td bgcolor=white align=center><b>Имя</b></td>
