...
if (!isset($sav_obj_br)) $sav_obj_br=array();
if (!isset($sav_obj_sms)) $sav_obj_sms=array();
if (!isset($sav_requre)) $sav_requre=array();
...
if (isset($sav_obj)) sav_obj($sav_ordering,$sav_obj_name,$sav_obj_sms_name,$sav_obj_type,$sav_obj_width,$sav_obj_height,$sav_obj_br,$sav_obj_sms,$sav_tag_before,$sav_tag_after,$sav_requre,$c);
if (isset($copy_form)) {
...
echo " <a href=\"javascript:del_form('".$form_id."')\"><img src=del.gif title=\"Удалить форму\" border=0></a>";
	echo "<a href='edit_form_php_before_save.php?form_id=".$form_id."' onclick='window.open(this.href,\"php_before_save\",\"location=no,width=600,height=800,toolbar=no,menubar=no,status=no,resizable=yes,scrollbars=yes\"); return false;'>
	php
	</a>";
}
...
if (isset($form_id) and $form_id<>'') {
	...
	<td bgcolor=white width=3><b>СМС</b></td>
	<td bgcolor=white><b>Название в СМС</b></td>
	<td bgcolor=white><input type=submit name=sav_obj value=СОХРАНИТЬ></td>";
	echo "</tr>";
	...
	t.name type_name,o.width,o.height,o.br,o.sms,o.sms_name,
	replace(replace(o.tag_after,'<','&lt;'),'>','&gt;') tag_after,
	o.invisible,o.ordering,o.requre,o.call_field_id,cf.var_name call_field_varname,cf.name call_field_name,t.grp
	from sc_form_object o
	left join sc_form_obj_type t on t.id=o.type_id
	left join sc_call_fileds cf on cf.id=o.call_field_id
	where o.type_id=t.id and o.form_id='".$form_id."' and o.project_id='".$_SESSION['project']['id']."' 
	order by ordering");
	
	$q_type=OCIParse($c,"select id,name from SC_FORM_OBJ_TYPE t
	where grp=:grp order by ord");
	
	OCIExecute($q,OCI_DEFAULT);
	$ordering=0;
	while (OCIFetch($q)) {$ordering++;
	...
	<input type=text name='sav_ordering[".OCIResult($q,"ID")."]' size=1 value='".$ordering."'>
	</nobr></td>
	<td bgcolor=white><input type=checkbox name=sav_requre[".OCIResult($q,"ID")."] value='Да'".(OCIResult($q,"REQURE")=="Да"?" checked":"")."></td>
	<td bgcolor=white width=3><input type=text name=sav_tag_before[".OCIResult($q,"ID")."] size=2 value='".OCIResult($q,"TAG_BEFORE")."'></td>
	<td bgcolor=white><input type=text name=sav_obj_name[".OCIResult($q,"ID")."] size=35 value='".htmlentities(OCIResult($q,"NAME"))."'></td>
	<td bgcolor=white><select name=sav_obj_type[".OCIResult($q,"ID")."]>";
	$tmp_grp=OCIResult($q,"GRP");
	OCIBindByName($q_type,":grp",$tmp_grp);
	OCIExecute($q_type,OCI_DEFAULT);
	while(OCIFetch($q_type)) {
		echo "<option value='".OCIResult($q_type,"ID")."'".(OCIResult($q_type,"ID")==OCIResult($q,"TYPE_ID")?" selected":"").">".OCIResult($q_type,"NAME")."</option>";
	}
	echo "</select>";
	if (OCIResult($q,"TYPE_ID")=='SE' or OCIResult($q,"TYPE_ID")=='MS' or OCIResult($q,"TYPE_ID")=='RA' or OCIResult($q,"TYPE_ID")=='CH' or OCIResult($q,"TYPE_ID")=='SA') {
		echo "<a href=edit_obj.php?obj_id=".OCIResult($q,"ID")."&form_id=".$form_id."><img src=edit.gif title=\"Редактировать\" border=0></a>";
	}
	//echo OCIResult($q,"TYPE_NAME");
	echo "</td>
	<td bgcolor=white><input type=text name=sav_obj_width[".OCIResult($q,"ID")."] size=2 value='".OCIResult($q,"WIDTH")."'></td>
	<td bgcolor=white><input type=text name=sav_obj_height[".OCIResult($q,"ID")."] size=2 value='".OCIResult($q,"HEIGHT")."'></td>
	<td bgcolor=white><input type=checkbox name=sav_obj_br[".OCIResult($q,"ID")."] value='Да'".(OCIResult($q,"BR")=="Да"?" checked":"")."></td>
	<td bgcolor=white width=3><input type=text name=sav_tag_after[".OCIResult($q,"ID")."] size=2 value='".OCIResult($q,"TAG_AFTER")."'></td>
	<td bgcolor=white><input type=checkbox name=sav_obj_sms[".OCIResult($q,"ID")."] value='Да'".(OCIResult($q,"SMS")=="Да"?" checked":"")."></td>
	<td bgcolor=white><input type=text name=sav_obj_sms_name[".OCIResult($q,"ID")."] size=20 value='".htmlentities(OCIResult($q,"SMS_NAME"))."'></td>";
	echo "<td bgcolor=white><html>";
	echo "<a href=\"?del_obj=1&obj_id=".OCIResult($q,"ID")."&form_id=".$form_id."\"><img src=del.gif title=\"Удалить\" border=0></a>";
	echo "</html></td>";
	echo "</tr>";
	}
	echo "</table>";
	...
	function ch_new_obj_type() {
		...
		if (document.all.new_obj_type.value=='SN') {
		document.all.new_width.value='200';
		document.all.new_height.value='3';		
		document.all.new_height.style.display='';
		document.all.new_width.style.display='';	
		document.all.new_tag_after.value='<hr>';
		}
...
if (isset($form_id) and $form_id<>'') {
	...
	show_form($c,$form_id,$_SESSION['project']['id'],'','','','','','','','y');
}
/*
...
function save_form($form_id,$form_name,$c,$send_cdpn,$send_cgpn,$send_agid,$show_ivr_sec,$show_queue_sec,$show_alerting_sec,$show_connected_sec,$show_connected_min,$show_call_sec,$show_call_min,$CODED_AON) {
	...
	$sql_text="insert into sc_forms (id,name,project_id,send_cdpn,send_cgpn,send_agid,show_ivr_sec,show_queue_sec,show_alerting_sec,show_connected_sec,show_connected_min,show_call_sec,show_call_min,CODED_AON)
	values (
	'".$new_form_id."',
	'".$form_name."',
	'".$_SESSION['project']['id']."',
	...
	$ins=OCIParse($c,$sql_text);
		if (@OCIExecute($ins,OCI_DEFAULT)) {
		OCICommit($c);
		$form_id=$new_form_id;
		} 
		else {
		echo "<font color=red>ОШИБКА! форма с таким именем уже существует!</font>";
		}
	}
	else {
	$upd=OCIParse($c,"update sc_forms set name='".$form_name."', send_cdpn='".$send_cdpn."', send_cgpn='".$send_cgpn."', send_agid='".$send_agid."',
	show_ivr_sec='".$show_ivr_sec."',show_queue_sec='".$show_queue_sec."',show_alerting_sec='".$show_alerting_sec."',show_connected_sec='".$show_connected_sec."', 
	show_connected_min='".$show_connected_min."', show_call_sec='".$show_call_sec."', show_call_min='".$show_call_min."', CODED_AON='".$CODED_AON."'
...
function sav_obj($sav_ordering,$sav_obj_name,$sav_obj_sms_name,$sav_obj_type,$sav_obj_width,$sav_obj_height,$sav_obj_br,$sav_obj_sms,$sav_tag_before,$sav_tag_after,$sav_requre,$c) {

	$upd=OCIParse($c,"update sc_form_object 
	set 
	ordering=nvl(:ord,ordering),
	name=:sav_obj_name,
	sms_name=:sav_obj_sms_name,
	type_id=nvl(:sav_obj_type,type_id),
	width=:sav_obj_width,
	...
	foreach($sav_ordering as $obj_id => $ord) {
		if(!isset($sav_obj_type[$obj_id])) $sav_obj_type[$obj_id]='';
		if(!isset($sav_obj_br[$obj_id])) $sav_obj_br[$obj_id]='';
		if(!isset($sav_obj_sms[$obj_id])) $sav_obj_sms[$obj_id]='';
		if(!isset($sav_requre[$obj_id])) $sav_requre[$obj_id]='';
		
$tmp_obj_name   = $sav_obj_name[$obj_id];
$tmp_obj_sms_name   = $sav_obj_sms_name[$obj_id];
$tmp_obj_type	= $sav_obj_type[$obj_id];		
$tmp_obj_width	= $sav_obj_width[$obj_id];		
$tmp_obj_height	= $sav_obj_height[$obj_id];		
$tmp_tag_before	= $sav_tag_before[$obj_id];	
$tmp_tag_after	= $sav_tag_after[$obj_id];
$tmp_obj_br	    = $sav_obj_br[$obj_id];
$tmp_obj_sms	= $sav_obj_sms[$obj_id];
$tmp_requre	    = $sav_requre[$obj_id];
		
		OCIBindByName($upd,":ord",$ord);
		OCIBindByName($upd,":sav_obj_name",			$tmp_obj_name);
		OCIBindByName($upd,":sav_obj_sms_name",		$tmp_obj_sms_name);
		OCIBindByName($upd,":sav_obj_type",			$tmp_obj_type);
		OCIBindByName($upd,":sav_obj_width",		$tmp_obj_width);
		OCIBindByName($upd,":sav_obj_height",		$tmp_obj_height);
		OCIBindByName($upd,":sav_tag_before",		$tmp_tag_before);
		OCIBindByName($upd,":sav_tag_after",		$tmp_tag_after);
		OCIBindByName($upd,":sav_obj_br",			$tmp_obj_br);
		OCIBindByName($upd,":sav_obj_sms",			$tmp_obj_sms);
		OCIBindByName($upd,":sav_requre",			$tmp_requre);
		OCIBindByName($upd,":obj_id",$obj_id,16);
		OCIExecute($upd,OCI_DEFAULT);		
	}
	OCICommit($c);	
