...
//Выбор списка
	...
	$q=OCIParse($c,"select * from sc_shedule where id='".$shedule_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
	...
	$q=OCIParse($c,"select * from sc_shedule where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' order by name");
	OCIExecute($q,OCI_DEFAULT);
	while (OCIFetch($q)) {
		echo "<option value=".OCIResult($q,"ID").">".OCIResult($q,"NAME")."</option>";
	}
...
and t.project_id='".$_SESSION['project_id'][$_SESSION['i']]."'
...
function save_shedule($shedule_id,$shedule_name,$c) {
	...
	$ins=OCIParse($c,"insert into sc_shedule (id,name,project_id) values ('".$new_shedule_id."','".$shedule_name."','".$_SESSION['project_id'][$_SESSION['i']]."')");
	OCIExecute($ins,OCI_DEFAULT); 
	OCICommit($c);
	$shedule_id=$new_shedule_id;
	}
	else {
	$upd=OCIParse($c,"update sc_shedule set name='".$shedule_name."' where id='".$shedule_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
...
function del_shedule($shedule_id,$c) {
	$del=OCIParse($c,"delete from sc_shedule where id='".$shedule_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
...
function add_worktime($shedule_id,$day_of_week,$start_mi,$start_hh,$end_mi,$end_hh,$c) {
	for ($i=0; $i<count($day_of_week); $i++) {
		...
		//if ($start_time<>'' or $end_time<>'') {
			...
			'".$shedule_id."','".$_SESSION['project_id'][$_SESSION['i']]."')");
			OCIExecute($ins,OCI_DEFAULT); 
			OCICommit($c);
		//}
	}
...
function del_worktime($worktime_id,$c) {
	$del=OCIParse($c,"delete from sc_shedule_times where id='".$worktime_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
	OCIExecute($del,OCI_DEFAULT); 
	OCICommit($c);
}
//
