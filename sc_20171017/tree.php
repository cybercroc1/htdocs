...
where t.punkt_id=p.id and p.deleted is null and p.project_id='".$_SESSION['project_id'][$_SESSION['i']]."'
...
$ins=OCIParse($c,"insert into sc_punkt (id,text,project_id) values ('".$punkt_id."',:node_name,'".$_SESSION['project_id'][$_SESSION['i']]."')");
...
if (isset($edit_node)) {
	...
	if($punkt_id=='main') {
	$upd=OCIParse($c,"update sc_projects set punkt_name='".$tree_name."' where id='".$_SESSION['project_id'][$_SESSION['i']]."'");
	OCIExecute($upd,OCI_DEFAULT);
	OCICommit($c);
	}
	else {
	...
	and p.project_id='".$_SESSION['project_id'][$_SESSION['i']]."'
...
where t.punkt_id=p.id and p.deleted is null and t.shedule_id=s.id(+) and p.project_id='".$_SESSION['project_id'][$_SESSION['i']]."'
connect by prior t.punkt_id=t.parent_id start with t.parent_id is null
order siblings by p.text");
OCIExecute($q,OCI_DEFAULT);

echo "ширина:<input type=text size=2 name=fr_width value=".$_SESSION['fr_width'][$_SESSION['i']]."><input type=submit name=width_save value=ok><hr>";
...
while(OCIFetch($q)) {
	...
	if (OCIResult($q,"LVL")>1) echo "<hr color=gray size=1>"; else echo "<hr color=gray size=2>";
...
if ($row_id>0) echo "<tr><td><hr color=gray size=2></td></tr>";
?>
</table>
</form>
<script>
