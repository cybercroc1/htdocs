...
if(isset($del_table)) {
$del=OCIParse($c,"delete from sc_dynamic_table where id='".$del_table."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
...
if($save_as=='table') {
	if(isset($other_project_id) and $other_project_id<>'') {
		$t=$table_id;
		$table_id='';
		$project_id=$_SESSION['project_id'][$other_project_id];
		echo $project_id;
		}
		else {		
...
if(!isset($no_change)) {
	...
	$ins_row=OCIParse($c,"insert into sc_dynamic_table_rows (table_id,row_num,attrib,style,height) 
	values ('".$table_id."',:row_num,'',:style,:height)");
	...
	values ('".$table_id."',:row_num,:cell_num,:attrib,:style,:html,:display,decode(:faq,'y',SEQ_SC_DYNAMIC_TABLE_FAQ_ID.nextval,:faq),:phones,:width,:height)");
	...
	foreach($row_height as $rownum => $height) {
		...
		foreach($col_width as $cellnum => $width) {
			...
			if($rownum>0 and $cellnum>0) {
				...
				if(isset($other_project_id) and $other_project_id<>'' and $faq[$rownum][$cellnum]*1>0) $faq[$rownum][$cellnum]='y'; 
...
if($save_as=='table' and (!isset($no_change) or (isset($other_project_id) and $other_project_id<>''))) {
	if(isset($other_project_id) and $other_project_id<>'') {
		$table_id=$t;
