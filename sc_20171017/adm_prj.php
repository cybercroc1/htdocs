...
if (isset($del_prj_go)) {
	...
	//удаление папки
	foreach (@glob($path_to_folders.$project_name."\\*.*") as $filename) {
		if (@!unlink($path_to_folders.$project_name."\\".basename($filename))) echo "<br><font color=red>Файл: ".basename($filename)." не удален!</font>";
	}
	if (@!rmdir($path_to_folders.$project_name)) echo "<br><font color=red>Папка: ".$project_name." не удалена!</font>";
	//
...
echo "<form name=projects action=adm_prj.php method=post>";
	...
	echo "<font size=4>Проекты</font> | <a href=adm_num.php>Номера</a> | <a href=adm_usr.php>Пользователи</a> | <a href=adm_holidays.php>Специальные дни</a><hr>";
	
	echo "<table id=tbl bgcolor=gray cellspacing=1 cellpadding=2>
	<tr>
	<td bgcolor=white align=center><b>ID</b></td>
	<td bgcolor=white align=center><b><a target='adm_prj_blank_frame' href='?order_by=p.name'>Название</b></td>
	<td bgcolor=white align=center><b><a target='adm_prj_blank_frame' href='?order_by=p.platform'>Октелл</b></td>
	<td bgcolor=white align=center><b><a target='adm_prj_blank_frame' href='?order_by=p.start_date'>Дата создания</a></b></td>
	<td bgcolor=white align=center><b><a target='adm_prj_blank_frame' href='?order_by=p.hidden'>Скрыт</b></td>
	<td bgcolor=white align=center><b><a target='adm_prj_blank_frame' href='?order_by=p.last_call_date'>Дата посл.зв.</b></td>
	<td bgcolor=white align=center><b>Номера</b></td>
	<td bgcolor=white align=center><b>Пользователи</b></td>
