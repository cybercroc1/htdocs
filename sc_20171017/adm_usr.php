...
echo "<form action=adm_usr.php method=post>"; //POST работает некорректно!
	...
	echo "<a href=adm_prj.php>Проекты</a> | <a href=adm_num.php>Номера</a> | <font size=4>Пользователи</font> | <a href=adm_holidays.php>Специальные дни</a><hr>";
	
//Выбор пользователя
	echo "<select name=login_id onchange=document.all.ch_login.click()>";
	if (!isset($login_id) or $login_id=='') {
	echo "<option value=''>СОЗДАТЬ ПОЛЬЗОВАТЕЛЯ</option>";
	$description='';
	$login='';
	$password='';
	$email='';
	...
	OCIFetch($q);
	echo "<option value=".OCIResult($q,"ID").">".OCIResult($q,"LOGIN")."</option>";
	...
	$q=OCIParse($c,"select * from sc_login where id<>1 order by login");
	OCIExecute($q,OCI_DEFAULT);
	while (OCIFetch($q)) {
		echo "<option value=".OCIResult($q,"ID").">".OCIResult($q,"LOGIN")."</option>";
	}
	echo "</select>
	<input type=submit name=ch_login value=ВЫБРАТЬ>";
