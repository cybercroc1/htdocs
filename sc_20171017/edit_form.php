...
if (isset($new_obj)) new_obj($form_id,$new_obj_name,$new_obj_type,$new_width,$new_height,$new_br,$new_tag_before,$new_tag_after,$c);
...
if (isset($sav_obj)) sav_obj($obj_id,$sav_obj_name,$sav_obj_type,$sav_obj_width,$sav_obj_height,$sav_obj_br,$sav_tag_before,$sav_tag_after,$sav_ordering,$c);
if (isset($copy_form)) {
	if (isset($copy_to_project) and isset($_SESSION['admin']) and $_SESSION['admin']==1) {
		...
		$q=OCIParse($c,"select id,name from sc_projects where id='".$_SESSION['project_id'][$_SESSION['i']]."'");
		OCIExecute($q,OCI_DEFAULT);
		OCIFetch($q);
		echo "<option value=".OCIResult($q,"ID").">".OCIResult($q,"NAME")."</option>";
		$q=OCIParse($c,"select id,name from sc_projects where id<>'".$_SESSION['project_id'][$_SESSION['i']]."' order by name");
		OCIExecute($q,OCI_DEFAULT);
			while (OCIFetch($q)) {
			echo "<option value=".OCIResult($q,"ID").">".OCIResult($q,"NAME")."</option>";
			}
...
if ($_SESSION['ch_email'][$_SESSION['i']]==1) echo "<a href=edit_email.php?form_id=".$form_id.">Редактирование е-мейлов</a><hr>";
...
from sc_forms
where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' and deleted is null");
OCIExecute($q,OCI_DEFAULT);
while (OCIFetch($q)) {
	$selected='';
...
if (isset($form_id) and $form_id<>'') {
	...
	$q=OCIParse($c,"select * from sc_form_obj_type");
	OCIExecute($q,OCI_DEFAULT);
	while (OCIFetch($q)) {
	echo "<option value=".OCIResult($q,"ID").">".OCIResult($q,"NAME")."</option>";
	}
	...
	o.invisible,o.ordering 
	from sc_form_object o, sc_form_obj_type t
	where o.type_id=t.id and o.form_id='".$form_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."' 
	order by ordering");
	OCIExecute($q,OCI_DEFAULT);
	while (OCIFetch($q)) {
...
echo "Внешний вид формы";
	echo "<table border=0 bgcolor=gray cellspacing=1 cellpadding=2><tr><td bgcolor=#EEFFEE>";
...
echo OCIResult($q_obj,"TAG_BEFORE");
	...
	//номер отчета
	if (OCIResult($q_obj,"TYPE_ID")=='NR' or OCIResult($q_obj,"TYPE_ID")=='NP') {
	...
	if (OCIResult($q_obj,"TYPE_ID")=='TI') {
		echo "<b>".OCIResult($q_obj,"NAME")."</b> (ID:".OCIResult($q_obj,"ID").")";
		if (OCIResult($q_obj,"BR")=='Да') echo "<br>";		
		echo "<input type=text size=1 maxlength=2>ч
		 <input type=text size=1 maxlength=2>м ";
	}
...
where form_id='".$form_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."' and ordering<'".$ordering."'");
	OCIExecute($q,OCI_DEFAULT);
	OCIFetch($q);
...
function new_obj($form_id,$new_obj_name,$new_obj_type,$new_width,$new_height,$new_br,$new_tag_before,$new_tag_after,$c) {
	$q=OCIParse($c,"select nvl(max(ordering),0)+1 ordering from sc_form_object
where form_id='".$form_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
	OCIExecute($q,OCI_DEFAULT);
	OCIFetch($q);
	$ordering=OCIResult($q,"ORDERING");
	$ins=OCIParse($c,"insert into sc_form_object (id,form_id,name,type_id,width,height,br,ordering,project_id,tag_before,tag_after)
	values (
...
function del_obj($form_id,$obj_id,$c) {
	...
	where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' and form_id='".$form_id."' and id='".$obj_id."'");
...
function save_form($form_id,$form_name,$c,$send_cdpn,$send_cgpn,$send_agid,$show_ivr_sec,$show_queue_sec,$show_alerting_sec,$show_connected_sec,$show_connected_min,$show_call_sec,$show_call_min,$CODED_AON) {
	...
	where id='".$form_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
...
function copy_form($form_id,$project_id,$c) {
	$old_project_id=$_SESSION['project_id'][$_SESSION['i']];
	if ($project_id=='') $new_project_id=$_SESSION['project_id'][$_SESSION['i']]; else  $new_project_id=$project_id;
	...
	SEQ_SC_FORM_OBJ_ID.nextval new_obj_id,id,'".$new_form_id."' form_id,name,type_id,width,height,br,invisible,ordering,project_id,tag_before,tag_after 
	from sc_form_object where form_id='".$form_id."' and project_id='".$old_project_id."'");
	OCIExecute($sel,OCI_DEFAULT);
		while (OCIFetch($sel)) {
	
			$ins2=OCIParse($c,"insert into sc_form_object (id,form_id,name,type_id,width,height,br,invisible,ordering,project_id,tag_before,tag_after)
			values (
			...
			$ins3=OCIParse($c,"insert into sc_form_values (id,name,obj_id,project_id,ordering,br)
	select SEQ_SC_FORM_VAL_ID.nextval,name,'".OCIResult($sel,"NEW_OBJ_ID")."','".$new_project_id."',ordering,br from sc_form_values where obj_id='".OCIResult($sel,"ID")."' and project_id='".$old_project_id."'");
...
function del_form($form_id,$c) {
	...
	$del=OCIParse($c,"delete from sc_body where form_id is not null and form_id='".$form_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
	OCIExecute($del,OCI_DEFAULT);

	$upd=OCIParse($c,"update sc_forms set deleted=sysdate where id='".$form_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
	OCIExecute($upd,OCI_DEFAULT);
	
	/*$del2=OCIParse($c,"delete from sc_forms where id='".$form_id."' and project_id='".$_SESSION['project_id'][$_SESSION['i']]."'");
	OCIExecute($del2,OCI_DEFAULT);*/

	OCICommit($c);	
}//
//Функция сохранения объекта
function sav_obj($obj_id,$sav_obj_name,$sav_obj_type,$sav_obj_width,$sav_obj_height,$sav_obj_br,$sav_tag_before,$sav_tag_after,$sav_ordering,$c) {
	
	$q=OCIParse($c,"select ordering,form_id from sc_form_object where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' and id='".$obj_id."'");
	OCIExecute($q,OCI_DEFAULT);
	OCIFetch($q);
	...
	where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' and id='".$obj_id."'");
	OCIExecute($upd,OCI_DEFAULT);
	OCICommit($c);	
}//
?>
...
if (!document.all.sav_obj) {
	...
	rows['tr_'+obj_id].cells[1].innerHTML='<input type=text name=sav_tag_before size=2 value="'+rows['tr_'+obj_id].cells[1].innerText+'">';
	v_name=rows['tr_'+obj_id].cells[2].innerText;
	rows['tr_'+obj_id].cells[2].innerHTML='<input type=text name=sav_obj_name size=35 value="'+v_name.replace('"','&quot;')+'">';
	//rows['tr_'+obj_id].cells[1].innerHTML='<input type=text name=sav_obj_name size=35 value="'+rows['tr_'+obj_id].cells[1].innerText+'">';

	rows['tr_'+obj_id].cells[3].innerHTML='<select name=sav_obj_type><option value='+type_id+'>'+type_name+'</option><option value=TE>Текст</option><option value=LT>Большой текст</option><option value=DA>Дата</option><option value=TI>Время</option><option value=SE>Выбор</option><option value=MS>Множественный выбор</option><option value=RA>Радио</option><option value=CH>Галочки</option><option value=CT>Телефон кодированный</option>';
	
	rows['tr_'+obj_id].cells[4].innerHTML='<input type=text name=sav_obj_width size=3 value='+rows['tr_'+obj_id].cells[4].innerText+'>';
	rows['tr_'+obj_id].cells[5].innerHTML='<input type=text name=sav_obj_height size=3 value='+rows['tr_'+obj_id].cells[5].innerText+'>';	
	
	if (rows['tr_'+obj_id].cells[6].innerText=='Да') {
	rows['tr_'+obj_id].cells[6].innerHTML='<input type=checkbox checked name=sav_obj_br value=Да>';}
	else {rows['tr_'+obj_id].cells[6].innerHTML='<input type=checkbox name=sav_obj_br value=Да>';}

	rows['tr_'+obj_id].cells[7].innerHTML='<input type=text name=sav_tag_after size=2 value="'+rows['tr_'+obj_id].cells[7].innerText+'">';
	rows['tr_'+obj_id].cells[8].innerHTML='<input type=hidden name=obj_id value='+obj_id+'><input type=submit name=sav_obj value=СОХРАНИТЬ>';
	}
}
}
</script>