...
if ($_SESSION['ch_email'][$_SESSION['i']]<>1) {echo "<font color=red>Страница недоступна!</font>"; exit();} 
?>
<?php
...
if ($_SESSION['ch_form'][$_SESSION['i']]==1) echo "<a href=edit_form.php?form_id=".$form_id.">Редактирование формы</a> ";
...
if (isset($form_id) and $form_id<>'') {
	...
	<td bgcolor=white><b>Отправлять</b></td>
	...
	echo "<td bgcolor=green colspan=2><input type=submit name=new_email onclick=\"javascript:if(!chk_new_email()){alert('email неверен');return false;}\" value=ДОБАВИТЬ></td></tr>";
	//
	//Емейлы
