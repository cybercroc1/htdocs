...
<table id=tbl2 border=1><tr>
<td>расписание: </td><td><select name=shedule_id>
<option value=''>Виден всегда</option>";
$q=OCIParse($c,"select id,name from sc_shedule where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' order by name");
...
$q=OCIParse($c,"select phone from SC_PHONES where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' order by phone");
OCIExecute($q,OCI_DEFAULT);
	$i=0;
	while (OCIFetch($q)) {
	$i++;
...
$q=OCIParse($c,"select * from sc_forms where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' /*and id not in (select form_id from sc_body
where form_id is not null and (punkt_id = '".$punkt_id."' or general='y'))*/");
OCIExecute($q,OCI_DEFAULT);
	while (OCIFetch($q)) {
	echo "<option value=".OCIResult($q,"ID").">".OCIResult($q,"NAME")."</option>";
	}
...
$q=OCIParse($c,"select * from sc_forw_list where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' /*and id not in (select forw_list_id from sc_body
...
function ch_blog_type() {
	...
	document.all.tbl2.rows[3].style.display='none';
...
if (!isset($form_id)) $form_id="";
	...
	$ins=OCIParse($c,"insert into sc_body (id,head,body,type,ordering,project_id,punkt_id,general,form_id,shedule_id,txt_tag_before,txt_tag_after,txt_align,forw_list_id,colapsed,new_window,faq) 
	values ((select nvl(max(id),0)+1 from sc_body),'".$quest."',:body,'".$blog_type."',".$ordering."+1,'".$_SESSION['project_id'][$_SESSION['i']]."','".$new_punkt_id."','".$general."','".$form_id."','".$shedule_id."','".$txt_tag_before."','".$txt_tag_after."','".$txt_align."','".$forw_list_id."','".$colapsed."','".$new_window."','".$faq."') returning id into :new_id");
	if ($answer=='') $answer=' ';
...
where b.id='".$blog_id."' and b.project_id='".$_SESSION['project_id'][$_SESSION['i']]."'
...
echo "<input type=submit name=edit_blog_go value=СОХРАНИТЬ>";
echo "</td>
<td>
<input type=text disabled size=2 value=\"<b>\"><b>текст</b><input type=text disabled size=2 value=\"</b>\"> - жирный шрифт<br>
<input type=text disabled size=2 value=\"<i>\"><i>текст</i><input type=text disabled size=2 value=\"</i>\"> - курсив<br>
...
if (isset($edit_blog_go)) {
	...
	OCIFetch($q);
		...
		$q=OCIParse($c,"select nvl(max(ordering),0)+1 new_ordering from sc_body where project_id='".$_SESSION['project_id'][$_SESSION['i']]."' and general='".$general."'");
		OCIExecute($q,OCI_DEFAULT);
		OCIFetch($q);
