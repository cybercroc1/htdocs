...
order by u.create_date desc");
	...
	if($err==0) {	
		...
		//иначе шлем СМС
			$res=send_sms_old('',$mob_phone,$code,'send_code');
			
			if(substr($res['result_text'],0,2)=="OK") {
...
if(isset($send_ank) and isset($_SESSION['user_id']) and isset($_SESSION['mob_phone']) and $err==0) {
	...
	$to_email=OCIResult($q,"EMAIL");
		...
		echo "Отправка EMAIL: ".$smtp_server.":".$smtp_port." - ".$to_email." - ".$email_res;
		...
		values
		(sysdate,'','','',:smtp_server,".$smtp_port.",:smtp_login,:smtp_from_email,:smtp_to_email,:subj,:smtp_result,".$smtp_dur_sec.")");
