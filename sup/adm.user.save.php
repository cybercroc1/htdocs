...
if(isset($user_id) and $user_id<>'') {
	...
	if((isset($save) or  isset($activate) or isset($send_reg_sms) or isset($send_reg_email)) and ($status=='active' or $status=='wait_activation' or $status=='deleted')) {
		...
		if(!isset($eval)) $eval='';
		if(!isset($trudozatrati)) $trudozatrati=''; 
		...
		if($grp_id<>'') {
			...
			if($create_new=='y' or $solution=='y' or $deny_close=='y' or $redirect=='y' or $look=='y' or $eval=='y' or $trudozatrati=='y' or $rep_stat=='y' or 
			...
			$sm_new<>'' or $sm_redir<>'' or $sm_prisv<>'' or $sm_delay<>'' or $sm_ready<>'' or $sm_close<>''                   or $sm_resume<>'') {
				...
				create_new, solution, deny_close, redirect, look, eval, trudozatrati, rep_stat, 
	...
	if((isset($activate) and $status=='wait_activation') or isset($send_reg_sms)) {
		...
		if(count($phones)>0) {
			...
			echo "Отправка СМС: ".$sms_result['result_text']."<hr>";
			if($sms_result['result_text']=='OK') {
				echo "<script>alert('Пользователю отправлено СМС с рег. данными');</script>";
			}
			else {
				echo "<script>alert('Ошибка отправки СМС: ".$sms_result['result_text']."');</script>";
			}
		}
	}		
	...
	if((isset($activate) and $status=='wait_activation') or isset($send_reg_email)) {
		...
		$i=0; while(OCIFetch($q)) {$i++;
			...
			echo "<hr>Отправка EMAIL: ".$to_email." - ".$email_res;
