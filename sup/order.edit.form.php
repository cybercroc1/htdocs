...
if($author=='y' or $executor=='y' or ($redirect=='y' and $opened=='y') or ($solution=='y' and $opened=='y') or $look=='y') {}
	...
	if($author_id<>'') {
		$q_tmp=OCIParse($c,"select distinct phone,ord from SUP_TEXNARI_PHONES t where texnari_id='".$author_id."' and contact='y' and valid_date is not null order by ord");
		...
		$q_tmp=OCIParse($c,"select distinct email from SUP_TEXNARI_emails where texnari_id = '".$author_id."' and valid_date is not null");
		OCIExecute($q_tmp,OCI_DEFAULT);
		//$mailtos=array();
		$i=0; while(OCIFetch($q_tmp)) {$i++;
	...
	echo "<div id=div_turdozatarti_isp></div>";
	
	//трудозатраты, одобренные заказчиком
	echo "<div id=div_turdozatarti_zak></div>";	
