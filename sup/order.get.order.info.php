...
function get_order_info($c,$base_id) {
   ...
   (select slt.location_id,slt.trbl_id, max(sla.create_new) create_new, max(sla.solution) solution, max(sla.deny_close) deny_close, max(sla.redirect) redirect, max(sla.look) look, max(sla.eval) eval, max(sla.trudozatrati) trudozatrati 
   ...
   where bbb.location_id(+)=aaa.klinika_id and bbb.trbl_id(+)=aaa.trbl_type_id";
	...
	if(OCIFetch($q)) {
		...
		$base['trudozatrati']=OCIResult($q,"TRUDOZATRATI");
