...
if(isset($user_id) and $user_id<>'') {
	...
	if($new_role_id=='') {//текущие настройки пользовтеля
		if($grp_id<>'') {
		    ...
		    a.look,a.solution,a.redirect,a.eval,a.trudozatrati,a.create_new,a.deny_close,a.rep_stat, --права
			...
			$sql_user="select u.admin,u.registrar,
		    '' look, '' solution, '' redirect, '' eval, '' trudozatrati, '' create_new, '' deny_close, '' rep_stat, --права
	...
	elseif($new_role_id=='clear') {
		...
		$sql_user="select 
	    t.look,t.solution,t.redirect,t.eval,t.trudozatrati,t.admin,t.create_new,t.deny_close,t.rep_stat,t.registrar, --права
	...
	//$role_HTML.="<td valign=top>";
		...
		if($grp_id<>'') {
			...
			$role_HTML.="<th align=right>Создана заявка<br><i>Создана, переадресована на группу (открыта), возобновлена (открыта)(пользователем или авто в 9:00)</i></th><td>".show_select('em_new',OCIResult($q_user,"EM_NEW"))."</td>";
			...
			$role_HTML.="<th align=right>Возобновлена<br><i>Возвращеная в работу (у заявки есть исполнитель) пользователем или авто в 9:00</i></th><td>".show_select('em_resume',OCIResult($q_user,"EM_RESUME"))."</td>";
			...
			$role_HTML.="<tr>";
			$role_HTML.="<td><input name=trudozatrati type=checkbox value='y'".(OCIResult($q_user,"TRUDOZATRATI")=='y'?' checked':'')."><b>Трудозатраты</b></input><br><i>Разрешает оценивать трудозатраты</i></td>";
