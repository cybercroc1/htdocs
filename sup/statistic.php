...
$sum_grp=array(); $sum=0; $sum_dub=0; $sum_kr=0; $sum_trudozatrati_ispolnitel=0; $sum_trudozatrati_zakazchik=0;
...
echo '<Row ss:Height="28">
    ...
    echo '<Cell ss:StyleID="s19"><Data ss:Type="String">общее</Data></Cell>
		...
		<Cell ss:StyleID="s19"><Data ss:Type="String">Трудозатраты исполнителя(час)</Data></Cell>
		<Cell ss:StyleID="s19"><Data ss:Type="String">Трудозатраты одобрено(час)</Data></Cell>
   </Row>';



$q2=OCIParse($c,"select count(*) cnt, count(dublikat) cnt_dub, count(krivie_ruki) cnt_kr, 
sum(trudozatrati_ispolnitel) sum_trudozatrati_ispolnitel, sum(trudozatrati_zakazchik) sum_trudozatrati_zakazchik
from 
(select distinct b.id,b.dublikat,b.krivie_ruki, b.trudozatrati_ispolnitel, b.trudozatrati_zakazchik from sup_base b, sup_klinika k
where b.date_in_call between to_date(:dat,'DD.MM.YYYY') and to_date(:dat,'DD.MM.YYYY')+1
".($obj_ids[0]=='all'?"and b.klinika_id is not null":"and b.klinika_id in (".implode(",",$obj_ids).")")."
".($user_ids[0]=='all'?"and b.texnari_id is not null":"and b.texnari_id in (".implode(",",$user_ids).") --считаются все заявки или только назначенные")."
and k.id=b.klinika_id
and k.location_grp_id=nvl(:loc_grp,k.location_grp_id)
".($trbl_ids[0]=='all'?"and b.trbl_type_id is not null":"and b.trbl_type_id in (".implode(",",$trbl_ids).")")."
)");
...
while ($date1<=$date2) {
	...
	echo '<Cell ss:StyleID="s25">'; echo OCIResult($q2,"SUM_TRUDOZATRATI_ISPOLNITEL")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q2,"SUM_TRUDOZATRATI_ISPOLNITEL")/60).'</Data>'; echo'</Cell>';
	echo '<Cell ss:StyleID="s25">'; echo OCIResult($q2,"SUM_TRUDOZATRATI_ZAKAZCHIK")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q2,"SUM_TRUDOZATRATI_ZAKAZCHIK")/60).'</Data>'; echo'</Cell>';
	echo '</Row>';
	$sum+=OCIResult($q2,"CNT");
	$sum_dub+=OCIResult($q2,"CNT_DUB");
	$sum_kr+=OCIResult($q2,"CNT_KR");
	$date1=strtotime("+1 day", $date1);
	$sum_trudozatrati_ispolnitel+=OCIResult($q2,"SUM_TRUDOZATRATI_ISPOLNITEL");
	$sum_trudozatrati_zakazchik+=OCIResult($q2,"SUM_TRUDOZATRATI_ZAKAZCHIK");
}
	//ИТОГО
	echo '<Row>
	...
	<Cell ss:StyleID="s25"><Data ss:Type="Number">'.str_replace(",",".",$sum_trudozatrati_ispolnitel/60).'</Data></Cell>
	<Cell ss:StyleID="s25"><Data ss:Type="Number">'.str_replace(",",".",$sum_trudozatrati_zakazchik/60).'</Data></Cell>
	</Row>';
...
".($trbl_ids[0]=='all'?"and tt.id is not null":"and tt.id in (".implode(",",$trbl_ids).")")."
...
while (OCIFetch($q1)) {
	...
	echo '<Cell ss:StyleID="s18">'; echo OCIResult($q2,"SUM_TRUDOZATRATI_ISPOLNITEL")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q2,"SUM_TRUDOZATRATI_ISPOLNITEL")/60).'</Data>'; echo'</Cell>';
	echo '<Cell ss:StyleID="s18">'; echo OCIResult($q2,"SUM_TRUDOZATRATI_ZAKAZCHIK")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q2,"SUM_TRUDOZATRATI_ZAKAZCHIK")/60).'</Data>'; echo'</Cell>';	  
   echo '</Row>';
	...
	while(OCIFetch($q3)) {
		...
		<Cell ss:StyleID="s181"
     	ss:Formula="=CONCATENATE(INT(INT(RC[-1])/24),&quot;д &quot;,INT(RC[-1]-INT(INT(RC[-1])/24)*24),&quot;ч &quot;, ROUND((RC[-1]-INT(RC[-1]))/100*6000,0),&quot;м &quot;)"><Data
      ss:Type="String"></Data></Cell>';
	echo '<Cell ss:StyleID="s181">'; echo OCIResult($q4,"SUM_TRUDOZATRATI_ISPOLNITEL")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q4,"SUM_TRUDOZATRATI_ISPOLNITEL")/60).'</Data>'; echo'</Cell>';
	echo '<Cell ss:StyleID="s181">'; echo OCIResult($q4,"SUM_TRUDOZATRATI_ZAKAZCHIK")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q4,"SUM_TRUDOZATRATI_ZAKAZCHIK")/60).'</Data>'; echo'</Cell>';	   
...
".($obj_ids[0]=='all'?"and k.id is not null":"and k.id in (".implode(",",$obj_ids).")")."
...
sum(trudozatrati_ispolnitel) sum_trudozatrati_ispolnitel, sum(trudozatrati_zakazchik) sum_trudozatrati_zakazchik  
from
(select distinct b.id,b.date_in_call,b.in_work,b.ready_to_close,b.date_close,b.dublikat,b.krivie_ruki,b.trudozatrati_ispolnitel,b.trudozatrati_zakazchik 
from sup_base b
where b.date_in_call between to_date('".$start_date_day."','DD.MM.YYYY') and to_date('".$end_date_day."','DD.MM.YYYY')+1
...
while (OCIFetch($q1)) {
	...
	<Cell ss:StyleID="s17"
     ss:Formula="=CONCATENATE(INT(INT(RC[-1])/24),&quot;д &quot;,INT(RC[-1]-INT(INT(RC[-1])/24)*24),&quot;ч &quot;, ROUND((RC[-1]-INT(RC[-1]))/100*6000,0),&quot;м &quot;)"><Data
      ss:Type="String"></Data></Cell>';
	echo '<Cell ss:StyleID="s17">'; echo OCIResult($q2,"SUM_TRUDOZATRATI_ISPOLNITEL")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q2,"SUM_TRUDOZATRATI_ISPOLNITEL")/60).'</Data>'; echo'</Cell>';
	echo '<Cell ss:StyleID="s17">'; echo OCIResult($q2,"SUM_TRUDOZATRATI_ZAKAZCHIK")==0?'':'<Data ss:Type="Number">'.str_replace(",",".",OCIResult($q2,"SUM_TRUDOZATRATI_ZAKAZCHIK")/60).'</Data>'; echo'</Cell>';	   
...
from sup_user su
where 
".($user_ids[0]=='all'?"su.id is not null":"su.id in (".implode(",",$user_ids).")")."
...
round(avg(nvl(in_work,nvl(ready_to_close,date_close))-date_in_call)*24,2) avg_react, sum(trudozatrati_ispolnitel) sum_trudozatrati_ispolnitel, sum(trudozatrati_zakazchik) sum_trudozatrati_zakazchik   
from
(select distinct b.id,b.date_in_call,b.in_work,b.ready_to_close,b.date_close,b.dublikat,b.krivie_ruki,b.trudozatrati_ispolnitel,b.trudozatrati_zakazchik 
from sup_base b
where b.date_in_call between to_date('".$start_date_day."','DD.MM.YYYY') and to_date('".$end_date_day."','DD.MM.YYYY')+1
...
echo "<select multiple id=trbl_ids name=trbl_ids[]><option value=all selected>ВЫБРАТЬ ВСЕ</option>";
...
echo "<select multiple id=obj_ids name=obj_ids[]><option value=all selected>ВЫБРАТЬ ВСЕ</option>";
...
echo "<select multiple id=user_ids name=user_ids[]><option value=all selected>ВЫБРАТЬ ВСЕ</option>";
