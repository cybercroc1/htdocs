...
if($last_change<>$old_last_change) {
	echo "<script>alert('Ошибка! Заявка модифицирована другим пользователем. Действие отменено!')</script>";
...
//трудозатраты, оценненные исполнителем (710)
if(isset($new_turdozatarti_isp) and isset($old_turdozatarti_isp) and $new_turdozatarti_isp <> $old_turdozatarti_isp) {
	$result=710;
	
	$ins=OCIParse($c,"insert into sup_texnari_history (id,base_id,texnari_id,texnary_coment,datetime,result_call)
	values (sup_history_id.nextval, '".$base_id."','".$_SESSION['user_id']."',:coment,sysdate,'".$result."') returning id into :hist_id");
	$trud_comment='Оценка трудозатрат исполнителем: '.($new_turdozatarti_isp/60).' часов';
	OCIBindByName($ins,":coment",$trud_comment);
	OCIBindByName($ins,":hist_id",$hist_id,16);
	OCIExecute($ins,OCI_DEFAULT);
		
	$upd=OCIParse($c,"update sup_base 
	set trudozatrati_ispolnitel='".$new_turdozatarti_isp."', last_change=sysdate 
	where id='".$base_id."'");
	OCIExecute($upd,OCI_DEFAULT);
	OCICommit($c);
}
...
//трудозатраты, одобренные заказчиком (720)
if(isset($new_turdozatarti_zak) and isset($old_turdozatarti_zak) and $new_turdozatarti_zak <> $old_turdozatarti_zak) {
	$result=720;
	
	$ins=OCIParse($c,"insert into sup_texnari_history (id,base_id,texnari_id,texnary_coment,datetime,result_call)
	values (sup_history_id.nextval, '".$base_id."','".$_SESSION['user_id']."',:coment,sysdate,'".$result."') returning id into :hist_id");
	$trud_comment='Трудозатраты, одобренные заказчиком: '.($new_turdozatarti_zak/60).' часов';
	OCIBindByName($ins,":coment",$trud_comment);
	OCIBindByName($ins,":hist_id",$hist_id,16);
	OCIExecute($ins,OCI_DEFAULT);
		
	$upd=OCIParse($c,"update sup_base 
	set trudozatrati_zakazchik='".$new_turdozatarti_zak."', last_change=sysdate 
	where id='".$base_id."'");
	OCIExecute($upd,OCI_DEFAULT);
	OCICommit($c);
}
...
if($result==110) {
    ...
    and (
       (sla.em_new in ('my','all') and su.id in (b.kto_id,b.texnari_id)) --если хотя бы в одной из групп пользователь должен получать свои заявки, то свои получает независимо от группы    
...
if($result==110) {
	...
	and slt.trbl_id=b.trbl_type_id and slt.location_id=b.klinika_id 
	and (
       (sla.sm_new in ('my','all') and su.id in (b.kto_id,b.texnari_id)) --если хотя бы в одной из групп пользователь должен получать свои заявки, то свои получает независимо от группы    
