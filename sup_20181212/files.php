...
if(isset($_FILES['new_file'])) {
	...
	foreach($_FILES['new_file']['name'] as $key => $val) {
		...
		if($size<1) {
			$div_InnerHTML.="<font color=red>Файл ".$name." не загружен: нулевой размер; </font>";
			continue;
		}
		if($size>3000000) {
			$div_InnerHTML.="<font color=red>Файл ".$name." не загружен: размер превышает 3 МБ; </font>";
			continue;
		}		
...
if($div_InnerHTML<>'') echo "<script>parent.document.getElementById('div_tmp_files').innerHTML='".str_replace("'","\'",$div_InnerHTML)."<hr>';</script>";
else echo "<script>parent.document.getElementById('div_tmp_files').innerHTML='';</script>";
?>
