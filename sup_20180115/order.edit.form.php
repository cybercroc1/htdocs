...
if($author=='y' or $executor=='y' or ($redirect=='y' and $opened=='y') or ($solution=='y' and $opened=='y') or $look=='y') {}
	...
	$i=0; while(OCIFetch($q_files)) { $i++;
		echo "<a href='files.php?download&id=".OCIResult($q_files,"ID")."'>".OCIResult($q_files,"FILENAME")."</a> <a href='files.php?del&id=".OCIResult($q_files,"ID")."&base_id=".$base_id."&sid=".$sid."' target='logFrame'><font color=red title='Удалить'>x</font></a>; ";
	}
	if($i>0) echo "<hr>";
	echo "</div>";
