...
if (isset($new_user)) {
	foreach($new_user as $idx => $null) {
		...
		echo "Добавлен пользователь \"".$new_fio[$idx]."\"<hr>";
		...
		select ".$new_user_id.", ug.group_id from STC_USER_GRP_USR ug, STC_USER_GROUP g where g.id=ug.group_id and ((g.default_group='y' and ug.user_id=".$_SESSION['user']['id'].") or ug.group_id='".$group_id."')");
		OCIExecute($ins,OCI_DEFAULT);		
