...
function del() {
	if(confirm('Будут удалены только необработанные (с пустым статусом) записи из выбранных загрузок. Удалить?')) {
...
t.null_rows, t.dublicates,t.status
...
order by ".$_SESSION['adm']['loads']['order_by']);
	...
	while (OCIFetch($q)) {
		...
		<td>".(OCIResult($q,"STATUS")<>'Загружается...'?"<input type=checkbox name=mark[] value='".OCIResult($q,"ID")."'>":NULL)."</td>
