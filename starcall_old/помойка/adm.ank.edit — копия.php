...
echo "<font align=right><a href='adm.ank.edit.help.html' target='_blank'>Справка</a></font>";
echo "<hr>";
...
$q_obj=OCIParse($c,"select o.id obj_id, o.obj_type_id, o.num_on_group obj_num, o.quest_num, o.message, nvl(o.answ_ord_type,'По порядку') answ_ord_type, o.must,
f.id field_id, f.text_name, f.code_name, f.quoted,
...
$i=0; while(OCIFetch($q_grp)) { //группы
	...
	if(OCIResult($q_grp,"PAGE_NUM")<>$page_num) {$i++;
		...
		echo "<input type=hidden name='obj_idx[$i]' value='page'><input type=hidden name='obj_id[$i]' value='".$grp_id."'>";
	...
	//только новая группа
		...
		echo "<input type=hidden name='obj_idx[$i]' value='group'><input type=hidden name='obj_id[$i]' value='".$grp_id."'>";
		echo $page_num.".".$grp_num."<br>(".$grp_id.")";
		echo "</td>";
		echo "<td>";
	...
	$last_obj_type=OCIResult($q_obj,"OBJ_TYPE_ID");
		...
		if(substr(OCIResult($q_obj,"OBJ_TYPE_ID"),0,5)=='q_sn_') {
			echo "<th></th>";
			echo "<td style='cursor:s-resize' onMouseDown='fMD(this)' onMouseUp='fMU(this)'>";
			echo "<input type=hidden name='obj_idx[$i]' value='".OCIResult($q_obj,"OBJ_ID")."'><input type=hidden name='obj_id[$i]' value='".OCIResult($q_obj,"OBJ_ID")."'>";
		...
		//вопросы с выбором
			...
			echo "</td>";
			echo "<td>об<input type=checkbox name=must[$i] ".(OCIResult($q_obj,"MUST")<>''?' checked':NULL)." onchange='notsaved()'></td>";
			echo "<td>кв<input type=checkbox name=quoted[$i] ".(OCIResult($q_obj,"QUOTED")<>''?' checked':NULL)." onchange='notsaved()'></td>";
		...
		//все остальное
			...
			echo $page_num.".".$grp_num.".".OCIResult($q_obj,"OBJ_NUM")." (".OCIResult($q_obj,"OBJ_ID").")"; 
			echo "</td>";
			echo "<td>".OCIResult($q_obj,"OBJ_TYPE_NAME")."</td>";
			echo "<td colspan=6><textarea name=message[$i] onchange='notsaved()' style='width:100%'>".OCIResult($q_obj,"MESSAGE")."</textarea></td>";
			//echo "<td colspan=5><input type=text style='width:100%' name=message[$i] value='".OCIResult($q_obj,"MESSAGE")."' onchange='notsaved()'></td>";
			echo "</tr>";		
		}
