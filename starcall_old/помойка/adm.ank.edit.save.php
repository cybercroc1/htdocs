...
order by num");
	...
	$i=0; foreach($obj_id as $idx => $id) {$i++; 	//ключ - индекс объекта на странице редактирования; значения: для существующих - ID объекта; для новых: new
		...
		if($p==0 and $type[$idx]=='obj' and $obj_type_id[$idx]<>'') {$p++; $g++; $g_on_p=1; $o_on_g=0; //если первый элемент анкеты объект (не страница и не группа) и не пустой, то создаем страницу и группу
			$def_name='';
		...
		if($type[$idx]=='page' or $type[$idx]=='group') {$g++; $g_on_p++; $o_on_g=0; //страница и группа
			if($type[$idx]=='page' or $p==0) {$p++; $g_on_p=1;} //если первая группа не начинает новую страницу, то добавляем страницу

			if($id<>'new')	{ //если группа существует, то обновляем имя и порядковый номер
				$current_grp_id=$id;
		...
		//объекты
		if($type[$idx]=='obj') {
			if($id=='new' and $obj_type_id[$idx]=='') { //игнорим пустые новые объекты 
				continue;
			} 
			$o++;
			$o_on_g++;
			...
			if($id<>'new') { //обновляем существующие
...
where i.project_id=".$project_id." and i.object_id in (".implode(",",$add_quoted).")
	...
	//КВОТЫ. Удаляем индексы для отмененных квот
	if(isset($del_quoted)) {
	...
	if(isset($changed_quote)) {
		$upd=OCIParse($c,"update STC_PROJECTS set QST_QUOTE_BROKEN='yes', status='Приостановлен' where id='".$project_id."'");
		OCIExecute($upd,OCI_DEFAULT);
	}	
	OCICommit($c);
