...
if (!isset($_SESSION['user']['id'])) {
	if (isset($starcall_user) and isset($starcall_pass)) {
		$q=OCIParse($c,"select u.id, u.role_id, u.fio, 
		...
		if (OCIFetch($q)) {
			if(OCIResult($q,"OTHER_LOCK")=='y') {
				session_destroy();
				echo "<font color=red>Данный ползователь уже подключен в другом месте или не вышел из системы, попробуйте через 2 минуты!</font>";
			}
			else {
		...
		else echo "<font color=red>Не верное имя или пароль!</font>";
	}
}
if(isset($_SESSION['user']['id'])) {
	if($_SESSION['user']['operator_only']=='y') echo "<script>document.location='survey.projects.php'</script>";
										   else echo "<script>document.location='adm.main.frame.php'</script>";
}
?>
