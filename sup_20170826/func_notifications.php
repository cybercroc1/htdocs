...
function create_notice($base_id,$result,$tex_coment) {
	...
	if(OCIResult($q,"KTO_ID")<>'') {
		$q_tmp=OCIParse($c,"select phone from SUP_TEXNARI_PHONES t where texnari_id='".OCIResult($q,"KTO_ID")."' and contact='y' and valid_date is not null order by ord");
	...
	//
	$mail_mess.="</tr></table>";
	$mail_mess.="<a href='http://sup.wilstream.ru?ticketId=".$base_id."' target=_balnk>sup.wilstream.ru</a>";
}
?>