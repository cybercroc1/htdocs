...
if($_SESSION['solution']=='y' or $_SESSION['redirect']=='y') {
	...
	if($_SESSION['redirect']=='y') {//можно выбрать только локации из групп, в которых есть стрелочники или исполнители
...
if($_SESSION['solution']=='y' or $_SESSION['redirect']=='y') {
	//тип проблемы
	if($_SESSION['redirect']=='y') { //можно выбрать только типы проблем из групп, в которых есть стрелочники или исполнители
...
$i++;	
	...
	if($from_user_id=='' and OCIResult($q,"ID")==$_SESSION['user_id']) $to_user_id_opts.=" selected"; //если заявка еще не принята в работу, то по умолчанию - присвоить
	$to_user_id_opts.=">".OCIResult($q,"FIO")."</option>";	
}

if($i==0) {//если нет инженеров для решения выбранного типа проблемы
		
	$callback_who_opts.="<option value='".$_SESSION['user_id']."'>".$_SESSION['fio']."</option>";
	
	$to_user_id_opts.="<option value='' style='color:indigo'>оставить комментарий</option>";
	$comm='y';
	$to_user_id_opts.="<option value='".$_SESSION['user_id']."'";
	$to_user_id_opts.=" style='color:green'";
	//if($from_user_id=='') $to_user_id_opts.=" selected"; //если заявка еще не принята в работу, то по умолчанию - присвоить
...
if($_SESSION['redirect']=='y') {
	$to_user_id_HTML.="<font color=indigo>Комменитровать</font> / <font color=maroon>Переадресовать</font> / <font color=green>Присвоить заявку</font>: ";
	$to_user_id_HTML.="<nobr>";
	$to_user_id_HTML.="<select name=to_user_id onchange=fn_check()>";
	//если пользователь имеет право переадресовывать и при этом выбрал локацию и тип проблемы, которую не имеет права решить, то он может обнулить пользователя, что бы заявку принял тот, кто имеет право решать ее.	
	$q=OCIParse($c,"select 'х' from SUP_USER_LT_ALLOC sla, SUP_LT lt, sup_lt_group slg
	where lt.location_id='".$new_location_id."'
	...
	OCIExecute($q,OCI_DEFAULT);
	if(OCIFetch($q)) {
		if(isset($comm)) $to_user_id_HTML.="<option value='' style='color:indigo'>оставить комментарий</option>";
	}
	else {
		//if($from_user_id<>'') $to_user_id_HTML.="<option value='group' style='color:maroon'>переадресовать в группу инженеров (открыть заявку)</option>";
	}
	$to_user_id_HTML.=$to_user_id_opts;
	/*if($from_user_id<>'')*/ $to_user_id_HTML.="<option value='group' style='color:blue'>переадресовать на группу инженеров (открыть заявку)</option>";
	$to_user_id_HTML.="</select>";
}
if($_SESSION['solution']=='y' or $_SESSION['redirect']=='y' or $_SESSION['eval']=='y' or $_SESSION['create_new']=='y') {
	$to_user_id_HTML.=" <nobr><input type=submit disabled name=save style='background-color:#66FF66' value='Сохранить'>";
}
$to_user_id_HTML.="</nobr>";
