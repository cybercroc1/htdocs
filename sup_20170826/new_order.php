...
//не авторизованный пользователь (оператор)
	...
	if($lt_grp_ids=='') {
		echo "<font color=red><b>ОШИБКА: Не указана группа или группа не существует</b></font>";
		exit();
	}
}
if($lt_grp_ids=='') {
	echo "<font color=red><b>ОШИБКА: Вам не назначена ни одна группа</b></font>";
	exit();
}
//
...
echo "<hr>";	
echo "<b><font color=red size=4>*</font>ФИО, номер места/добавчного<br>(у кого не работает):</b><br> <input type=text size=30 name=u_kogo style='width:97%' value='".$u_kogo."' onkeyup='check()'></input><hr>";
//echo "</td>";
...
check();
function check() {
	if (frm.location_id.value=='' || frm.oper_coment.value=='' || frm.trbl_id.value=='' || frm.u_kogo.value=='' || ('kto' in frm && frm.kto.value=='')) frm.send.disabled=true;
...
$cont_phones=array();
if($kto_id<>'') {
	$q_tmp=OCIParse($c,"select phone from SUP_TEXNARI_PHONES t where texnari_id='".$_SESSION['user_id']."' and contact='y' and valid_date is not null order by ord");
...
$from_email='noreply@wilstream.ru';
...
$mess.="</b><hr>Суть проблемы: <b>".nl2br($oper_coment)."</b></td></tr>
</table>";
$mess.="<a href='http://sup.wilstream.ru?ticketId=".$num_zayavki."' target=_balnk>sup.wilstream.ru</a>";
//

	//Шлём только тем, кто может заниматься этой конкретной проблемой в конкретном месте, кроме себя ))
	$q=OCIParse($c,"select distinct ste.email from sup_trbl_type stt, sup_lt slt, sup_user_lt_alloc sla, sup_user su, sup_texnari_emails ste
	where stt.id=".$trbl_id." and slt.location_id='".$location_id."' 
	and slt.trbl_id=stt.id 
