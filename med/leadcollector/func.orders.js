//—ќ–“»–ќ¬ ј---------------------------------
function show_all_orders() { //отображеиние всех значков сотрировки, в соотвествие с нистройками
...
function get_order_elem_by_cell(cell) { //получение ссылки на див с параметрами сотрировки дл€ заданной €чейки
		elem=cell.childNodes[0].childNodes[0].rows[0].cells[1];
		if(elem) return elem;		
		//старый вариант с дивами:
		/*for(j=0;j<cell.childNodes.length;j++) { //перебор дочерних эелементов €чейки
			elem=cell.childNodes[j]; 
			if(elem.tagName=='DIV') {
				if(elem.childNodes[1]) { //вотрой элемент в €чейке - это див с отображением значка сортировки
