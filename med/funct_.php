...
//echo "<br><textarea>".$sqlstr."</textarea>";
    ...
    static function GetServiceDetails($strID = NULL, $iService = SERVICE_NOT, $bDeleted = TRUE)
    {
        if (DB_OCI)
            $sqlstr = "SELECT ID, SERVICE_ID, NAME, to_char(Deleted,'dd.mm.yyyy hh24:mi:ss') DELETED FROM SERVICE_DET WHERE 1=1";
...
to_char(sb.DATE_ADD,'dd.mm.yyyy hh24:mi:ss') AS DATE_ADD, RUB, sa.deleted 
FROM SOURCE_AUTO sa
 ...
 FROM SOURCE_AUTO_DETAIL sad, SOURCE_AUTO sa WHERE sa.ID = sad.SOURCE_AUTO_ID";
    ...
    departament_id IN (SELECT dep_id FROM user_dep_alloc WHERE user_id = " . $_SESSION['login_id_med'] . "))";*/
        ...
        if (DB_OCI) {
            ...
            OCIExecute($query, OCI_DEFAULT);
            if ($BNumber) {
                self::$array_source_auto = OCI_Fetch_Row($query);
				$nrows = 1;
			}
            else {
    ...
    static function GetUsersDep($bDeleted = TRUE, $strWhere = NULL, $income = NULL, $bActive = 'not')
    {
        ...
        if (!isset($_SESSION['sec_chance_arr'])) { // Если не выходии из программы
        ...
        $selectstr .= " AND usr.ID != ".SPEC_USER. " AND usr.ID not in (".implode(',',$_SESSION['sec_chance_arr']).")";
//var_dump($selectstr);
        ...
        if (DB_OCI) {
            ...
            $_POST['array_userd'] = self::$array_userd;
            oci_free_statement($query);
        } else {
...
WHERE usr.id = uda.user_id AND usr.ROLE_ID = ".USER_SUPER;
    ...
    static function GetCallByCallId($sc_call_id = '', $oktell_idchain = '')
    {
        ...
        elseif ('' != $sc_call_id) $sqlstr = "SELECT * FROM ".$table_name." WHERE SC_CALL_ID = " . $sc_call_id ." ORDER BY ID DESC";
        else return $nrows;
