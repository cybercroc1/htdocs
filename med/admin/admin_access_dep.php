...
//    "ORDER BY dep.NAME, sr_a.NAME, sr_man.NAME, serv.NAME";
    ...
    <form action="" method="POST">
        ...
        if (GetData::GetCallType() > 0) {
            // вставить проверку на соответствие
            if (DB_OCI) {
                foreach(GetData::$array_ctype as $key => $value) {
                    if (TRUE == ENCODE_UTF) {
    ...
    function OnClick()
    {
        ...
        if (isset($S_Auto) && isset($S_Man) && isset($S_Type) && isset($ServName)) {
            $checkstr = "SELECT departament_id FROM ACCESS_DEP 
                        WHERE source_auto_id = {$S_Auto} AND source_type_id = {$S_Type} 
                        AND source_man_id = {$S_Man} AND service_id = {$ServName}";
            if (DB_OCI) {
        ...
        if ($result[0] === $_POST['Depart']) {
            ...
            $insertstr = "INSERT INTO ACCESS_DEP (id, departament_id, source_auto_id, source_man_id, source_type_id, service_id)
                VALUES (SEQ_ACCESS_DEP_ID.nextval, {$_POST['Depart']}, {$S_Auto}, {$S_Man}, {$S_Type}, {$ServName} )";
            if (DB_OCI) {
