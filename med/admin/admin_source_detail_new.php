...
if (isset($getdet))
{
 ...
 LEFT JOIN SOURCE_AUTO_DETAIL sad ON sad.ID = saa.SA_DETAIL_ID";
    ...
    OCIExecute($query, OCI_DEFAULT);
    $array_sa_detail = array();
    $nDetailRows = OCI_Fetch_All($query,$array_sa_detail, 0, -1, OCI_FETCHSTATEMENT_BY_ROW);
    oci_free_statement($query);
//var_dump($array_sa_detail);
    if ($nDetailRows > 0) {
        foreach ($array_sa_detail as $key => $value) {
            $id = $value['ID'];
            $name = $value['SADNAME'];
...
<form action="" method="post" style="margin-bottom: 0">
    ...
    echo "<select id='S_Auto' name='S_Auto' onchange='ifr_all.location=\"admin_source_detail_new.php?getdet=\"+this.value'>";
