...
if (isset($save) and $S_Auto > 0 && isset($source_det_ids)) {
    foreach($source_det_ids as $source_det_id => $fucking_val) {
        ...
        $checkstr = "SELECT SERVICE_IDS FROM SOURCE_AUTO_DETAIL WHERE ID = ".$source_det_id." and SOURCE_AUTO_ID=".$S_Auto;
...
if (isset($getdet))
{
    ...
    //$sel .= "<th style='width: 101px;'>Сохранение</th>";
    $sel .= "</tr></thead>";
    $sel .= "<tbody>";
    if (SOURCE_ALL == $getdet)
        $filt_str = NULL;
    else $filt_str = "SOURCE_AUTO_ID=".$getdet;
    ...
    if ($nDetailRows > 0) {
        foreach(GetData::$array_sa_detail as $key => $value) {
            ...
            //$sel .= "<td style='text-align: center; width: 100px'><a href='?update_id=" . $value['ID'] . "'>Записать в базу</a></td>";
            $sel .= "</tr>";
        }
    }
    $sel .= "</tbody>";
...
//var_dump($insertstr);
    ...
    if ($query_result) { // Записываем детализацию
        ...
        $checkstr = "SELECT ID FROM SOURCE_AUTO_DETAIL WHERE SOURCE_AUTO_ID = {$_POST['S_Auto']} AND NAME = '{$_POST['NameIst']}'";
        if (DB_OCI) {
        ...
        if ($count >= 1) {
            ...
            /*if (SOURCE_NOT == $_POST['Reservoir']) // добавился новый источник рекламы
                $insertstr = "INSERT INTO SOURCE_AUTO_DETAIL (ID, SOURCE_AUTO_ID, SOURCE_MAN_ID, SERVICE_IDS) VALUES ( seq_source_alloc_id.NEXTVAL, '{$_POST['S_Auto']}', seq_source_man_id.CURRVAL, '{$serv_list}' )";
            else*/
            $insertstr = "INSERT INTO SOURCE_AUTO_DETAIL (ID, SOURCE_AUTO_ID, SERVICE_IDS, NAME) VALUES ( seq_source_ad_id.NEXTVAL, {$_POST['S_Auto']}, '{$service_list}', '{$_POST['NameIst']}' )";
            if (DB_OCI) {
                $query = OCIParse(GetData::GetConnect(), $insertstr);
                $query_result = OCIExecute($query);
                oci_free_statement($query);
            ...
            if ($query_result) {
                ...
                echo "<p style='color: green'>Новая детализация успешно добавлена в таблицу. Идет перезагрузка данных...</p>";
            } else {
...
//Изменение в чекбоксах
/*if (isset($_GET['update_id'])) {
    //echo '<pre>', print_r($_POST, 1), '</pre>';
    echo "<script type='text/javascript'>
        var checkboxes = document.getElementsByName('checkx2');
    ...
    $serv_list = '';
    if (isset($_POST['check_stom'.$_GET['update_id']])) $serv_list .= SERVICE_STOM . ',';
    if (isset($_POST['check_kosm'.$_GET['update_id']])) $serv_list .= SERVICE_KOSM . ',';
    if (isset($_REQUEST['check_gine'.$_GET['update_id']])) $serv_list .= SERVICE_GINE . ',';
    if (isset($_REQUEST['check_plas'.$_GET['update_id']])) $serv_list .= SERVICE_PLAS . ',';
    if (isset($_REQUEST['check_trih'.$_GET['update_id']])) $serv_list .= SERVICE_TRIH . ',';
    if (strlen($serv_list) > 0)
        $serv_list = substr($serv_list, 0, -1);
    $updatestr = "UPDATE SOURCE_AUTO_DETAIL SET SERVICE_IDS = '{$serv_list}' WHERE ID = '{$_GET['update_id']}'";
    //var_dump($updatestr);
    if (DB_OCI) {
        $query = OCIParse(GetData::GetConnect(), $updatestr);
        //$query_result = OCIExecute($query);
    ...
    if ($query_result) {
        ...
        echo "<p style='color: red'>Произошла ошибка изменения записи.</p>";
    }
}*/
?>
        </div>
