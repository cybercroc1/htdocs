...
<?php if (strpos($_SERVER["HTTP_USER_AGENT"], "MSIE") !== false ||
    ...
    $theads = array(
        ...
        'BNUMBER' => array('name' => 'Б Номер', 'width' => '101'),
        'NAME' => array('name' => 'Наименование', 'width' => '401'),
        'SOURCE_TYPE' => array('name' => 'Устройство', 'width' => '121'),
        //'SERVICE_ID' => array('name' => 'Услуга', 'width' => '121'),
        'CITY_ID' => array('name' => 'Город', 'width' => '101'),
    ...
    if (GetData::GetSourceAuto(NULL, NULL, FALSE, $key, $sort) > 0) {
        // Получение списка столбцов
        /*foreach ($_POST['array_source_auto'] as $source => $row) {
            $field[$source]  = $row[$key];
            $name[$source]  = $row['NAME'];
        }
        if ('desc' == $sort)
            array_multisort($field, SORT_DESC, $name, SORT_ASC, $_POST['array_source_auto']);
        else array_multisort($field, SORT_ASC, $name, SORT_ASC, $_POST['array_source_auto']);*/
        ...
        foreach($_POST['array_source_auto'] as $key => $value) {
            ...
            //$str_serv = ($value['SERVICE_ID'] != SERVICE_ALL ? SERVICE_LIST[$value['SERVICE_ID']] : "Все услуги");
            ...
            echo '<tr><td style="text-align: center; width: 35px">' . $value['ID'] . '</td>
				<td style="text-align: center; width: 100px">' . $value['BNUMBER'] . '</td>
				<td style="text-align: center; width: 400px">' . $value['NAME'] . '</td>
				<td style="text-align: center; width: 120px">' . DEVICES[$value['SOURCE_TYPE']] . '</td>
				<td style="text-align: center; width: 100px">' . CITIES[$city_id] . '</td>
...
if (isset($BNumber)) {
    ...
    if ($bCanAdd == 0) {
        ...
        $max_id++;
        //Вставляем данные
        $insertstr = "INSERT INTO SOURCE_AUTO (ID, BNUMBER, NAME, SOURCE_TYPE) 
VALUES (seq_source_auto_id.nextval, '{$BNumber}', '{$Reservoir}', '{$Device}')";
        if (DB_OCI) {
			if (TRUE == ENCODE_UTF) {
                $tmpstr = iconv ('utf-8','windows-1251', $Reservoir);
                $tmpstrbn = iconv ('utf-8','windows-1251', $BNumber);
                $insertstr = "INSERT INTO SOURCE_AUTO (ID, BNUMBER, NAME, SOURCE_TYPE, SERVICE_ID) 
VALUES (seq_source_auto_id.nextval, '{$tmpstrbn}', '{$tmpstr}', '{$Device}')";
			}
...
<div>
    <form action="" method="post">
        <h3>
            BNumber: <input type="text" name="BNumber" placeholder="BNumber">
            &nbsp;Источник:&nbsp;<input type="text" name="Reservoir" style="width: 350px" placeholder="Источник рекламы">
            <br><label id="DeviceT" for="Device">Устройство:</label>
