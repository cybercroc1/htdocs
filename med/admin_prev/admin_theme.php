...
<h3 style="margin-bottom: 0;margin-top: 0;">Список Тем Входящих звонков</h3>
    ...
    if (GetData::GetThemes(NULL) > 0) {
        foreach($_POST['array_theme'] as $key => $value) {
            ...
            echo '<tr><td style="text-align: center">' . $value['ID'] . '</td>
                ...
                <td style="text-align: center">' . ($value['TARGET'] ? 'Да' : "Нет") . '</td>
				<td style="text-align: center">' . ($value['DELETED'] ? $value['DELETED'] : "нет") . '</td>';
            if ($value['DELETED']) {
...
if (isset($_POST['Name'])) {
    ...
    if ($bCanAdd == 0) {
        echo "<p style='font-size: larger; color: red'>Тема '".$_POST['Name']."' уже существует.<br /></p>";
    }
    else {
        //Вставляем данные
        if (DB_OCI) {
            ...
            $max_id++;
            $insertstr = "INSERT INTO CALL_THEME (ID, NAME, TARGET) VALUES ( {$max_id}, '".trim($_POST['Name'])."', '{$iTarget}' )";
