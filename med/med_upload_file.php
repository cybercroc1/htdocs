...
//отправка письма
if(isset($send_email)) {
	if(isset($send_record)) {
		$mess.=$mess_link_record;
	}
	$mess .= "</table>";
	...
	//настройки отправки почты пользователя, пока используется только поле email для ответов на письма
	$q=OCIparse($c,"select t.email,t.smtp_server,t.smtp_port,t.smtp_login,t.smtp_pass from USERS t where t.id='".$_SESSION['login_id_med']."' and t.email is not null");
	OCIExecute($q);
	if(OCIFetch($q)) {
		$reply_to_email=OCIResult($q,"EMAIL");
	...
	foreach($email_list as $to_email) {
		...
		if($to_email<>'') { 
            ...
            if (0 == strncmp($res[$enum]['text'], "OK", 2)) {
                ...
                if (in_array($_SESSION['login_id_med'],array(10,54,55))) // Соколовой дублируем на ее почту
                    $to_email_my = 'anna@wilstream.ru';
                /*elseif (1 == $_SESSION['login_id_med'])
                    $to_email_my = '2392967@gmail.com';*/

                if (in_array($_SESSION['login_id_med'],array(10,54,55))) {
                    $enum++;
...
<div id="drop-info1"><font color=red>Внимание! Отправляемое письмо будет содержать ссылку на записи разговоров!</font></div>

<b><input type='checkbox' checked name='send_record' onchange='if(this.checked==false){document.getElementById("drop-info1").style.display="none";}else{document.getElementById("drop-info1").style.display="";}' />
Отправлять ссылку на записи разговоров </b><hr> 

<div id="drop-info"><font color=red>Если необходимо прикрепиить к письму дополнительные файлы нажмите сюда:</font>
<input type=button value="Прикрепить файлы" onclick="if(document.getElementById('drop-area').style.display=='none')document.getElementById('drop-area').style.display=''; else document.getElementById('drop-area').style.display='none';"></input>
</div>
