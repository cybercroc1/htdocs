...
<form action="export_xlsx.php" method="POST">
    <h1 style='margin-bottom: 0; margin-top: 0'><label for='ReportId'>&nbsp;Наименование отчета:&nbsp;</label>
    <?php
    ...
    function ReportChanged() {
        ...
        if (<?=EXPORT_CALL?> == report_sel || <?=EXPORT_CALL_SEC?> == report_sel ||
        <?=EXPORT_CALL_SHORT?> == report_sel || <?=EXPORT_OPERATOR_SEC_CALL?> == report_sel) {
            document.getElementById('FirstPart').style.visibility = 'visible';
            document.getElementById('FirstPart').style.position = 'inherit';
            ...
            if (<?=EXPORT_CALL_SEC?> == report_sel || <?=EXPORT_OPERATOR_SEC_CALL?> == report_sel) {
                ...
                /*if (< ?=EXPORT_CALL_SHORT?> == report_sel)
                    document.getElementById('Export_but').style.visibility = 'hidden';*/
    ...
    <h2>Даты:&nbsp;
        ...
        echo "<div id='DateTypes' style='visibility: hidden;'><label for='DateType'>Фильтр:&nbsp;</label>";
        echo "<select id='DateType' name='DateType'>";
        echo "<option value='1'>По дате появления заявки</option>";
        echo "<option value='2'>По дате последнего изменения</option>";
        echo "<option value='3'>По дате заявки</option>";
        echo "</select></div>";
//..position: absolute; selected="selected"
        if (stristr($_SESSION['login_name'],'ДМТ'))
            echo "<script>$('#DateType').val('3').change();</script>";
        else echo "<script>$('#DateType').val('1').change();</script>";
        ...
        else echo "<div id='ServStat' style='float: left; width: 65%'><div>";
        ...
        //Детализация
        echo "<label for='sel_serv_det' id='sel_serv_detT' style='float: left;'>Уточнение:&nbsp;</label>";
...
and d.id=uda.dep_id and d.deleted is null and ad.departament_id=d.id)").
            ...
            if (SERVICE_ALL == $value['ID'])
                ...
                if ($nServices > 2) {
                    switch ($value['SERVICE_ID']) {
                        case SERVICE_STOM: $startstr = '(Стом.) '; break;
                        case SERVICE_KOSM: $startstr = '(Косм.) '; break;
                        case SERVICE_GINE: $startstr = '(Гинек.) '; break;
                        case SERVICE_PLAS: $startstr = '(Пласт.) '; break;
                        case SERVICE_TRIH: $startstr = '(Трих.) '; break;
                        default: $startstr = '';
                    }
        ...
        echo "<div id='mistakes' style='float: right; width: 34%; visibility: hidden; position: absolute'>";
        ...
        echo "<label for='Providers' style='float: left;'>Поставщик:&nbsp;</label>";
        //echo "<select id='Providers' name='Providers' onchange='ifr_all.location=\"admin_supplier_edit.php?getprov=\"+this.value'>";
        echo "<select multiple id='Providers' name='Providers[]' style='height: 295px;'>"; // overflow-y: hidden
        echo "<option value='".SOURCE_ALL."'>Все поставщики</option>";
...
Добавить дубли:&nbsp;<input type=checkbox id='add_repeat' name='add_repeat' title='и заявки повторных клиентов'></div>";
