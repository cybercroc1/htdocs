...
to_char(cb.ENTRY_DATE_1C,'dd.mm.yyyy hh24:mi') ENTRY_DATE_1C, cb.outer_order_id, ";
    if (EXPORT_CALL_SEC == $ReportId)
...
//echo "<br><textarea>".$q_text."</textarea><br>";
    ...
    while (OCIFetch($q)) {
        ...
        if (IT_PLANET != $_SESSION['login_id_med']) { // ограничение для IT Planet
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "SC_CALL_ID")));
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "SC_PROJECT_ID")));
            if (in_array(CAN_PHONE, $data_acc_arr)) {
                $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
                $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "ANUMBER")));
            }
        ...
        if (IT_PLANET != $_SESSION['login_id_med']) { // ограничение для IT Planet
            ...
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "RESULT")));
            if (!in_array($_SESSION['login_id_med'], EXPORT_CUT)) {
                $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
        ...
        $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
        $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "CLIENT_NAME")));
        if (in_array(CAN_PHONE, $data_acc_arr)) {
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "PHONE_MOB")));
        }
        $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
        ...
        $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
        $sheet[$snum]->setCellValue($coord, trim(u8(OCIResult($q, "COMMENTS"))));
        $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
...
LEFT JOIN HOSPITALS hosp ON HOSPITAL_ID = hosp.ID WHERE BASE_ID=:id");
        ...
        if (IT_PLANET != $_SESSION['login_id_med'] && !in_array($_SESSION['login_id_med'],EXPORT_CUT)) { // ограничение для IT Planet и Гусарова
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "FIO")));
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8($hospital));
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8($clinic_surname . ' ' . $clinic_name . ' ' . $clinic_patronymic));
            if (in_array(CAN_PHONE, $data_acc_arr)) {
                $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
                $sheet[$snum]->setCellValue($coord, u8($clinic_client_phone));
            }
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
        ...
        if (in_array($_SESSION['login_id_med'],OUTER_ORDER)) {
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
            $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "OUTER_ORDER_ID")));
        }
    }
...
elseif (EXPORT_OPERATOR == $ReportId || EXPORT_OPERATOR_ALL == $ReportId || EXPORT_OPERATOR_SEC == $ReportId) {
    ...
    if (EXPORT_OPERATOR_SEC == $ReportId) {
        ...
        or cb.ID in (select base_id from call_base_hist where status_id = ".STATUS_WORK." and 
