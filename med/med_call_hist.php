...
$phone_num_norm = phone_norm_single($phone_num, 'ru_dial');
$phone_num_segment = phone_segment($phone_num_norm);
...
function show_res($Result, $sid)
{
    ...
    if (GetData::GetCallHistory($base_id) > 0) {
        ...
        if (USER_VIEW != $_SESSION['user_role'] && (in_array($_SESSION['login_id_med'],SPEC_USER_VIEW) || $_SESSION['sec_chance'] || $_SESSION['user_role'] <= USER_SUPER)) {
...
if(isset($send_email)) {
    ...
    if (GetData::GetUsers(FALSE, FALSE,"usr.ID = ".$_SESSION['login_id_med'], "FIO") > 0) {
        ...
        /*$sqlstr = "SELECT cb.ID, cb.CLIENT_NAME, cb.PHONE_MOB, cb.BNUMBER, sa.NAME 
		FROM ".$table_name." cb
        ...
        WHERE (PHONE_MOB_NORM like '%".$phone_mob."%' or PHONE_MOB like '%".$phone_mob."%' or 
               ...
               ANUMBER like '%".$phone_mob."%' or cb.BNUMBER like '%".$phone_mob."%')
                AND cb.DATE_CALL <= sysdate ORDER BY ID";*/

		$sqlstr = "SELECT cb.ID, cb.CLIENT_NAME, cb.PHONE_MOB, cb.BNUMBER, sa.NAME, cb.source_auto_id, cb.status_id, cb.CALL_DOUBLE 
		FROM ".$table_name." cb
		LEFT JOIN SOURCE_AUTO sa ON sa.id = cb.source_auto_id
		WHERE (PHONE_MOB_NORM like '%".$phone_num_norm."%' or 
			PHONE_NEW_NORM like '%".$phone_num_norm."%' or 
			ANUMBER_NORM like '%".$phone_num_norm."%')
			AND cb.DATE_CALL <= sysdate ORDER BY ID";				
...
<div style='display: inline-block; text-align: center; width: 100%;'>
    <h1 style='margin-top: -5px; margin-bottom: 1px;'>История заявок с номером: '<span style='color: red;'>&nbsp;В поиске введен не верный номер</span>'.</h1>
</div>
<?php
exit();
}
?>
<div style='display: inline-block; text-align: center; width: 100%;'>
    <h1 style='margin-top: -5px; margin-bottom: 1px;'>История заявок с номером: '<span style='color: #3aa311;'>&nbsp;<?=$phone_num_segment?></span>'.</h1>
</div>
