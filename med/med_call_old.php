...
<head>
    ...
    if (isset($sc_call_id) && GetData::GetCallByCallId($sc_call_id) > 0) {
        $existing_call = 1;
...
if (isset($max_call) && $status < STATUS_CLOSED) {
...
if (isset($sc_call_id) && isset($sc_project_id) && isset($bnumber)) {
    $nrowsAuto = GetData::GetSourceAuto(NULL, $bnumber,FALSE,'Name','asc',FALSE,FALSE);
    if (isset(GetData::$array_source_auto)) {
        $source_auto_id = GetData::$array_source_auto[0];
        if (TRUE == ENCODE_UTF)
            $source_auto_name = u8(GetData::$array_source_auto[2]);
...
<!--    <h2>Цель звонка: <input type="textarea" cols="10" rows="5" name="age" style="width: 400px; height: 50px;"/></h2> -->
    <h2><label for="comment">Комментарий:</label>
        <textarea name="comment" id="comment" title="Комментарий" placeholder="Введите комментарий" rows=30 cols=68 style="vertical-align: text-top; height: 45px"><?=$comment?></textarea>
        <br/>
        <label for="surname">ФИО:&nbsp;</label><input type="text" name="surname" style="width: 90%" value="<?=$fio?>" placeholder="Фамилия Имя Отчество"/>
		...
		<input type="text" name="phone_mob" title="Контактный телефон" style="width: 10em;" value="<?=$phone_mob_norm?>"/>
