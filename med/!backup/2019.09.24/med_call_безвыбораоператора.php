...
<head>
    ...
    if (isset($getoper)) { ?>
        ...
        if (SERVICE_NOT == $getoper) {
            ...
            if (GetData::GetUsersDep(FALSE, NULL, $arr_filt, 'mix') > 0) {
                ...
                $sel = "<select id=\"OperatorsId\" name=\"OperatorsId\" onchange=\"OperatorSelected();\">";
                ...
                foreach(GetData::$array_userd as $key => $value) {
                    ...
                    //$sel .= "<option value=\"" . $value['ID'] . "\">" . $value['FIO'] . "</option>";
    ...
    function ResultSelected()
    {
        ...
        } else if ( <?=RESULT_WAIT?> == document.getElementById("ResultId").value ) { // Ждет звонка
            elem = document.getElementById('save_but');
            document.getElementById('ResultId').style.backgroundColor = 'white';
            document.getElementById('KC_Number').style.visibility = 'hidden';
            document.getElementById('write_cl').style.visibility = 'hidden';
...
if ("POST" == $_SERVER["REQUEST_METHOD"] && isset($save_but)) { // Принимаем данные из формы
    $purpose = (isset($_POST['PurposeId']) ? $_POST['PurposeId'] : NULL); // тут Id Темы звонка
    //$voice = CALL_FIRST; //(isset($_POST['voice']) ? $_POST['voice'] : CALL_FIRST); // считаем, что первичный
    if (THEME_MED == $purpose) {
        $service = (isset($_POST['ServiceId']) ? $_POST['ServiceId'] : NULL); // тут Id Услуги
        $reservoir = (isset($_POST['Reservoir']) ? $_POST['Reservoir'] : NULL); // тут Id Источника
        $source_man_det = $Result_det = "NULL";

        $comment = (isset($_POST['comment']) ? htmlspecialchars($_POST['comment'], ENT_QUOTES) : "");
        $surname = (isset($_POST['surname']) ? stripcslashes(htmlspecialchars($_POST['surname'],ENT_QUOTES)) : "---");
        ...
        $OperatorsId = (isset($_POST['OperatorsId']) ? $_POST['OperatorsId'] : "");
        if ("" != $OperatorsId)
            $OperatorsId = substr($OperatorsId, 0, strpos($OperatorsId, ':'));
        $clinic = (isset($_POST['call_clinic']) ? $_POST['call_clinic'] : "");
    } else {
    ...
    /*if (SERVICE_STOM == $service) { // Детализация для стоматологии
        if (isset($_POST['ServiceStom'])) {
            $service_det = $_POST['ServiceStom'];
        }
    }*/
    ...
    if (THEME_MED == $purpose) {
        $updatestr = "UPDATE ".$table_name." SET CALL_THEME_ID = {$purpose}, SOURCE_MAN_ID = {$reservoir}, SERVICE_ID = {$service}, 
    COMMENTS = '{$comment}', CLIENT_NAME = '{$fio}', PHONE_MOB = '{$phone_mob_seg}', PHONE_MOB_NORM = '{$phone_mob_norm}', CALL_BACK_NUM = 10, 
    RESULT_ID = {$ResultId}, LAST_CHANGE = to_date('{$_POST['date_call']}','DD.MM.YYYY hh24:mi:ss')";
        ...
        $updatestr = "UPDATE ".$table_name." SET CALL_THEME_ID = {$purpose}, STATUS_ID = " . STATUS_CLOSED .",
...
// для медицинских заявок добавляем первую строку истории по этому звонку c именем оператора
    if ($query_result && THEME_MED == $purpose) {
...
if (TRUE == DEBUG_MODE) echo "<textarea>".$insertstr."</textarea><br/>";
        ...
        if (RESULT_KC == $ResultId && $query_result && THEME_MED == $purpose &&
...
if (TRUE == DEBUG_MODE) echo "<textarea>".$insertstr."</textarea><br/>";
    ...
    if (GetData::GetThemes("ID=".$purpose) > 0) {
        $theme_text = GetData::$array_theme[0]['NAME'];
    }
    else $theme_text = $purpose;
    ...
    if (THEME_MED == $purpose && TRUE == SEND_EMAIL && ($query_result || DEBUG_MODE)) {
...
if (!isset($source_auto_id) || 0 == $source_auto_id || NULL == $source_auto_id) {
    ...
    if (FALSE == DEBUG_MODE) {
        ...
        CALL_THEME_ID, SOURCE_AUTO_ID, SOURCE_MAN_ID, SOURCE_TYPE_ID, CALL_TYPE_ID, SERVICE_ID, STATUS_ID, LAST_CHANGE, OKTELL_IDCHAIN)
        ...
        to_date('{$date_call}','DD.MM.YYYY hh24:mi:ss'), '{$oktell_idchain}') returning ID into :max_call";
    } else {
