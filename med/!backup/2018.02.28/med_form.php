...
else */
{
    ...
    if (DB_OCI) {
        ...
        $updatestr = "UPDATE CALL_BASE SET SOURCE_MAN_ID = {$reservoir}, CALL_THEME_ID = {$purpose}, CALL_TYPE_ID = {$voice}, 
            SERVICE_ID = {$service}, SOURCE_MAN_DET_ID = {$source_man_det}, COMMENTS = '{$comment}', 
            CLIENT_NAME = '{$fio}', AGE =  {$ages}, PHONE_MOB = '{$phone_mob}', PHONE_HOME = '{$phone_home}', EMAIL ='{$email}', 
            RESULT_ID = {$ResultId}, RESULT_DET = '{$Result_det}', TRANSFER_NUM = '{$_POST['trans_num']}',
            LAST_CHANGE = to_date('{$_POST['date_call']}','DD.MM.YYYY hh24:mi:ss')
            WHERE ID = " . $_POST['max_call'];
...
VALUES (SEQ_CALL_BASE_HIST_ID.nextval, $max_call, '{$_POST['sc_agid']}', $status, to_date('{$_POST['date_call']}','DD.MM.YYYY hh24:mi:ss'), '{$comment}')";
