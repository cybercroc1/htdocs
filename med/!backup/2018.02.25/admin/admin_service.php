...
<table border='2' style="display: inline-block;">
    ...
    if (DB_OCI) {
        $selectstr = "SELECT ID, Name, to_char(Deleted,'dd.mm.yyyy hh24:mi:ss') Deleted FROM SERVICES WHERE ID != -1";
        ...
        $selectstr = "SELECT ID, Name, DATE_FORMAT(Deleted,'%d.%m.%Y %H:%i:%s') AS Deleted FROM SERVICES WHERE ID != -1";
...
if (isset($_POST['Name'])) {
    ...
    if (DB_OCI) {
        ...
        $checkstr = "SELECT ID FROM SERVICES WHERE NAME LIKE '{$_POST['Name']}' limit 1";
    ...
    if ($count === 1) {
        echo "<p style='color: red'>Услуга '".$_POST['Name']."' уже существует.<br /></p>";
    }
    else {
        $max_id++;

        //Вставляем данные
        $insertstr = "INSERT INTO SERVICES (ID, NAME) VALUES ( {$max_id}, '{$_POST['Name']}' )";
        if (DB_OCI) {
			if (TRUE == ENCODE_UTF) {
				$tmpstr = iconv ('utf-8','windows-1251', $_POST['Name']);
                $insertstr = "INSERT INTO SERVICES (ID, NAME) VALUES ( {$max_id}, '{$tmpstr}' )";
			}
