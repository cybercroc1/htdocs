...
<img src='$img'><a href=\"?$get\" style='color: black'>Дата заявки<br/></a>
    ...
    if (!in_array($_SESSION['login_id_med'],SPEC_USER_CALL)) {
        if ((USER_SUPER == $_SESSION['user_role'] || $_SESSION['on_duty_today']) && !isset($show_closed)) {
            if (!isset($show_work))
                $q_where .= " and cb.STATUS_ID != " . STATUS_WORK;
            if (!isset($show_delayed))
                $q_where .= " and cb.CALL_BACK_DATE is NULL";
            if (!isset($show_nedo)) {
                $q_where .= " and cb.CALL_BACK_DATE <= sysdate and cb.DATE_CLOSE is null";
                if (!$_SESSION['on_duty_today'])
                    $q_where .= " or cb.STATUS_ID=" . STATUS_OPEN;
            }
