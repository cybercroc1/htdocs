...
function findLeads($phone,$email,$company_name){
    $leads = [];
    $check_phone = substr(preg_replace("/[^0-9]/", "", $phone ), -10); // Последние 10 цифр номера телефона
        ...
        for($i = 0; $i < count($res_c); $i++){  
           ...
           $com_title = false;
            ...
            if($semantic_id == 'P'){
                ...
                if(strrpos($pure_phone, $check_phone) !== false){
...
function genTaskComment($user_params){
    ...
    Источник: ".$user_params['source']."<br>
...
function createNewLead($user_params){ // Создаем новый лид
    $comments = '';
    if(!isset($user_params['company']) && empty($user_params['company'])) $user_params['company'] = '';
    if(!isset($user_params['source']) && empty($user_params['source'])) $user_params['source'] = '';
    if(!isset($user_params['email']) && empty($user_params['email'])) $user_params['email'] = '';
...
function getRespId($id,$el){ // Получаем ID ответственного 
...
function getCompanyId($company_name){ // Получаем ID компании по названию
    $data = http_build_query(array(
        'filter' => array(
            'TITLE' => $company_name
        )
           ...
           if(count($res_c)){
            for($i = 0; $i < count($res_c); $i++){
                $contact_id = $res_c[$i]->ID;
                    ...
                    $pure_phone = substr(preg_replace("/[^0-9]/", "", $phone ), -10);
                    if(strrpos($phone_number, $pure_phone) !== false){
                     return $contact_id;  
                    }
                    }
                }
