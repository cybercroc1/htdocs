...
$log_file_name = __DIR__.'/logs/'.time().'_'.rand(0,9999).'.data'; // Файл логов для заявки

$req_dump = print_r($_REQUEST, TRUE);
$fp = fopen($log_file_name, 'a');
fwrite($fp, 'Request_'.$req_dump);
fclose($fp);
...
if(isset($cdn) && !empty($cdn)){
      switch($cdn){
        ...
        break;
		case '4993700289':
		$source = '7'; //Звонок Wilstream (карты)
		break;
		case '4996495953':
		$source = '8'; //Звонок МА Lagency (Wilstream)
		break;		
		case '4959671946':
		$source = '9'; //ДМТ Ценные обращения
		break;
    }
} 
...
if($contacts == false && $leads == false && $companies == false){
	...
	//Если нет контактов, лидов, компаний
	//сразу создаем пустой лид
	/*$new_lead_id=addEmptyLead($aon,$source);
	if($new_lead_id && is_int($new_lead_id)){
		echo "Лид создан. ID лида $new_lead_id";
	}
	else */
	$new_lead_id='';
	
	showFormLink(); //и выводим ссылку на форму, где этот лид будет модифицироваться
    exit;
}
else{
...
function addEmptyLead($aon,$source) {
	$data = http_build_query(array( // Формируем запрос для создания нового лида
		'FIELDS' => array(
		    'TITLE' => 'Входящий звонок',
			'NAME'=> 'Входящий звонок',
			'PHONE' => array(array('VALUE' => $aon,'VALUE_TYPE' => 'WORK')), 
			'SOURCE_ID'=> $source
		)
	));	
	$json = b24request('crm.lead.add', $data);
	$lead_id = $json->result;
	return $lead_id;
}
/*
...
function showFormLink() {
	...
	<a href='' onclick='window.open(\"http://bt24.wilstream.ru/callcenter/form.php?source=".$source."&aon=".$aon."&lead_id=".$new_lead_id."\",\"bitwin\",\"location=no,width=350,height=600,top=100,left=100,toolbar=no,menubar=no,status=yes,resizable=yes,scrollbars=yes\");return false;'>
