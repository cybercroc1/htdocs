...
ini_set("error_log", "C:\Apache24\htdocs\bt24/parse.data");
extract($_POST);
...
if ($new) {
    $test_file_name = __DIR__ . '/test/' . time() . '_' . rand(0, 9999) . '.data'; // Файл логов для заявки

    $req_dump = print_r(['get' => $_GET, 'post' => $_POST, 'req' => $_REQUEST], TRUE);
    $fp = fopen($test_file_name, 'a');
    fwrite($fp, "\nRequest_" . $req_dump);
    fclose($fp);
}
/*==================================================================== */
...
if (isset($formname)) {
	...
	if (isset($stats_id)) $user_params['stats_id'] = $stats_id; //ДМТ - ID заявки
	if (isset($stats_descruptor)) $user_params['stats_descruptor'] = $stats_descruptor; //ДМТ - что интересовало
