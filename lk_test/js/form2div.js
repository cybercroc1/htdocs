function form2div(frm_id,div_id,clicked_button,act) {
	...
	for (var i = 0, element; element = frm.elements[i++];) {
		...
		if(element.type=='button' || element.type=='submit') {
			if(element==clicked_button) {
				is_form_button='y';
				if(method=='post') data.append(element.name,element.value); 
			...
			for(var j = 0, opt; opt = element.options[j++];) {
				if(opt.selected==true) {
					if(method=='post') data.append(element.name,opt.value);
	...
	if(is_form_button!='y') {
		if(method=='post') data.append(clicked_button.name,clicked_button.value);
