...
$q=OCIParse($c,"select id,name,type from sup_lt_group where id<>0 and type='common' order by name");
...
$q=OCIParse($c,"select id,name,type from sup_lt_group where id<>0 and type='oper_only' order by name");
OCIExecute($q,OCI_DEFAULT);
while(OCIFetch($q)) {
	echo "<option value=".OCIResult($q,"ID");
	if(OCIResult($q,"ID")==$lt_grp_id) {echo " selected"; $lt_grp_name=OCIResult($q,"NAME"); $lt_grp_type=OCIResult($q,"TYPE");}
...
//
if($lt_grp_type=='oper_only') {echo "<br><font size=3>Эта группа используется только для <b>приема заявок (операторы, менеджеры)</b></font>";}
echo "<hr>";

if($lt_grp_id<>'') {
//пользователи
echo "пользователи: ";
$q=OCIParse($c,"select t.login,t.fio,t.coment,t.send,t.look,t.solution,t.redirect,t.eval,t.admin,decode(t.lt_grp_id,'0','y',null) all_grp, t.oper 
from SUP_USER t
...
$q1=OCIParse($c,"select id,name from SUP_LOCATION_GROUP
order by name");
$q2=OCIParse($c,"select id,name from SUP_KLINIKA
where location_grp_id=:location_grp_id and deleted is null
order by name");
OCIExecute($q1,OCI_DEFAULT);
$i=0; while(OCIFetch($q1)) {
	$i++; if($i>1) echo "<hr>";
	echo "Группа мест: <input type=checkbox name=location_grp_ids[".OCIResult($q1,'ID')."]'";
	if(isset($location_grp_ids[OCIResult($q1,'ID')])) echo " checked";
	echo "><b>".OCIResult($q1,'NAME')."</b><br>";
	$location_grp_id=OCIResult($q1,'ID');
	OCIBindByName($q2,":location_grp_id",$location_grp_id);
	OCIExecute($q2,OCI_DEFAULT);
	while(OCIFetch($q2)) {
		echo "<input type=checkbox location_id name=location_ids[".OCIResult($q2,'ID')."] value='".OCIResult($q2,'ID')."' onclick='uncheck_location(this)'";
		if(isset($location_ids[OCIResult($q2,'ID')])) echo " checked";
