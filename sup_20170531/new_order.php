...
if(isset($sid)) {
	if(isset($_SESSION['create_new']) and $_SESSION['create_new']=='y') {
		...
		echo "<font color=red><b>ОШИБКА: У Вас нет прав для создания заявки или Вы не авторизовались в системе. Авторизуйтесь в главном окне программы.</b></font>";
		exit();
	}
}
//
//не авторизованный пользователь (оператор)
else {
	if(
		substr($_SERVER['REMOTE_ADDR'],0,8)<>'192.168.'
	) {
		echo "<font color=red><b>ОШИБКА: Не авторизованный доступ возможен только из локальной сети</b></font>";
		exit();
	}
	...
	if(isset($lt_oper_grp) and $lt_oper_grp<>'') {
		...
		echo "<font color=red><b>ОШИБКА: Не указана группа</b></font>";
		exit();
	}
}
//
...
if (isset($send)) {
	...
	if(!isset($trbl_ids) or count($trbl_ids)=='0') {echo "<font color=red><b>ОШИБКА: Не выбран тип проблемы</b></font><br>"; $err='';}
	...
	if(!isset($err)) {
		...
		$i=1;	while(OCIFetch($q)) {
			...
			$i++;
			if ($err_count==0) echo "<font color=green>Заявка №<b>$num_zayavki</b> ОТПРАВЛЕНА!</font><hr>";
		}
		$num_zayavki=''; $location_id=''; $trbl_ids=array(); $kto=''; $u_kogo=''; $oper_coment='';
		if ($err_count==0) {echo "<script>alert('ОТПРАВЛЕНО!');
...
if(count($location_ids)==1) {
	...
	echo "<font size=4>".$location_name.". <font color=green>Новая заявка</font></font><br>";
...
echo "<hr>";	
echo "<b><font color=red size=4>*</font>ФИО, номер места/добавчного<br>(у кого не работает):</b><br> <input type=text size=30 name=u_kogo style='width:97%' value='".$u_kogo."' onkeyup='check()'></input><br>";
echo "</td>";

echo "<td bgcolor=white valign=top><b><font color=red size=4>*</font>Тип проблемы: </b><br>";
...
echo "<b><font color=red size=4>*</font>Описание проблемы:</b><br>
...
echo "<b><font color=red size=4>*</font></b> - обязательное поле";
echo "</td></tr></table>";
echo "</form>";
...
$mess.="<td bgcolor=white valign=top>№ заявки: <b>".$num_zayavki."</b><hr>";
if($cdpn<>'') $mess.="АОН: <b>".$cdpn."</b><br>";
$mess.="IP: <b>".$_SERVER['REMOTE_ADDR']."</b><hr>";
$mess.="Кто обратился:<br><b>".$kto."</b><br>";
if($kto_id<>'') {
	...
	if(isset($mailtos)) {
		...
		$mess.="<a href='mailto:".$mailtos."?subject=Заявка №".$num_zayavki." - ответ'>".$mailtos."</a><br>";
	}
	
}
$mess.="<hr>
У кого не работает:<br><b>".$u_kogo."</b><br></td>
...
*/
	...
	//Шлём заявку всем, кто пересекаеися по группе проблем
	...
	//Шлём только тем, кто может заниматься этой конкретной проблемой в конкретном месте, кроме себя ))
	$q=OCIParse($c,"select distinct ste.email from sup_trbl_type stt, sup_lt slt, sup_user su, sup_texnari_emails ste
	...
	while (OCIFetch($q)) {
		...
		//Только для Ильичева не удалять!
		...
		$mess2.="<b>У кого не работает:</b>".$u_kogo."<br>";
		$mess2.="<b>Описание проблемы:</b><br>
		".nl2br($oper_coment);
