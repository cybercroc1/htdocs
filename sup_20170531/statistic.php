...
from sup_base b, sup_trbl_alloc ta, sup_trbl_type tt
where b.date_in_call between to_date('".$start_date_day."','DD.MM.YYYY') and to_date('".$end_date_day."','DD.MM.YYYY')+1
and b.klinika_id in (".implode(",",$obj_ids).")
and b.texnari_id in (".implode(",",$user_ids).") 
and tt.id=:trbl_id and ta.base_id=b.id and tt.id=ta.trbl_type_id and b.trbl_grp_id=nvl(:trbl_grp_id,b.trbl_grp_id)");
...
round(avg(nvl(b.in_work,nvl(b.ready_to_close,b.date_close))-b.date_in_call)*24,2) avg_react  
from sup_base b, sup_trbl_det_alloc da, sup_trbl_detail dd
where b.date_in_call between to_date('".$start_date_day."','DD.MM.YYYY') and to_date('".$end_date_day."','DD.MM.YYYY')+1
and b.klinika_id in (".implode(",",$obj_ids).")
and b.texnari_id in (".implode(",",$user_ids).") 
and da.base_id = b.id
and da.trbl_detail_id=dd.id
...
while (OCIFetch($q1)) {
	...
	OCIBindByName($q2,":trbl_id",OCIResult($q1,"TRBL_ID"));
	OCIBindByName($q2,":trbl_grp_id",OCIResult($q1,"GROUP_ID"));
	OCIExecute($q2);
	OCIFetch($q2);
	...
	OCIBindByName($q3,":trbl_id",OCIResult($q1,"TRBL_ID"));
	OCIExecute($q3);
	while(OCIFetch($q3)) {
		echo '<Row>
    		<Cell ss:StyleID="s181"><Data ss:Type="String">'.OCIResult($q3,"NAME").'</Data></Cell>';	
		OCIBindByName($q4,":trbl_id",OCIResult($q1,"TRBL_ID"));	
		OCIBindByName($q4,":trbl_det_id",OCIResult($q3,"ID"));
		OCIExecute($q4);
		OCIFetch($q4);
...
from
(select distinct b.id,b.date_in_call,b.in_work,b.ready_to_close,b.date_close,b.dublikat,b.krivie_ruki from sup_base b, sup_trbl_alloc ta
where b.date_in_call between to_date('".$start_date_day."','DD.MM.YYYY') and to_date('".$end_date_day."','DD.MM.YYYY')+1
...
while (OCIFetch($q1)) {
	...
	OCIBindByName($q2,":loc_id",OCIResult($q1,"LOC_ID"));
	OCIExecute($q2);
	OCIFetch($q2);
...
$q1=OCIParse($c,"select decode(slg.id,0,'общая группа',slg.name) group_name, slg.id group_id,su.fio fio, su.id user_id
from sup_user su, sup_lt_group slg
where slg.id=su.lt_grp_id
and su.id in (".implode(",",$user_ids).")
...
while (OCIFetch($q1)) {
	if(OCIResult($q1,"GROUP_ID")<>$group_id) {
		echo '<Row>
    <Cell ss:MergeAcross="4" ss:StyleID="s43"><Data ss:Type="String">'.OCIResult($q1,"GROUP_NAME").'</Data></Cell>
   </Row>';
		//echo "<tr><th colspan=3 align=left>".OCIResult($q1,"GROUP_NAME")."</th></tr>";
	}
	$group_id=OCIResult($q1,"GROUP_ID");
	//echo "<tr>";
	//echo "<td>".OCIResult($q1,"FIO")."</td>";
	echo '<Row>
    <Cell ss:StyleID="s17"><Data ss:Type="String">'.OCIResult($q1,"FIO").'</Data></Cell>';
	OCIBindByName($q2,":user_id",OCIResult($q1,"USER_ID"));
	OCIExecute($q2);
	OCIFetch($q2);
...
echo "<table width=100%><tr><td align=left><font size=4>Общая статистика</font> | <font size=3><a href='stat_fingers.php'>Отпечатки пальцев</a></font></td>";
echo "<td align=right>"; 
