...
if($_SESSION['user_role']==1 or isset($_SESSION['access']['data_acc']['CAN_FINANCE'])) {
    ...
    $c++;
    $columns[$c]['full_name'] = "Ожидаемая выручка" . chr(10) . " с обращения за период";
    $columns[$c]['width'] = 12;
...
if($_SESSION['user_role']==1 or isset($_SESSION['access']['data_acc']['CAN_FINANCE'])) {
    ...
    $c++;
    $columns[$c]['full_name'] = "% закрытия планов" . chr(10) . "за период";
    $columns[$c]['width'] = 12;
    $columns[$c]['name'] = "PRC_CLOSE_PLAN_BY_PER";
    $columns[$c]['format'] = "#,##0.00";

    $c++;
    $columns[$c]['full_name'] = "Выручка с обращения" . chr(10) . "за период";
    $columns[$c]['width'] = 12;
...
$sheet[$snum]->setTitle(u8("За период"));
	...
	foreach($columns as $colnum => $null) {
		...
		//кастомные поля, которых нет в запросе
			...
			if (isset($result_arr[$res_key]['COST_VISIT']) && isset($result_arr[$res_key]['VISIT_BY_PER'])) {
                ...
                if (isset($group_itog['PAY_VISIT_PER'])) $group_itog['PAY_VISIT_PER'] += $val;
                else  $group_itog['PAY_VISIT_PER'] = 0;
                if (isset($common_itog['PAY_VISIT_PER'])) $common_itog['PAY_VISIT_PER'] += $val;
                else  $common_itog['PAY_VISIT_PER'] = 0;
            }
			else $val='';
			$sheet[$snum]->setCellValue($coord,u8(str_replace(",",".",$val)));
		}
			...
			if (isset($result_arr[$res_key]['COST_VISIT']) && isset($result_arr[$res_key]['VISIT_BY_PER'])) // и другое
                $val -= $result_arr[$res_key]['COST_VISIT']*$result_arr[$res_key]['VISIT_BY_PER'];
            if (isset($group_itog['DOHOD_PER'])) $group_itog['DOHOD_PER'] += $val;
            else $group_itog['DOHOD_PER'] = 0;
            if (isset($common_itog['DOHOD_PER'])) $common_itog['DOHOD_PER'] += $val;
            else  $common_itog['DOHOD_PER'] = 0;
			$sheet[$snum]->setCellValue($coord,u8(str_replace(",",".",$val)));
		}
	}
	//ГРУППОВЫЕ ИТОГИ
	if(substr(next($result_sources),0,4)<>$tmp_grp_name) {
		...
		foreach($columns as $colnum => $null) {
            ...
            /*else if($columns[$colnum]['name']=='PAY_ORDER') {
                ...
                $val=$group_itog['PAY_VISIT_PER'];
                /*if (isset($group_itog['COST_VISIT']) && isset($group_itog['VISIT_BY_PER']))
                    $val=$group_itog['COST_VISIT']*$group_itog['VISIT_BY_PER'];
                else $val='';*
                $sheet[$snum]->setCellValue($coord,u8(str_replace(",",".",$val)));
            }
            else if($columns[$colnum]['name']=='DOHOD_PER') {
                if (isset($group_itog['PAYMENT_SUM_BY_PER']))
                    $val = $group_itog['PAYMENT_SUM_BY_PER'];
                else $val = 0;

                if (isset($group_itog['COST_ORDER']) && isset($group_itog['ORDER_COUNT'])) // вычитаем одно
                    $val -= $group_itog['COST_ORDER']*$group_itog['ORDER_COUNT'];
                if (isset($group_itog['COST_VISIT']) && isset($group_itog['VISIT_BY_PER'])) // и другое
                    $val -= $group_itog['COST_VISIT']*$group_itog['VISIT_BY_PER'];
                $sheet[$snum]->setCellValue($coord,u8(str_replace(",",".",$val)));
            }*/
		}
...
//ИТОГИ
		...
		foreach($columns as $colnum => $null) {
            ...
            /*else if($columns[$colnum]['name']=='PAY_ORDER') {
                ...
                $val=$common_itog['PAY_VISIT_PER'];
                /*if (isset($common_itog['COST_VISIT']) && isset($common_itog['VISIT_BY_PER']))
                    $val=$common_itog['COST_VISIT']*$common_itog['VISIT_BY_PER'];
                else $val='';*
                $sheet[$snum]->setCellValue($coord,u8(str_replace(",",".",$val)));
            }
            else if($columns[$colnum]['name']=='DOHOD_PER') {
                if (isset($common_itog['PAYMENT_SUM_BY_PER']))
                    $val = $common_itog['PAYMENT_SUM_BY_PER'];
                else $val = 0;

                if (isset($common_itog['COST_ORDER']) && isset($common_itog['ORDER_COUNT'])) // вычитаем одно
                    $val -= $common_itog['COST_ORDER']*$common_itog['ORDER_COUNT'];
                if (isset($common_itog['COST_VISIT']) && isset($common_itog['VISIT_BY_PER'])) // и другое
                    $val -= $common_itog['COST_VISIT']*$common_itog['VISIT_BY_PER'];
                $sheet[$snum]->setCellValue($coord,u8(str_replace(",",".",$val)));
            }*/
		}
