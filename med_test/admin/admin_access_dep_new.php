...
if (!isset($_SESSION['user_role']) || USER_ADMIN != $_SESSION['user_role'] && !in_array($_SESSION['login_id_med'],COST_EDIT)) {
...
if (isset($getdet))
{
    ...
    if (OCIExecute($query)) {
        ...
        while ($result = OCI_Fetch_Array($query)) {
            ...
            //if (strstr($result['DEPNAME'],'\'')) $result['DEPNAME'] = str_replace('\'','\"',$result['DEPNAME']);
            //if (strstr($result['SRANAME'],'\'')) $result['SRANAME'] = str_replace('\'','\"',$result['SRANAME']);
            if (strstr($result['DEPNAME'],'"')) $result['DEPNAME'] = str_replace('"','\'',$result['DEPNAME']);
            if (strstr($result['SRANAME'],'"')) $result['SRANAME'] = str_replace('"','\'',$result['SRANAME']);
            if (strstr($result['SRTNAME'],'"')) $result['SRTNAME'] = str_replace('"','\'',$result['SRTNAME']);
            if (strstr($result['SRMNAME'],'"')) $result['SRMNAME'] = str_replace('"','\'',$result['SRMNAME']);
            if (strstr($result['SRVNAME'],'"')) $result['SRVNAME'] = str_replace('"','\'',$result['SRVNAME']);
...
<form action="" method="post" style="margin-bottom: 0">
    ...
    echo "<select id='Depart' name='Depart' onchange='ifr_all.location=\"admin_access_dep_new.php?getdet=\"+this.value'>";
...
//    echo '<a href="'.PATH.'/admin/admin_access_dep_new.php"><button name="UndoAll" class="enter_button" style="width: 190px; height: 2.1em;">Отменить все изменения</button></a>';
    ...
    if (GetData::GetSourceAuto("DELETED IS NULL", NULL, FALSE) > 0) {
        echo "<select id='S_Auto' name='S_Auto[]' multiple style='height: 350px; max-width: 100%'>";
...
<?php
function OnClick($Adding, $Refreshing)
{
    if (isset($_POST['S_Auto']) && isset($_POST['S_Man']) && isset($_POST['S_Type']) && isset($_POST['ServName']))
    {
        //echo "<script>alert('OnClick');</script>";
        $query_delete = TRUE;
        $query_result = FALSE;
        if (FALSE == DEBUG_MODE)
            $table_name = 'ACCESS_DEP';
        else $table_name = 'ACCESS_DEP_TEST';
        if ($Refreshing) { // удаляем все для выбранного департамента
        ...
        if ($query_delete) {
            ...
            foreach ($_POST['S_Auto'] as $key_sAuto => $sAuto) {
            foreach ($_POST['S_Man'] as $key_sMan => $sMan) {
            foreach ($_POST['ServName'] as $key_serv => $sServ) {
                if ($Adding) { // проверяем наличие аналогичной записи
                    $checkstr = "SELECT count(*) FROM ".$table_name." WHERE departament_id = ".$_POST['Depart']." AND source_auto_id = ".$sAuto." AND source_type_id = ".$_POST['S_Type']." AND source_man_id = ".$sMan." AND service_id = ".$sServ;
                    /*if ($sServ != -1)  $checkstr .= " or source_auto_id = {$sAuto} AND source_type_id = {$S_Type} AND source_man_id = {$sMan} AND service_id = -1";
                      if ($sMan != -1)   $checkstr .= " or source_auto_id = {$sAuto} AND source_type_id = {$S_Type} AND source_man_id = -1 AND service_id = {$sServ}";
                      if ($S_Type != -1) $checkstr .= " or source_auto_id = {$sAuto} AND source_type_id = -1 AND source_man_id = {$sMan} AND service_id = {$sServ}";
                      if ($sAuto != -1)  $checkstr .= " or source_auto_id = -1 AND source_type_id = {$S_Type} AND source_man_id = {$sMan} AND service_id = {$sServ}";*/
//echo "<script>alert('".$checkstr."');</script>";
                    $result = NULL;
                    if (DB_OCI) {
                ...
                if (NULL == $result || $result[0] == 0) { // Вставляем данные
                    if (FALSE == DEBUG_MODE)
...
if (isset($Adding) || isset($Refreshing))
    OnClick(isset($Adding), isset($Refreshing));
?>
</body>
</html>