...
// Обработка действий
if (isset($Name)) { // Если переменная Name передана
    $count = 0;
    $checkstr = "SELECT ID FROM HOSPITALS WHERE NAME LIKE '{$Name}' AND SERVICE_ID = {$Services}";
    if (DB_OCI) {
    ...
    if ($count === 1) {
        echo "<p style='color: red'>Клиника с таким профилем '".$Name."' уже существует.<br /></p>";
    }
...
//$insertstr = "INSERT INTO HOSPITALS (ID, NAME, SERVICE_ID, ADDRESS, PHONE) VALUES ( {$max_id}, {$Services}, '{$tmpstr}', '{$adrstr}', '{$phonestr}' )";
        }
        $insertstr = "INSERT INTO HOSPITALS (ID, NAME, SERVICE_ID, ADDRESS, PHONE, CITY, MANAGER_ID, TRADEMARK) 
                  VALUES ( {$max_id}, '{$Name}', {$Services}, 
                  '{$Address}', '{$Phone}', '".CITIES[$CityId]."}', {$UserId}, '{$TrademarkId}' )";
        if (DB_OCI) {
