...
if (DB_OCI) {
    ...
    OCIFetch($q);
    echo "<p style='display: inline; color: #007fff'> / Последнее изменение:&nbsp;<span style='color: darkblue;'>". OCIResult($q, "LAST_CHANGE") ."</span></p>";
...
//echo "<br/><textarea>".$_SESSION['refresh_where']."</textarea>";
    ...
    while(OCIFetch($q)) {
        ...
        if (isset($show_trans)) { // результат входящего звонка
            $res_id = OCIResult($q, "RESULT_ID");
            $res_det_id = OCIResult($q, "RESULT_DET");
            if (!isset($res_arr[$res_id]))
                $res_arr[$res_id] = CALL_RES[$res_id];
        }
    ...
    if (isset($show_text)) {
        ...
        while(OCIFetch($q_comment)) {
            echo "<hr style='margin: 0'>".OCIResult($q_comment,"DATE_DET")."; ФИО:".OCIResult($q_comment,"FIO")."; Статус: ".OCIResult($q_comment,"NAME").";<br/>".OCIResult($q_comment,"COMMENTS")."; ";
        }
        //файлы
        ...
        if (isset($show_trans)) {// переводные
            echo "<td bgcolor=white style='text-align: center;";
            if (RESULT_WAIT == $res_id)
                echo " color:".$status_color.";'>".CALL_RES[$res_id]."</td>";
            elseif (RESULT_CLINIC == $res_id) {
                ...
                echo " color:" . $status_color . ";'>" . CALL_RES[$res_id] . "<br/>(".$clinic_name.")</td>";
            }
            else
                echo " color:".$status_color.";'>" . CALL_RES[$res_id] . "<br/>(".$res_det_id.")</td>";
        }
        echo "</tr>";
    }
    echo "</table>";
