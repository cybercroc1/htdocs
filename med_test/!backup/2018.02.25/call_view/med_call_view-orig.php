...
<script type="application/javascript">
    ...
    function open_edit(base_id,texnari_id,sid) {
        //win=window.open("call.edit.form.php?base_id="+base_id+"&texnari_id="+texnari_id+"&sid="+sid,"sup_order_"+base_id,"width=550, height=700, toolbar=no, scrollbars=yes, resizable=yes, status=yes");
        win=window.open("../med_call_out.php?base_id="+base_id+"&texnari_id="+texnari_id+"&sid="+sid,"sup_order_"+base_id,"width=720, height=640, toolbar=no, scrollbars=yes, resizable=yes, status=yes");
        win.focus();
    }
...
echo "<h3 style='margin-bottom: 0;'>Просмотр входящих звонков</h3>";
...
if (DB_OCI) {
    ...
    OCIFetch($q);
    echo "Последнее изменение:&nbsp;<input disabled name='last_change' value='".OCIResult($q,"LAST_CHANGE")."'></nobr>";
...
if (USER_ADMIN != $_SESSION['user_role']) {
    ...
    echo "<th style='background-color: white; vertical-align: top;; width: 120px'>agid<br>";
...
//        or upper(replace(sr_det.NAME,' ')) like '%'||upper(replace('".$find_id."',' '))||'%'
        ...
        else { //  а бывает ли такое?
            if (!isset($show_closed) && !isset($show_delayed) && !isset($show_nedo)) {
                $q_where .= " and cb.STATUS_ID < " . STATUS_CALL_STOP; // только те, что в работе
                if (USER_SUPER == $_SESSION['user_role']) // только новые и перезвон сегодня
                    $q_where .= " and (to_char(cb.CALL_BACK_DATE,'dd.mm.yyyy') <= to_date('$date','DD.MM.YYYY') OR cb.STATUS_ID = " . $mystatus . ")";
                elseif (USER_USER == $_SESSION['user_role']) // только назначенные и перезвон сегодня
                    $q_where .= " and (to_char(cb.CALL_BACK_DATE,'dd.mm.yyyy') <= to_date('$date','DD.MM.YYYY') OR cb.STATUS_ID = " . $mystatus . ")";
            } elseif (isset($show_closed)) {
                $q_where .= " and cb.STATUS_ID <= " . STATUS_CLOSED;
            } elseif (isset($show_nedo)) {
                $q_where .= " and cb.STATUS_ID = " . STATUS_CALL_NOT . " and cb.LAST_CHANGE+15/1440 <= sysdate";
            } elseif (isset($show_delayed)) {
            }
        }
    ...
    if (OCIExecute($query)) {
        ...
        // приближающиеся для перезвона
        //$q_where.=" ) or cb.CALL_BACK_DATE between to_date('$start_date','DD.MM.YYYY') and to_date('$end_date','DD.MM.YYYY')+1 ";
    }
    ...
    $q_text1 = "SELECT cb.ID, to_char(cb.DATE_CALL,'dd.mm.yyyy hh24:mi:ss') DATE_CALL, cb.ANUMBER, cb.BNUMBER, cb.SC_AGID, cb.SC_CALL_ID, cb.SC_PROJECT_ID, 
                    ...
                    nvl(cb.FIO_ID,0)+nvl(cb.STATUS_ID,0)+nvl(cb.SOURCE_MAN_DET_ID,0)) as checksum
                    ";
    ...
    $q_text3 = " LEFT JOIN CALL_TYPE ctt ON cb.CALL_TYPE_ID = ctt.ID
        ...
        ";
    $q_text4 = $q_where;
    //$q_text5 = " ORDER BY cb.LAST_CHANGE DESC, cb.DATE_CALL, serv.NAME, sr_a.NAME, sr_man.NAME, CALL_TYPE_ID ";
...
//echo "<br/><textarea>".$_SESSION['refresh_where']."</textarea>";
    ...
    if (isset($show_text)) {
        ...
        //$q_comment = OCIParse($c,"select stat.NAME, stat.OPERATOR || usr.FIO as FIO, COMMENTS from CALL_BASE_HIST hist where ID=:base_id" );
        $sqlstr = "SELECT to_char(DATE_DET,'dd.mm.yyyy hh24:mi:ss') as DATE_DET, stat.NAME, OPERATOR || usr.FIO as FIO, COMMENTS FROM CALL_BASE_HIST hist
                    LEFT JOIN USERS usr ON usr.ID = hist.USER_ID
                    LEFT JOIN MED_STATUS stat ON hist.STATUS_ID = stat.ID
                    WHERE hist.ID=:base_id";
    ...
    while(OCIFetch($q)) {
        ...
        if(!isset($tehnari_arr[OCIResult($q,"FIO_ID")]))
            $tehnari_arr[OCIResult($q,"FIO_ID")] = OCIResult($q,"FIO");
    ...
    if (isset($show_text)) {
        ...
        echo "AGID: <b>".OCIResult($q,"SC_AGID")."</b>";
        echo "; PROJECT_ID: <b>".OCIResult($q,"SC_PROJECT_ID")."</b>";
        echo "; CALL_ID: <b>".OCIResult($q,"SC_CALL_ID")."</b><br>";

        OCIBindByName($q_comment,":base_id",$tmp_base_id);
        OCIExecute($q_comment);
        echo "Комментарии:";
        while(OCIFetch($q_comment)) {
            echo "<hr style='margin: 0'>".OCIResult($q_comment,"DATE_DET")."; ФИО:".OCIResult($q_comment,"FIO").";<br>Статус: ".OCIResult($q_comment,"NAME")."; ".OCIResult($q_comment,"COMMENTS")."; ";
        }
        //файлы
        ...
        //echo " color:".$status_color.";'>(".OCIResult($q,"SRDET_ID").")".$detail_name."</td>";
...
if (USER_ADMIN != $_SESSION['user_role']) {
    ...
    asort($tehnari_arr);
    foreach($tehnari_arr as $key => $val) {
