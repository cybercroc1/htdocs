...
abstract class GetData
{
    ...
    {
        $sqlstr = "SELECT MAX(to_number(substr(TRANSFER_NUM, instr(TRANSFER_NUM,'-',1,4)+1, length(TRANSFER_NUM)-".TRANS_NUM."))) as tr_num 
            FROM CALL_BASE WHERE TRANSFER_NUM like '" . $trans_date ."%'";
//echo $sqlstr;
        $tr_num = 1;
        if (DB_OCI) {
            $query = OCIParse(self::GetConnect(), $sqlstr);
