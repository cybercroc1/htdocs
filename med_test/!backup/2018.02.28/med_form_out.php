...
// Принимаем новые данные из формы
    ...
    $user = (isset($_POST['UserId']) ? $_POST['UserId'] : $_SESSION['login_id']); // кто выбран или кто меняет статус
    ...
    if (DB_OCI) {
        ...
        //if (STATUS_WORK == $StatusId) { // кому назначено
        if (STATUS_OPEN != $StatusId) { // для всех изменений статуса?
        ...
        if (STATUS_CALL_STOP == $StatusId || STATUS_CLINIC == $StatusId ||
            STATUS_NEGATIVE == $StatusId || STATUS_ERROR == $StatusId) { // Закрываем звонок
...
//echo "<textarea>".$insertstr."</textarea>";
        ...
        $insertstr = "INSERT INTO CALL_BASE_HIST ( ID, BASE_ID, USER_ID, STATUS_ID, DATE_DET, COMMENTS ) 
                      VALUES (SEQ_CALL_BASE_HIST_ID.nextval, {$_POST['Base_Id']}, '{$_SESSION['login_id']}', 
                      {$StatusId}, to_date('{$date_call}','DD.MM.YYYY hh24:mi:ss'), '{$full_comment}')";
//echo "<textarea>".$insertstr."</textarea>";
        $query = OCIParse(GetData::GetConnect(), $insertstr);
        $query_result = OCIExecute($query);
