...
<head>
    ...
    if (GetData::GetCallByCallId((isset($sc_call_id) ? $sc_call_id : ''),(isset($oktell_idchain) ? $oktell_idchain : '')) > 0) {
        ...
        //var_dump('Exist - '.$max_call. ' Статус - '.$status);
    } else {
        //var_dump('New one');
        $existing_call = 0;
    }
    ...
    if (isset($getoper)) {
        ...
        if (isset($user_id) && $user_id > 0 /*&& 0 == $existing_call*/) {
            // если окно у оператора исхода, то проверяем его департаменты
...
(select dep_id from USER_DEP_ALLOC where user_id = " . $user_id . ")";
        ...
        if ( <?= $user_id ?> > 0 )
        {
            ...
            if (selectobject) {
                ...
                if ( 1 == <?= $need_self ?> ) {
                    ...
                    //var textnode = parent.document.createTextNode("Назначить себе"); // Create a text node
                    //node.appendChild(textnode);                         // Append the text to <li>
                    //parent.document.getElementById("ResultId").appendChild(node); // Append <option> to <select> with id="ResultId"
                    var option = new Option("Назначить себе", <?=RESULT_KC_SELF?>, true, true);
            ...
            if (<?=RESULT_KC?> == selectobject.value /*&& 0 == < ?= $existing_call ?>*/ ) { // если уже выбран перевод в КЦ
                ...
                //parent.document.getElementById('call_center').value = ''; // TODO проверить после изменений с sc_call_id
            }
            else {
        ...
        if (SERVICE_NOT == $getoper) {
            ...
            if (true || !$existing_call) { // попытки загрузить экран с данными не нужны, ибо все в call_out
                ...
                echo "<script>var ins_sel = parent.document.getElementById('AllInOneIst'); if (ins_sel) ins_sel.innerHTML='Источник рекламы:&nbsp;';</script>";
    ...
    function ResultSelected()
    {
        ...
        document.getElementById('KC_Number').style.visibility = 'hidden';
        document.getElementById('write_cl').style.visibility = 'hidden';
        if ( <?=RESULT_KC?> == res_id || <?=RESULT_KC_SELF?> == res_id ) { // Перевод в КЦ (и на себя)
...
if ("POST" == $_SERVER["REQUEST_METHOD"] && isset($save_but)) { // Принимаем данные из формы
    ...
    if (RESULT_KC == $ResultId || RESULT_KC_SELF == $ResultId) { // Перевели в КЦ
    ...
    if (THEME_MED == $PurposeId) {
        if (RESULT_KC_SELF == $ResultId) $ResultId = RESULT_KC; // минимизация изменений, ибо тот же перевод только на себя
    ...
    RESULT_ID = {$ResultId}, LAST_CHANGE = to_date('{$date_call}','DD.MM.YYYY hh24:mi:ss')";
		...
		if (isset($check_hist) && OCIFetch($check_hist) && OCIResult($check_hist,"CNT_HIST")>2) {
			$updatestr = "UPDATE ".$table_name." SET LAST_CHANGE = sysdate"; // что-то надо обновить, чтоб потом смогли найти
		}
		else {
...
if (TRUE == DEBUG_MODE) echo "<textarea>".$insertstr."</textarea><br/>";
    ...
    if (RESULT_KC == $ResultId || RESULT_KC_SELF == $ResultId) $mess_subj .= ' (Переводной)';
    ...
    if ($query_result) {
        ...
        if (RESULT_KC == $ResultId || RESULT_KC_SELF == $ResultId) {
            ...
            if (isset($max_call) && $max_call > 0 && isset($OperatorsId) && $OperatorsId > 0) {
                ...
                echo "<button type='button' name='close_but' id='close_but' class='send_button' onclick='open_call(".$max_call.",".$OperatorsId.",\"".session_id()."\");'>Закрыть</button>";
            }
            else echo "<button type='button' name='close_but' id='close_but' class='send_button' onclick='window.close();'>Закрыть</button>";
...
echo '<div style="display: inline-block;">
    ...
    <h1 class="heading" style="margin-top: -5px; margin-bottom: 1px;">Входящий звонок 
    '.(isset($max_call) ? ' (заявка '.$max_call.')' : "").'</h1>
</div>';
if (isset($max_call) && isset($status) && $status < STATUS_CLOSED) {
    GetData::GetMedStatus("ID = ".$status, TRUE, TRUE);
    echo '<h2 style="color: '.GetData::$array_status[0]['COLOR'].'">Заявка уже в работе! Статус: "' . GetData::$array_status[0]['NAME'] . '".</h2>';
    if (isset($max_call) && $max_call > 0 && isset($OperatorsId) && $OperatorsId > 0) {
        $_SESSION['auth'] = md5($_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT']);
        echo "<button type='button' name='close_but' id='close_but' class='send_button' onclick='open_call(".$max_call.",".$OperatorsId.",\"".session_id()."\");'>Закрыть</button>";
    }
...
if (isset($sc_call_id) && isset($sc_project_id) && isset($bnumber)) {
    ...
    if (0 == $nrowsAuto) // если звонят по уже удаленному источнику, чей номер не переназначили другому
...
if (!isset($source_auto_id) || 0 == $source_auto_id) {
    ...
    if (!isset($bnumber) || NULL != $bnumber)
        echo "Сообщите админу ошибочное значение BNumber: <span style='color: mediumvioletred; text-decoration: underline'>'".$bnumber."'</span>";
    else echo "Сообщите админу, что <span style='color: mediumvioletred; border-bottom: dashed'>BNumber пустой!</span>";
    echo "</h2>";
} else { // Сразу вставляем строку в CALL_BASE с начальными данными, если такого call_id еще нет.
    $query_result = FALSE;
    if ($existing_call) { // получаем данные имеющейся заявки
...
if (TRUE == DEBUG_MODE) echo "<br/><textarea>" . $insertstr . "</textarea><br/>";
    ...
    if (isset($oktell_uid) && preg_match("/^(\{)?[a-f\d]{8}(-[a-f\d]{4}){4}[a-f\d]{8}(?(1)\})$/i", $oktell_uid)) {
        ...
        if ($row = $q->fetch()) {
            ...
            if (GetData::GetUsers(FALSE, FALSE, "usr.login = '" . $row['login'] . "'", "FIO") > 0) {
                ...
                $Result_det = GetData::$array_user[0]['PIN']; // Для сохранения при назначении самому себе
    ...
    else echo '<br/>Некорректный GUID.';
}
?>
...
<div>
    ...
    <h2 style="display: inline;">
		...
		//кнопка перезвона по АОН
        if (isset($aon_for_backcall) and $aon_for_backcall<>'' && isset($max_call)) {
            echo " | <input disabled id='callto_button' type='button' onclick='callto(".$aon_for_backcall.",".$max_call.")' value='     Перезвонить по АОН' title='".$aon_for_backcall."' 
...
style='height: 25px; background-image: url(\"".PATH."/images/call_stop.png\"); background-repeat: no-repeat; display:none;' />";
        ...
        echo '<br/><label id="ServiceT" for="ServiceId" style="visibility: hidden">Услуги:&nbsp;</label>';
    ...
    <h2>
        ...
        if (true || !$existing_call) { // попытки загрузить экран с данными не нужны, ибо все в call_out
        ...
        else { // без варианта изменения у имеющейся заявки ?
            echo "<label style='position: relative; float: left;' for='Reservoir'>Источник рекламы:&nbsp;</label>";
            ...
            echo "<option value=\"" . SOURCE_NOT . "\">Выберите источник рекламы</option>";
            if (GetData::GetIstochnik(FALSE, FALSE, "(instr(in_dep, '" . $getoper . "') != 0)", FALSE) > 0) {
                foreach (GetData::$array_istochnik as $key => $value) {
                    if (TRUE == ENCODE_UTF) {
                        $value['NAME'] = u8($value['NAME']);
...
//var_dump($reservoir); var_dump($source_man_det);
            if (!isset($reservoir) || 0 == $reservoir || !isset($source_man_det)) {
                ...
                $nrows = GetData::GetSourceDetail(FALSE, NULL, $reservoir);
                if ($nrows > 0) { // у остальных списка нет
                    if (SOURCE_FLAER == $reservoir || SOURCE_CATALOG == $reservoir ||
                        SOURCE_FLAER_SUB == $reservoir || SOURCE_FLAER_CAR == $reservoir ||
                        SOURCE_LIFT == $reservoir || SOURCE_STOP == $reservoir) {
                        $getdetailstr = "SELECT ID, NAME FROM SUBWAYS";
                    ...
                    echo "<option value=''>Выберите детализацию</option>";
                    $q = OCIParse($c, $getdetailstr);
                    if (OCIExecute($q)) {
                        while (OCIFetch($q)) {
                    ...
                    if (SOURCE_SERT == $reservoir) {
                        echo "<option value=" . DETAILS_PROMO . ">На улице у промоутера</option>";
                        echo "<option value=" . DETAILS_OTHER . ">Другое</option>";
                        $i += 2;
                    } elseif (SOURCE_NOT != $reservoir && SOURCE_COUPON != $reservoir) {
                        echo "<option value=" . DETAILS_AMNESY . ">Не помнит</option>";
                        $i++;
                    }
                    echo "</select>";
                    if ($i == 0) {
                        echo '-----';
                    } //'Пустой список детализации!';
                }
            }
...
<!--    <h2>Цель звонка: <input type="textarea" cols="10" rows="5" name="age" style="width: 400px; height: 50px;"/></h2> -->
    <h2><label for="comment">Комментарий:</label>
        <textarea name="comment" id="comment" title="Комментарий" placeholder="Введите комментарий" rows=30 cols=68 style="vertical-align: text-top; height: 45px"><?= isset($comment)?$comment:'' ?></textarea>
        <br/>
        <label for="surname">ФИО:&nbsp;</label><input type="text" name="surname" style="width: 90%" value="<?= isset($fio)?$fio:'' ?>" placeholder="Фамилия Имя Отчество"/>
		...
		<input type="text" name="phone_mob" title="Контактный телефон" style="width: 10em;" value="<?= isset($phone_mob_norm)?$phone_mob_norm:'' ?>"/>
    ...
    <h2 style="display: inline-block; margin-top: 0"> Результат:
        <select id="ResultId" name="ResultId" onchange="ResultSelected();" title="Результат" style="background-color:<?=needs?>">
            ...
            <!--?php if ($user_id > 0) echo '<option value="'.RESULT_KC_SELF.'">Назначить себе</option>'; ?-->
        </select>
...
<!--          &nbsp;Номер заявки:&nbsp;<span style="color: black; font-size: smaller;">< ?=$const_str?>-</span>
        ...
        <span id="write_cl" style="visibility: hidden">&nbsp; Клиника:&nbsp;
            <select id='call_clinic' name='call_clinic' title="Клиника">
            <?php
