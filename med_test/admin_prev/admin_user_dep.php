...
<div class="container">
    ...
    if (DB_OCI) {
        $selectstr = "SELECT uda.ID, dep.NAME as depName, usr.ID as usrID, usr.FIO as usrName, role.NAME as ROLE, to_char(uda.DELETED,'dd.mm.yyyy hh24:mi:ss') Deleted, to_char(usr.ACTIVITY,'dd.mm.yyyy hh24:mi:ss') ACTIVITY
...
//              ORDER BY usr.ROLE_ID, usr.FIO, dep.NAME ";
        ...
        if ($query_result) {
            while ($result_array = OCI_Fetch_Array($query)) {
                ...
                echo '<tr><td style="text-align: center; width: 35px">' . $result_array['ID'] . '</td>
                    <td style="text-align: center; width: 40px">' . $result_array['USRID'] . '</td>
    ...
    {
        // Обработка действий
        // Если Департамент и Пользователь выбраны... хотя, всегда выбран первый, если данные там уже есть
