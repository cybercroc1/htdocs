...
if (EXPORT_CALL == $_POST['ReportId'] || EXPORT_CALL_SEC == $_POST['ReportId']) {
...
to_char(cb.ENTRY_DATE_1C,'dd.mm.yyyy hh24:mi') ENTRY_DATE_1C,";
    if (EXPORT_CALL_SEC == $_POST['ReportId'])
...
//echo "<br><textarea>".$q_text."</textarea><br>";
    ...
    if (EXPORT_CALL_SEC == $_POST['ReportId']) { // убираем столбцы с записью разговора
        array_pop($head_arr);
        array_pop($head_arr);
    }
...
LEFT JOIN HOSPITALS hosp ON HOSPITAL_ID = hosp.ID WHERE BASE_ID=:id");
        ...
        if (EXPORT_CALL_SEC != $_POST['ReportId'] && $data_acc_arr && in_array(CAN_HEAR, $data_acc_arr)) { // ограничение для прослушивания записей
