...
//echo "<br><textarea>".$q_text."</textarea><br>";
    ...
    $head_arr = array(0=>"№ заявки", 1=>"Дата звонка", 2=>"ID звонка",
        3=>"Проект", 4=>"ANumber", 5=>"Оператор", 6=>"Тема", 7=>"Услуга",
        8=>"Тип источника", 9=>"BNumber", 10=>"Источник (Авто)",
        11=>"Источник (вход.)", 12=>"Детализация источника (вход.)",
        13=>"Источник (исх.)", 14=>"Результат входящего", 15=>"Детализация результата",
        16=>"ФИО", 17=>"Контактный телефон", 18=>"Статус", 19=>"Уточнение ошибки",
        20=>"Комментарий вход", 21=>"Комментарий последний",
        22=>"Назначено", 23=>"Клиника", 24=>"Записан", 25=>"Контакт", 26=>"Дата записи", 27=>"Итог",
        28=>"Время посл. события", 29=>"Закрыто", 30=>"ID Цепочки", 31=>"Запись звонка");
    ...
    if ($rep_start_date != $rep_end_date)
        $out_string[]= out_encode($bMac,'Экспорт заявок с '.$rep_start_date." по ".$rep_end_date);
    else $out_string[]= out_encode($bMac,'Экспорт заявок за '.$rep_start_date);
...
//<a href="'.$oktell_records_url.'?idchain='.OCIResult($q,"OKTELL_IDCHAIN").'">'.out_encode($bMac,"Ссылка").'</a></td>';
                //Запись звонка
