...
if (!isset($end_date) || '' == trim($end_date)) {
    $end_date=date('d.m.Y',mktime(0,0,0,date("m"),date("d"),date("Y")));
}
$_SESSION['end_date'] = $end_date;
if (!isset($start_date) || '' == trim($start_date)) {
    $start_date=$end_date; //date('d.m.Y',mktime(0,0,0,date("m"),date("d")-7,date("Y")));
}
...
echo "<h3 style='margin-top: 0;margin-bottom: 0;'>Просмотр заявок";
    ...
    OCIFetch($q);
    if (USER_ADMIN == $_SESSION['user_role'])
        echo "<span style='display: inline; color: #007fff'> / Изменение:&nbsp;<span style='color: darkblue;'>". OCIResult($q, "LAST_CHANGE") ."</span></span><br/>";
    else echo "<span style='display: inline; color: #007fff'> / Последнее изменение:&nbsp;<span style='color: darkblue;'>". OCIResult($q, "LAST_CHANGE") ."</span></span><br/>";
...
if (isset($DutyToday) && isset($UserId) && '' != $UserId) {
    GetData::SetUserDuty($UserId, $_SESSION['login_id_med']);
}
if (isset($MoveDep) && isset($MyDepId) && '' != $MyDepId) {
    GetData::SetUsersDep($MyDepId, $_SESSION['login_id_med']);
    if (SEC_CHANCE_CALL != $MyDepId)
        $_SESSION['sec_chance'] = FALSE;
    else $_SESSION['sec_chance'] = TRUE;
}
$UsersDeps = GetData::GetUserDeps($_SESSION['login_id_med'],FALSE,NULL);
...
if (USER_ADMIN == $_SESSION['user_role'] || USER_SUPER == $_SESSION['user_role']) {
    ...
    echo "</select>";
    echo "<input type='submit' name='DutyToday' value='Назначить' class='add_button' style='margin-top: 0'/>";
    echo "</form>";
    echo "</div>";
} elseif ($UsersDeps > 1 && USER_USER == $_SESSION['user_role']) {
    echo "<div style='display: inherit; width: 30%'>";
    echo "<form action='' method='post' target='' style='margin-bottom: 0'>";
    echo "<label for='MyDepId' style='color: crimson; font-weight: bold'>Департамент:&nbsp;</label>";
    echo "<select id='MyDepId' name='MyDepId'>";
//$strWhere = " ID in (select dep_id from user_dep_alloc where user_id = 61 and deleted is null)";
//if (GetData::GetDepartments(FALSE,FALSE,$strWhere) > 0) {
    foreach (GetData::$array_userdeps as $key => $value) {
        if (0 == $value['PRIORITY']) $selected = 'selected'; else $selected = '';
        echo "<option value='" . $value['ID'] . "'".$selected.">" . $value['NAME'] . "</option>";
    }
    echo "</select>"; // style="margin-top: 0;background: blue;color: white;"
    echo "<input type='submit' name='MoveDep' value='Изменить департамент' class='add_button' style='margin-top: 0'/>";
    echo "</form>";
    echo "</div>";
}
...
//Хедер-футер. КОНТЕНТ
    ...
    $theads = array(
        ...
        'sup.SUP_NAME' => array('name' => 'Поставщик', 'width' => '100', 'vari' => 'suppl_id'),
    ...
    if (isset($_GET['key'])) {
        ...
        /*if (USER_ADMIN == $_SESSION['user_role'] || USER_VIEW == $_SESSION['user_role']) {
            ...
            if ($_SESSION['sec_chance']) {
                //$key = " cb.STATUS_ID asc, cb.DATE_CALL ";
                //$key = " cb.DATE_CALL desc, cb.STATUS_ID ";
				$key = " cb.DATE_SECOND_CHANCE ";
                $sort = 'desc';
            }
            else {
    ...
    foreach ($theads as $k => $thead) {
        if ("usr.FIO" == $k && $_SESSION['sec_chance'])
            continue;
        elseif ("usr.SECOND_FIO" == $k && !$_SESSION['sec_chance'])
            continue;
        elseif ("sup.SUP_NAME" == $k && USER_USER == $_SESSION['user_role'])
            continue;
        ...
        $get .= '&start_date="' . $_SESSION['start_date'].'"'; //$start_date;
        $get .= '&end_date="' . $_SESSION['end_date'].'"'; //$end_date;
...
<img src='$img'><a href=\"?$get\" style='color: black'>Дата заявки<br/></a>
    c <input type='text' name='start_date' id='start_date' autocomplete='off' size=6 onchange='ok.click()' value='" . $_SESSION['start_date'] . "' />
    по <input type='text' name='end_date' id='end_date' autocomplete='off' size=6 onchange='ok.click()' value='" . $_SESSION['end_date'] . "' />";
    ...
    $q_where = " cb.DATE_CALL <= sysdate and cb.CALL_THEME_ID=" . THEME_MED; // только Медицинские услуги ...
    ...
    if (!isset($show_closed) /*&& !isset($show_delayed)*/) { // Перезвон может быть и у закрытых заявок
        if ($_SESSION['sec_chance'])
            $q_where .= " and (cb.DATE_SECOND_CLOSE is null or cb.CALL_BACK_DATE is not NULL)";
        else {
            if (USER_USER != $_SESSION['user_role'])
                $q_where .= " and cb.DATE_CLOSE is null";
            else $q_where .= " and (cb.DATE_CLOSE is null or cb.CALL_BACK_DATE is not NULL)";
        }
    }
    $q_where .= " and cb.service_id != 0 ";

    if (USER_ADMIN != $_SESSION['user_role'] && !$_SESSION['sec_chance']) { // права доступа по департаментам
    ...
    if ($suppl_id <> "") {
        $q_where_top .= ($btop ? " and sup.ID='" : " sup.ID='") . $suppl_id . "'"; $btop = TRUE;
    }
...
//echo "<br/><textarea>".$q_where_top."</textarea>";
    ...
    } elseif ($btop && (USER_ADMIN == $_SESSION['user_role'] || $_SESSION['sec_chance']))
        echo "<script>document.getElementById('blink1').style.visibility = 'visible';</script>";
    else echo "<script>document.getElementById('blink1').style.visibility = 'hidden';</script>";

    if ($_SESSION['sec_chance']) { // операторы второго дозвона
        ...
        $q_where .= " and (cb.CALL_BACK_DATE <= sysdate and (cb.SECOND_STATUS_ID <= ".STATUS_CALL_NOT." or cb.SECOND_STATUS_ID = ".STATUS_CLINIC."))";
        ...
        $q_where .= " or cb.CALL_BACK_DATE <= sysdate and (cb.STATUS_ID <= ".STATUS_CALL_NOT." or cb.STATUS_ID = ".STATUS_CLINIC.") and cb.SECOND_STATUS_ID is NULL"; // оставшиеся открытые, недошедшие до второго шанса
    ...
    if (isset($show_closed)) { // показываем все звонки
        ...
        if (isset($show_delayed) && !isset($show_nedo)) { // перезвон и открытые
            if ($_SESSION['sec_chance'])
                $q_where .= " or (CALL_BACK_DATE is not null and DATE_SECOND_CLOSE is null and cb.SECOND_FIO_ID=".$_SESSION['login_id_med'];
            ...
            if ($_SESSION['sec_chance'])
                $q_where .= " or (CALL_BACK_DATE is null and DATE_SECOND_CLOSE is null and cb.SECOND_STATUS_ID=".STATUS_CALL_NOT." and cb.SECOND_FIO_ID = ".$_SESSION['login_id_med'];
            ...
            if ($_SESSION['sec_chance'])
                $q_where .= " or (cb.SECOND_STATUS_ID in (".STATUS_CALL_NOT.",".STATUS_CALL_BACK.") and cb.SECOND_FIO_ID=".$_SESSION['login_id_med'];
                //$q_where .= " or (cb.SECOND_STATUS_ID in (".STATUS_CALL_NOT.",".STATUS_CALL_BACK.")";
            else {
				$q_where .= " or (cb.STATUS_ID in (".STATUS_CALL_NOT.",".STATUS_CALL_BACK.")";
    ...
    if ($btop && (USER_ADMIN == $_SESSION['user_role'] || $_SESSION['sec_chance']))
        $q_where .= " and " . $q_where_top;
...
//        or upper(replace(sr_det.NAME,' ')) like '%'||upper(replace('".$find_id."',' '))||'%'
    ...
    if (USER_USER != $_SESSION['user_role'] || $_SESSION['on_duty_today']) {
        $q_text1 .= ", sr_a.SUPPLIER_ID, sup.SUP_NAME";
    }
    if ($find_id != "")
    ...
    if (USER_USER != $_SESSION['user_role'] || $_SESSION['on_duty_today'])
        $q_text3 .= " LEFT JOIN SUPPLIERS sup ON sup.ID = sr_a.SUPPLIER_ID";
    if ($find_id != "")
...
//        or cb.ID in (select base_id from " . $lock_table . " where lock_date_end is not null and user_id = {$_SESSION['login_id_med']}))";
		$q_where .= " and (cb.ID not in (select base_id from " . $lock_table . " where user_id != ".$_SESSION['login_id_med'].") 
        or cb.ID in (select base_id from " . $lock_table . " where lock_date_end is null and user_id = ".$_SESSION['login_id_med']."))"; // ? null vs not null ?
...
/*if (TRUE == DEBUG_MODE || USER_ADMIN == $_SESSION['user_role'] || 241 == $_SESSION['login_id_med'] || 8 == $_SESSION['login_id_med'])
...
//exit;*/
 ...
 WHERE hist.BASE_ID=:base_id ORDER BY DATE_DET";
    ...
    while (OCIFetch($q)) {
        ...
        if ($_SESSION['sec_chance'] && NULL == $second_fio_id) {
            ...
            $q_lock = "select ID from " . $lock_table . " where lock_date_end is NULL and base_id = " . $tmp_base_id; // проверяем работает ли уже кто-то с заявкой
        ...
        if (USER_USER != $_SESSION['user_role'] || $_SESSION['on_duty_today']) {
            if (!isset($suppl_arr[OCIResult($q, "SUPPLIER_ID")]))
                $suppl_arr[OCIResult($q, "SUPPLIER_ID")] = OCIResult($q, "SUP_NAME");
        }
        ...
        elseif (STATUS_CALL_STOP <= $status_id && $cbd != '') // закрытые с пометкой Перезвонить
            $status_color = "violet";
        ...
        if (USER_USER != $_SESSION['user_role'] || $_SESSION['on_duty_today']) {
            echo "<td " . $onclick . " bgcolor=white style='text-align: center; color:" . $status_color . ";'>" . OCIResult($q, "SUP_NAME") . "</td>";
        }
    ...
    asort($suppl_arr);
    foreach ($suppl_arr as $key => $val) {
        if ($key == $suppl_id) $selected = 'selected'; else $selected = '';
        echo "add_options(document.all.suppl_id,'" . $key . "','" . $val . "','" . $selected . "');";
    }
    asort($status_arr);
    ...
    //if ($find_id == '' || $row_num == 0)
    {
        ...
        echo "if (document.all.suppl_id) document.all.suppl_id.disabled=false;";
...
<?php if (!isset($find_id) || $find_id == "") { ?>
    <iframe name=check_new src="med_check_new.php" style="display:"></iframe>
