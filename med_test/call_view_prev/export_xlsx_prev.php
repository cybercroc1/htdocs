...
elseif (EXPORT_OPERATOR == $_POST['ReportId'] || EXPORT_OPERATOR_ALL == $_POST['ReportId']) {
                   ...
                   $itog_visit_interval, $itog_visit, $itog_pay_interval, $itog_pay, $sum_pay_interval, $sum_pay)
{
    ...
    if (1 == $num_list)
        $head_arr = array('0'=>"Источник рекламы (Авто)", '1'=>"Тип ".chr(10)."источника",
            ...
            '8'=>"% записанных, ".chr(10)."от принятых".chr(10)." обращений");
    ...
    if (in_array(CAN_FINANCE, $data_acc_arr)) {
        $sheet[$snum]->getStyle('C4:H'.$rnum)->getNumberFormat()->setFormatCode('#,##0');
        $sheet[$snum]->getStyle('I4:I'.$rnum)->getNumberFormat()->setFormatCode('#,##0.00');
    }
...
usr.FIO as FIO, cb.CALL_DOUBLE, cb.OKTELL_IDCHAIN, 
...
//echo "<br><textarea>".$q_text."</textarea><br>";
    ...
    $head_arr = array('0'=>"№ ".chr(10)."заявки", '1'=>"Дата ".chr(10)."звонка", '2'=>"ID звонка",
        ...
        '22'=>"Назначено", '23'=>"Клиника", '24'=>"Записан", '25'=>"Контакт",
        '26'=>"Время".chr(10)." события", '27'=>"Закрыто",'28'=>"ID Цепочки",'29'=>"Запись".chr(10)." звонка");
    $remove_column = array(0,2,3,4,5,9,11,12,13,14,15,22,23,24,25,28,29);
    $gus_column = array(5,12,13,15,22,23,24,25,26,27); // убираем у Гусарова
    ...
    foreach($head_arr as $key=>$val) {
        ...
        if ((IT_PLANET != $_SESSION['login_id_med'] || !in_array($key, $remove_column)) &&
            ($data_acc_arr && in_array(CAN_HEAR, $data_acc_arr) || (28 != $key && 29 != $key))) { // ограничение для прослушивания записей
            if (GUSAROV == $_SESSION['login_id_med'] && in_array($key, $gus_column)) {
                continue;
            }
            else {
...
LEFT JOIN HOSPITALS hosp ON HOSPITAL_ID = hosp.ID WHERE BASE_ID=:id");
        ...
        if (IT_PLANET != $_SESSION['login_id_med']) { // ограничение для IT Planet
            ...
            if (GUSAROV != $_SESSION['login_id_med']) {
                $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
		...
		if (CALL_SECOND == OCIResult($q, "CALL_DOUBLE")) {
            ...
            if (USER_VIEW == $_SESSION['user_role'])
                $sheet[$snum]->setCellValue($coord, u8("Дубль"));
            else $sheet[$snum]->setCellValue($coord, u8(OCIResult($q, "STATUS")." (Дубль)"));
        }
        else {
            $coord = $sheet[$snum]->getCellByColumnAndRow($col++, $rnum)->getCoordinate();
        ...
        if (IT_PLANET != $_SESSION['login_id_med'] && GUSAROV != $_SESSION['login_id_med']) { // ограничение для IT Planet и Гусарова
...
elseif (EXPORT_OPERATOR == $_POST['ReportId'] || EXPORT_OPERATOR_ALL == $_POST['ReportId']) {
    ...
    if (EXPORT_OPERATOR_ALL == $_POST['ReportId']) //$q_text4 = " WHERE STATUS_ID between " . STATUS_CALL_BACK. " and " . STATUS_NOT_COME;
        $q_text4 = " WHERE STATUS_ID <= " . STATUS_NOT_COME;
    else $q_text4 = " WHERE STATUS_ID between " . STATUS_CALL_STOP . " and " . STATUS_NOT_COME;
    ...
    // FISRT_FIO_ID !!! для сохранения первоначально закрывшего оператора
    if (isset($_POST['UserId']) && strlen($q_users) > 0)
    ...
    while (OCIFetch($q)) { // наполняем данными о звонках
        ...
        if (STATUS_CLINIC == $status_id || STATUS_CLINIC_NOT == $status_id || // успешный статус
            ...
            if (STATUS_CLINIC == $status_id || STATUS_CLINIC_NOT == $status_id ||
                (STATUS_CALL_BACK == $status_id || STATUS_WORK == $status_id) && EXPORT_OPERATOR_ALL == $_POST['ReportId']) {
                $itog_zachet += $pnum;
...
// основные данные - подсчет за период
    if (EXPORT_EFFECT_ISH == $_POST['ReportId']) { // Комбо отчет
        ...
        cb.SOURCE_TYPE_ID, cb.STATUS_ID, cb.VISIT_DATE_1C, ph.RUB, cb.ID as CALL_ID, cb.INTERSTATE, cb.CALL_DOUBLE ";
    ...
    /*$q_text4 .= ") and (cb.STATUS_ID = ".STATUS_CLINIC." or cb.STATUS_ID = ".STATUS_CLINIC_NOT.
        " or cb.STATUS_ID = ".STATUS_CALL_BACK.")"; // !!! закрывающая скобка*/
...
//var_dump($q_text);
    ...
    while (OCIFetch($q)) {
        ...
        if (OCIResult($q,"CALL_ID") != $call_id) {
            ...
            if (STATUS_CLINIC == $status_id)
                $itog_write++;
            if (OCIResult($q, "VISIT_DATE_1C") != NULL && OCIResult($q, "VISIT_DATE_1C") != '')
                $itog_visit++;
        }
...
//var_dump($q_text);
    ...
    $q_text2 = " FROM CALL_BASE cb ";
    $q_text3 = " LEFT JOIN SOURCE_AUTO sr_a ON cb.SOURCE_AUTO_ID = sr_a.ID ";
...
//var_dump($q_text);
    ...
    if (EXPORT_EFFECT_IDYN == $_POST['ReportId']) {
        ...
        //$head_arr = array('0'=>"ID", '1'=>"Источник рекламы (Авто)", '2'=>"Тип ".chr(10)."источника",
