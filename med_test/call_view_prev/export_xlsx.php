...
function fill_list($num_list, $objPHPExcel, $rep_start_date, $rep_end_date, $usluga_auto_arr)
{
    ...
    foreach($head_arr as $key=>$val) { // $key не используется, колонки друг за другом
        ...
        {
            if ($key == 1 && EXPORT_EFFECT == $_POST['ReportId']) continue;
    ...
    foreach ($usluga_auto_arr as $key =>$value) { // отрисовываем полученные данные, кроме строк с нулевыми значениями
        ...
        if (strncmp($value['NAME'], $tmp_name, 4) != 0) { // Промежуточные итоги
            if ($tmp_row > 1) {
                $col = 2;
                if (EXPORT_EFFECT_ISH == $_POST['ReportId']) $col++;
...
if (EXPORT_EFFECT == $_POST['ReportId'] || EXPORT_EFFECT_IDYN == $_POST['ReportId'] || EXPORT_EFFECT_ISH == $_POST['ReportId']) {
...
//var_dump($q_text);
    ...
    if (EXPORT_EFFECT == $_POST['ReportId'] || EXPORT_EFFECT_ISH == $_POST['ReportId']) {
        fill_list(0, $objPHPExcel, $rep_start_date, $rep_end_date, $usluga_auto_arr);
    }
    else {
        fill_list(1, $objPHPExcel, $rep_start_date, $rep_end_date, $usluga_auto_arr);
        ...
        fill_list(2, $objPHPExcel, $rep_start_date, $rep_end_date, $usluga_auto_arr);
    }
}
...
foreach($sheet as $s => $fuck) {
    ...
    if (EXPORT_EFFECT == $_POST['ReportId'] || EXPORT_EFFECT_ISH == $_POST['ReportId'] || EXPORT_EFFECT_IDYN == $_POST['ReportId']) {
        ...
        //if (EXPORT_EFFECT == $_POST['ReportId'] || EXPORT_EFFECT_IDYN == $_POST['ReportId']) {
