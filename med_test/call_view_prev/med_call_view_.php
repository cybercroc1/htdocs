...
//$detail_arr_new = array();
    ...
    if (USER_ADMIN == $_SESSION['user_role'] || USER_SUPER == $_SESSION['user_role'])
        echo "<script>document.all.show_work.disabled=true;</script>";
...
<img src='$img'><a href=\"?$get\" style='color: black'>Дата заявки<br/></a>
    ...
    if (!in_array($_SESSION['login_id_med'], SPEC_USER_CALL)) {
        if (USER_SUPER == $_SESSION['user_role'] && !isset($show_closed)) {
            if (!isset($show_work))
                $q_where .= " and cb.STATUS_ID != " . STATUS_WORK;
            if (!isset($show_delayed))
                $q_where .= " and cb.CALL_BACK_DATE is NULL";
            if (!isset($show_nedo))
                $q_where .= " and cb.CALL_BACK_DATE <= sysdate and cb.DATE_CLOSE is null or cb.STATUS_ID=".STATUS_OPEN;
            /*if (isset($show_delayed) && isset($show_nedo))
                $q_where .= " and (cb.CALL_BACK_DATE is NULL or cb.CALL_BACK_DATE <= sysdate and cb.DATE_CLOSE is null) or cb.STATUS_ID=".STATUS_OPEN;
            */
        }
    }
    else {
        if (!isset($show_closed))
            $q_where .= " and cb.DATE_SECOND_CLOSE is null";
        if (!isset($show_delayed))
            $q_where .= " and (cb.CALL_BACK_DATE <= sysdate or cb.CALL_BACK_DATE is NULL)";
    }
    //else $q_where = " cb.SECOND_LAST_CHANGE > to_date('01.08.2018','DD.MM.YYYY')";
...
//echo "<br/><textarea>".$q_where_top."</textarea>";
    ...
    if (in_array($_SESSION['login_id_med'],SPEC_USER_CALL)) { // операторы второго дозвона
        $q_where .= " or (cb.CALL_BACK_DATE <= sysdate and cb.SECOND_STATUS_ID <= " . STATUS_CALL_NOT;
        ...
        $q_where .= " or CALL_BACK_DATE is NULL and cb.LAST_CHANGE+15/1440 <= sysdate and cb.STATUS_ID=" . STATUS_CALL_NOT . ")";
    } elseif (USER_SUPER == $_SESSION['user_role']) { // для всех кроме супервизора и наблюдателя ???
    ...
    if (isset($show_closed)) { // показываем все звонки
        ...
        if (isset($show_delayed) && !isset($show_nedo)) { // перезвон и открытые
            ...
            if (USER_SUPER == $_SESSION['user_role']) {
                if (isset($show_trans)) $q_where .= " or cb.RESULT_ID=" . RESULT_KC . " and DATE_CLOSE is null";
            }
            ...
            if (USER_SUPER == $_SESSION['user_role'] && isset($show_trans))
            ...
            if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
                $q_where .= " or (cb.SECOND_STATUS_ID <= " . STATUS_CALL_NOT;
            else $q_where .= " or (cb.STATUS_ID <= " . STATUS_CALL_NOT;
            if (USER_SUPER == $_SESSION['user_role']) {
                $q_where .= " and (cb.CALL_BACK_DATE is NULL or cb.CALL_BACK_DATE <= sysdate and cb.DATE_CLOSE is null)";
                if (isset($show_trans))
                    $q_where .= " or cb.RESULT_ID=" . RESULT_KC . " and DATE_CLOSE is null";
            }
            $q_where .= ") and " . $date_where;
...
//if (TRUE == DEBUG_MODE || USER_ADMIN == $_SESSION['user_role'] || 8 == $_SESSION['login_id_med'])
//echo "<textarea>" . $q_text . "</textarea>";
 ...
 WHERE hist.BASE_ID=:base_id ORDER BY DATE_DET";
    ...
    if (USER_ADMIN == $_SESSION['user_role'] || USER_SUPER == $_SESSION['user_role'])
        echo "<script>document.all.show_work.disabled=false;</script>";
