...
<script type="application/javascript">
    ...
    function open_edit(base_id, texnari_id, sid) {
        if (base_id > 0) {
            win = window.open("../med_call_out.php?base_id="+base_id+"&texnari_id="+texnari_id+"&sid="+sid, "med_call_"+base_id, "width=800, height=640, toolbar=no, scrollbars=yes, resizable=yes, status=yes");
            win.focus();
        }
    }
    function open_hist(phone_num, sid) {
        if (null != phone_num) {
            win = window.open("../med_call_hist.php?phone_num="+phone_num+"&sid="+sid, "med_call_"+phone_num, "width=750, height=640, toolbar=no, scrollbars=yes, resizable=yes, status=yes");
            win.focus();
        }
    }
    function export_screen(start_date, end_date, sid, type) {
        win = window.open("./med_export.php?start_date="+start_date+"&end_date="+end_date, "med_export_"+sid, "width=675, height=650 toolbar=no, scrollbars=yes, resizable=yes, status=yes");
        win.focus();
    }
...
if (USER_USER != $_SESSION['user_role']) {
    ...
    if (USER_ADMIN == $_SESSION['user_role'] || USER_SUPER == $_SESSION['user_role'] /*&&
        ...
        echo '<a href="'.PATH.'/admin/admin_source_detail_new.php" target="_blank" rel="noopener"><button name="Ist_Det_Auto" class="enter_button">Источники рекламы</button></a>';
    }
}
echo "</div>";
echo "<div style='float: right; width: 41%'>";
...
if (isset($phone_num) && '' != $phone_num) {
    echo "<script type='application/javascript'>open_hist('".$phone_num."','".$_SESSION['login_id_med']."','".$sid."');</script>";
    echo "<script>document.location.href=(document.location.pathname);</script>";
    //unset($phone_num);
    exit;
}
...
if (USER_VIEW != $_SESSION['user_role']) {
    echo "<form action='' method='post' target='RightFrame' style='float: right; width: 400px; margin-bottom: 0'>";
    $trans_arr = date_parse(date("Y-m-d HH:MM"));
    ...
    if (USER_ADMIN == $_SESSION['user_role']) {
        ...
        echo "<input type='my_number' class='my_number' name='trans_month' value='".$trans_arr['month']."' style='font-size: 1.17em; text-align: right;'>";
        echo "<label for='trans_month' style='color: black; font-size: 1.17em; font-weight: bold'>-</label>";
        echo "<input type='my_number' class='my_number' name='trans_day' value='".$trans_arr['day']."' style='font-size: 1.17em'>";
    }
...
echo "<form action='' method='post' target='RightFrame' style='float: right; width: 400px; margin-bottom: 0'>";
echo "<label for='phone_num' style='color: brown; font-size: 1.17em; font-weight: bold'>Поиск по телефону:&nbsp;</label>";
//echo "<input type='number' name='phone_num' id='phone_num' style='width: 9.5em; margin-top: 3px;' value='' placeholder='Введите номер'/>";
echo "<input name='phone_num' id='phone_num' style='width: 9.5em; margin-top: 3px;' value='' placeholder='Введите номер'/>";
echo "<input type='submit' name='History' value='Посмотреть' class='add_button' style='margin-top: 0'/>";
echo "</form>";
//echo "</frame>";
echo "</div>";
echo "</div>";
...
//$detail_arr_new = array();
    ...
    echo "<script>document.all.show_text.disabled=true;</script>";
...
//Хедер-футер. КОНТЕНТ
    ...
    $theads = array(
        'cb.DATE_CALL' => array('name' => 'Дата заявки', 'width' => '182', 'vari' => 'agid_id'),
    ...
    if (isset($_GET['key'])) {
        ...
        if (USER_ADMIN == $_SESSION['user_role'] || USER_VIEW == $_SESSION['user_role'])
            ...
            else $key = " cb.STATUS_ID, cb.DATE_CALL, cb.CALL_BACK_DATE, serv.NAME, sr_a.NAME, sr_man.NAME";
        }
        if (!in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
            $sort = 'asc';
    }
...
<img src='$img'><a href=\"?$get\" style='color: black'>Дата заявки<br/></a>
    ...
    по <input type='text' name='end_date' id='end_date' autocomplete='off' size=6 onchange=ok.click() value='" . (isset($end_date) ? $end_date : '') . "' />";
            ...
            echo "</th>";
        } elseif ('Время cобытия' != $thead['name'] && 'Звонил' != $thead['name'] && 'Телефон' != $thead['name']) {
            /*if ('Результат' == $thead['name'] && !isset($show_trans))
                $res_id = '';
            else*/
            if ('Уточнение' == $thead['name'] && !isset($show_closed))
                $stat_det_id = '';
            elseif ('Назначено' == $thead['name'] && USER_USER == $_SESSION['user_role'])
                $texnari_id = '';
            elseif ('Письма' == $thead['name']) {
                ...
                <img src='$img'><a href=\"?$get\" style='color: black'>{$thead['name']}</a>";
                    echo "<select style='width:100%' name=mail_id onchange=ok.click()>";
                    ...
                    echo "</select>";
                    echo "</th>";
                    echo "<script>document.all.mail_id.disabled=true;</script>";
                } else $mail_id = '';
            } elseif (USER_ADMIN != $_SESSION['user_role'] || ('Оператор' != $thead['name'] && 'АОН' != $thead['name'])) {
                echo "<th style='background-color: white; vertical-align: top; width:{$thead['width']}px'>
                <img src='$img'><a href=\"?$get\" style='color: black'>{$thead['name']}</a>";
                echo "<select style='width:100%' name={$thead['vari']} onchange=ok.click()>";
                echo "<option value='' style='color:green'>ВСЕ</option>";
                echo "</select>";
                echo "</th>";
                echo "<script>document.all." . $thead['vari'] . ".disabled=true;</script>";
            }
        } else {
    ...
    if (false) { // весь заголовок таблицы теперь в цикле чуть выше
        echo "<th style='background-color: white; vertical-align: top; white-space: nowrap' colspan=2>Дата заявки<br/>
    c <input type='text' name='start_date' id='start_date' size=6 onchange=ok.click() value='" . (isset($start_date) ? $start_date : '') . "' />
    по <input type='text' name='end_date' id='end_date' size=6 onchange=ok.click() value='" . (isset($end_date) ? $end_date : '') . "' />";
        ...
        if (USER_ADMIN != $_SESSION['user_role']) {
            ...
            echo "<th style='background-color: white; vertical-align: top; width: 80px'>АОН<br/>";
            ...
            /*
                ...
                echo "<th style='background-color: white; vertical-align: top; width: 80px'>Проект<br/>";
        ...
        /*echo "<th style='background-color: white; vertical-align: top; width: 80px'>Уточнение<br/>";
        echo "<select style='width:100%' name=service_det_id onchange=ok.click()>";
        echo "<option value='' style='color:green'>ВСЕ</option>";
        echo "</select>";
        echo "</th>";
        echo "<script>document.all.service_det_id.disabled=true;</script>";*/

        echo "<th style='background-color: white; vertical-align: top; width: 150px'>Источник(Авто)<br/>";
        echo "<select style='width:100%' name=source_a_id onchange=ok.click()>";
        echo "<option value='' style='color:green'>ВСЕ</option>";
        echo "</select>";
        echo "</th>";
        echo "<script>document.all.source_a_id.disabled=true;</script>";

        echo "<th style='background-color: white; vertical-align: top; width: 65px'>Канал<br/>";
        echo "<select style='width:100%' name=st_id onchange=ok.click()>";
        echo "<option value='' style='color:green'>ВСЕ</option>";
        echo "</select>";
        echo "</th>";
        echo "<script>document.all.st_id.disabled=true;</script>";

        echo "<th style='background-color: white; vertical-align: middle; width: 120px'>Звонил<br/></th>";
        echo "<th style='background-color: white; vertical-align: middle; width: 120px'>Телефон<br/></th>";

        echo "<th style='background-color: white; vertical-align: top; width: 100px'>Источник(вход)<br/>";
        echo "<select style='width:100px' name=source_m_id onchange=ok.click()>";
        echo "<option value='' style='color:green'>ВСЕ</option>";
        echo "</select>";
        echo "</th>";
        echo "<script>document.all.source_m_id.disabled=true;</script>";

        echo "<th style='background-color: white; vertical-align: top; width: 100px'>Детали(вход)<br/>";
        ...
        echo "<th style='background-color: white; vertical-align: top; width: 100px'>Источник(исх.)<br/>";
        echo "<select style='width:100px' name=source_man_id_new onchange=ok.click()>";
        echo "<option value='' style='color:green'>ВСЕ</option>";
        echo "</select>";
        echo "</th>";
        echo "<script>document.all.source_man_id_new.disabled=true;</script>";

        /*    echo "<th style='background-color: white; vertical-align: top; width: 100px'>Детали(исх.)<br/>";
        echo "<select style='width:100%' name=detail_id_new onchange=ok.click()>";
        echo "<option value='' style='color:green'>ВСЕ</option>";
        echo "</select>";
        echo "</th>";
        echo "<script>document.all.detail_id_new.disabled=true;</script>";*/
        ...
        echo "<th style='background-color: white; vertical-align: top; width: 90px'>Время<br/>cобытия<br/></th>";

        if (isset($show_trans)) { // переводные
            echo "<th style='background-color: white; vertical-align: top; width: 90px'>Результат<br/>";
        ...
        if (USER_ADMIN == $_SESSION['user_role'] || USER_SUPER == $_SESSION['user_role']) {
            echo "<th style='background-color: white; vertical-align: middle; width: 30px'><img src='../images/mail.png'></th>";
        }
    } // if (false)
    ...
    elseif (USER_USER == $_SESSION['user_role']) {
        $q_where .= " and cb.FIO_ID=" . $_SESSION['login_id_med'];// . " and cb.CALL_TYPE_ID=" . CALL_FIRST; // только свои
    }
    if (!isset($show_closed) && !isset($show_delayed)) {
        if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
            $q_where .= " and cb.DATE_SECOND_CLOSE is null";
        else $q_where .= " and cb.DATE_CLOSE is null";
    }
    //$q_where .= " and (cb.RESULT_ID=" . RESULT_WAIT . " or cb.RESULT_ID=" . RESULT_KC . ")";
    ...
    if (USER_ADMIN != $_SESSION['user_role']) {
        if ($anumber_id <> "") {
            $q_where_top .= " cb.ANUMBER='" . $anumber_id . "'";
            $btop = TRUE;
        }
        if ($agid_id <> "") {
            $q_where_top .= ($btop ? " and cb.SC_AGID='" : " cb.SC_AGID='") . $agid_id . "'";
            $btop = TRUE;
        }
    ...
    if ($st_id <> "") {
        $q_where_top .= ($btop ? " and cb.SOURCE_TYPE_ID='" : " cb.SOURCE_TYPE_ID='") . $st_id . "'";
        $btop = TRUE;
    }
    if ($service_id <> "") {
        $q_where_top .= ($btop ? " and cb.SERVICE_ID='" : " cb.SERVICE_ID='") . $service_id . "' ";
        $btop = TRUE;
    }
    //if ($service_det_id <> "")  {$q_where_top .= ($btop ? " and cb.SERVICE_DET_ID='" : " cb.SERVICE_DET_ID='") . $service_det_id . "' ";  $btop = TRUE; }
    if ($source_a_id <> "") {
        $q_where_top .= ($btop ? " and cb.SOURCE_AUTO_ID='" : " cb.SOURCE_AUTO_ID='") . $source_a_id . "'";
        $btop = TRUE;
    }
    if ($source_m_id <> "") {
        $q_where_top .= ($btop ? " and cb.SOURCE_MAN_ID='" : " cb.SOURCE_MAN_ID='") . $source_m_id . "'";
        $btop = TRUE;
    }
    if ($source_man_id_new <> "") {
        $q_where_top .= ($btop ? " and cb.SOURCE_MAN_ID_NEW='" : " cb.SOURCE_MAN_ID_NEW='") . $source_man_id_new . "'";
        $btop = TRUE;
    }
    if ($detail_id <> "") {
        $q_where_top .= ($btop ? " and cb.SOURCE_MAN_DET_ID='" : " cb.SOURCE_MAN_DET_ID='") . $detail_id . "'";
        $btop = TRUE;
    }
    ...
    if (/*isset($show_trans) &&*/
        $res_id <> "") {
        $q_where_top .= ($btop ? " and cb.RESULT_ID='" : " cb.RESULT_ID='") . $res_id . "'";
        $btop = TRUE;
    }
//echo "<br/><textarea>".$q_where_top."</textarea>";
    ...
    } elseif ($btop && USER_ADMIN == $_SESSION['user_role'])
        echo "<script>document.getElementById('blink1').style.visibility = 'visible';</script>";
    ...
    else //(USER_ADMIN == $_SESSION['user_role'])
        $mystatus = STATUS_OPEN;

    if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL)) {
        $q_where .= " or (cb.CALL_BACK_DATE <= sysdate and cb.SECOND_STATUS_ID=" . STATUS_CALL_BACK;
        $q_where .= " or cb.SECOND_LAST_CHANGE+15/1440 <= sysdate and cb.SECOND_STATUS_ID=" . STATUS_CALL_NOT . ")";
    } elseif (USER_SUPER == $_SESSION['user_role']) { // для всех кроме супервизора и набдюдателя ???
        $q_where .= " and cb.STATUS_ID=" . $mystatus . " and cb.RESULT_ID != " . RESULT_KC .
            " or (cb.STATUS_ID=" . $mystatus . " and cb.RESULT_ID != " . RESULT_KC;
        $q_where .= " or cb.CALL_BACK_DATE <= sysdate and cb.STATUS_ID=" . STATUS_CALL_BACK . " and cb.FIO_ID=" . $_SESSION['login_id_med'];
        $q_where .= " or cb.LAST_CHANGE+15/1440 <= sysdate and cb.STATUS_ID=" . STATUS_CALL_NOT . " and cb.FIO_ID=" . $_SESSION['login_id_med'] . ")";
    } elseif (USER_USER == $_SESSION['user_role']) {
        $q_where .= " or (cb.STATUS_ID=" . $mystatus;
        $q_where .= " or cb.CALL_BACK_DATE <= sysdate and cb.STATUS_ID=" . STATUS_CALL_BACK;
        $q_where .= " or cb.LAST_CHANGE+15/1440 <= sysdate and cb.STATUS_ID=" . STATUS_CALL_NOT . ")";
    }
    ...
    if (isset($show_closed)) { // показываем все звонки
        ...
        if (isset($show_delayed) && !isset($show_nedo)) { // перезвон и открытые
            if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
                $q_where .= " or (cb.SECOND_STATUS_ID=" . STATUS_CALL_BACK . " and " . $date_where;
            else $q_where .= " or (cb.STATUS_ID=" . STATUS_CALL_BACK . " and " . $date_where;
            if (USER_SUPER == $_SESSION['user_role']) {
                if (isset($show_trans)) $q_where .= " or cb.RESULT_ID=" . RESULT_KC;
                $q_where .= " or cb.CALL_BACK_DATE is not null and cb.STATUS_ID != " . STATUS_CALL_NOT . ")";
            } else {
                if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
                    $q_where .= " or cb.SECOND_LAST_CHANGE+15/1440 <= sysdate and cb.SECOND_STATUS_ID=" . STATUS_CALL_NOT . ")";
                else $q_where .= " or cb.LAST_CHANGE+15/1440 <= sysdate and cb.STATUS_ID=" . STATUS_CALL_NOT .
                    " or cb.CALL_BACK_DATE is not null and cb.STATUS_ID != " . STATUS_CALL_NOT . ")";
            }
            $q_where .= " and " . $date_where;
        } elseif (!isset($show_delayed) && isset($show_nedo)) { // недозвон отложенный на 15 минут и открытые
            if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
                $q_where .= " or (cb.SECOND_STATUS_ID=" . STATUS_CALL_NOT . " and " . $date_where;
            else $q_where .= " or (cb.STATUS_ID=" . STATUS_CALL_NOT . " and " . $date_where;
            if (USER_SUPER == $_SESSION['user_role']) {
                if (isset($show_trans)) $q_where .= " or cb.RESULT_ID=" . RESULT_KC . " and " . $date_where;
                $q_where .= ")";
            } else {
                if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
                    $q_where .= " or cb.CALL_BACK_DATE <= sysdate and cb.SECOND_STATUS_ID=" . STATUS_CALL_BACK . ")";
                else $q_where .= " or cb.CALL_BACK_DATE <= sysdate and cb.STATUS_ID=" . STATUS_CALL_BACK . ")";
            }
        } elseif (isset($show_delayed) && isset($show_nedo)) { // перезвон, недозвон отложенный на 15 минут и открытые
            if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
                $q_where .= " or ((cb.SECOND_STATUS_ID=" . STATUS_CALL_NOT . " or cb.SECOND_STATUS_ID=" . STATUS_CALL_BACK . ")" . " and " . $date_where;
            else $q_where .= " or ((cb.STATUS_ID=" . STATUS_CALL_NOT . " or cb.STATUS_ID=" . STATUS_CALL_BACK . ")" . " and " . $date_where;
            if (USER_SUPER == $_SESSION['user_role']) {
                if (isset($show_trans)) $q_where .= " or cb.RESULT_ID=" . RESULT_KC;
                $q_where .= " or cb.CALL_BACK_DATE is not null)";
            } else {
                if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
                    $q_where .= " or cb.CALL_BACK_DATE <= sysdate and cb.SECOND_STATUS_ID=" . STATUS_CALL_BACK . ")";
                else $q_where .= " or cb.CALL_BACK_DATE <= sysdate and cb.STATUS_ID=" . STATUS_CALL_BACK . ")";
            ...
            $q_where .= " or (cb.RESULT_ID=" . RESULT_KC . " and " . $date_where . ")";
        }
    }
    if ($btop && $bchecked)
        $q_where .= " and " . $q_where_top;
    ...
    if ($btop && USER_ADMIN == $_SESSION['user_role'])
        $q_where .= " and " . $q_where_top;
...
//        or upper(replace(sr_det.NAME,' ')) like '%'||upper(replace('".$find_id."',' '))||'%'
    ...
    cb.DATE_SECOND_CHANCE, to_char(cb.SECOND_LAST_CHANGE,'dd.mm.yyyy hh24:mi:ss') SECOND_LAST_CHANGE, 
    ...
    case when cb.SENT_MAIL is NULL then 0 else 1 end SENT_ALREADY, mlc.H_SUBJECT,mlc.MAIL_BODY_TEXT, mlc.H_FROM";
    //, serv_det.NAME as SERV_DET
    //if (!in_array($_SESSION['login_id_med'], SPEC_USER_CALL))
    ...
    LEFT JOIN MAIL_LEADCOLLECTOR mlc ON mlc.CALL_BASE_ID = cb.ID ";
...
//LEFT JOIN SERVICE_DET serv_det ON cb.SERVICE_DET_ID = serv_det.ID
    ...
    else $q_text5 = " ORDER BY cb.STATUS_ID, cb.DATE_CALL, cb.CALL_BACK_DATE, serv.NAME, sr_a.NAME, sr_man.NAME";*/
...
//if (TRUE == DEBUG_MODE || USER_ADMIN == $_SESSION['user_role'] || 8 == $_SESSION['login_id_med']) echo "<textarea>" . $q_text . "</textarea>";
 ...
 WHERE hist.BASE_ID=:base_id ORDER BY DATE_DET";
    ...
    while (OCIFetch($q)) {
        ...
        if (in_array($_SESSION['login_id_med'], SPEC_USER_CALL)) {
            if (NULL == $second_fio_id) {
                if ($locked_row > 0) break; //continue; // чтобы загрузить только одну свободную запись
                $locked_row++;
                ...
                if (!OCI_Fetch_Array($query_lock)) {
                    ...
                    if (OCIExecute($query_lock)) {
                        ...
                        //$updatestr = "UPDATE ".$table_name." SET DATE_SECOND_CHANCE = '".$sec_chance."', SECOND_LAST_CHANGE = '".$sec_chance."'";
                        $updatestr = "UPDATE ".$table_name." SET DATE_SECOND_CHANCE = sysdate, SECOND_LAST_CHANGE = sysdate";
        ...
        //$srv_det_id = OCIResult($q,"SERVICE_DET_ID");
        ...
        if (OCIResult($q, "SRV_ID") != SERVICE_NOT && !isset($service_arr[OCIResult($q, "SRV_ID")])) {
            $service_arr[OCIResult($q, "SRV_ID")] = OCIResult($q, "SRVNAME");
        }
        /*if ($srv_det_id && !isset($service_det_arr[$srv_det_id])) {
            $service_det_arr[$srv_det_id] = STOM_DET[$srv_det_id];
        }*/
        /*if ($srv_det_id && !isset($service_det_arr[$srv_det_id])) {
            $service_det_arr[$srv_det_id] = OCIResult($q, "SERV_DET");
        }*/
        ...
        if ($status_id >= STATUS_CALL_STOP) {
            ...
            $body .= chr(13);
            $body .= " " . chr(13);
            $body .= "\nУслуга: " . OCIResult($q, "SRVNAME") . chr(13);
            $body .= " " . chr(13);
            ...
            while (OCIFetch($q_comment)) {
                ...
                elseif (STATUS_WORK == OCIResult($q_comment, "STATUS_ID")) {
                    ...
                    if ($oper_id && GetData::GetUsers(TRUE, TRUE, "usr.ID = " . $oper_id, "FIO") > 0) {
                        $substr_name = $_POST['array_user'][0]['FIO'];
        ...
        echo "<td " . $onclick . " bgcolor=white style='text-align: center; color:" . $status_color . ";'>" . OCIResult($q, "SRVNAME") . "</td>";
        /*if ($srv_det_id) //echo "<td bgcolor=white style='text-align: center; color:".$status_color.";'>".STOM_DET[$srv_det_id]."</td>";
            echo "<td bgcolor=white style='text-align: center; color:".$status_color.";'>".OCIResult($q, "SERV_DET")."</td>";
    ...
    echo "</table>";
    OCIFreeStatement($q);
    echo "<script>document.all.show_text.disabled=false;</script>";
    echo "<script>document.all.show_nedo.disabled=false;</script>";
    echo "<script>document.all.show_trans.disabled=false;</script>";
    echo "<script>document.all.show_closed.disabled=false;</script>";
    echo "<script>document.all.show_delayed.disabled=false;</script>";
    //Хедер-футер. ФУТЕР
