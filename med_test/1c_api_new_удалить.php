...
*/	
	$q=OCIParse($c,"select b.id, st.name source_type_name, b.bnumber, sa.name source_auto_name, sm.name source_man_name, sd.name source_man_det
...
*/
    ...
    $q=OCIParse($c,"select count(*) cnt from write_hist where base_id='{$ticket_id}' and date_write=to_date('{$entry_date}','YYYYMMDDHH24MISS')");
    OCIExecute($q);
    OCIFetch($q);
    $write_num = OCIResult($q,"CNT"); // сколько раз уже записывали человека

    $ins=OCIParse($c,"insert into write_hist (base_id,date_add,date_visit) values ('{$ticket_id}',sysdate,to_date('{$entry_date}','YYYYMMDDHH24MISS'))");
    OCIExecute($ins);
    OCICommit($c);
    if (0 == $write_num) {
        $upd = OCIParse($c, "update call_base set check_entry_date_1c=sysdate, 
	entry_date_1c=to_date('{$entry_date}','YYYYMMDDHH24MISS') where id='{$ticket_id}'");
        if (OCIExecute($upd)) {
