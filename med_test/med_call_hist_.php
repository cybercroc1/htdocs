...
if ('8' == $phone_mob[0] && strlen($phone_mob) >= 11)
    $phone_mob = substr($phone_mob, 1);
if ('' == $phone_mob) $phone_mob = $phone_num;

function show_res($Result)
{
    if (GetData::GetCallHistory($Result[0]) > 0) {
        echo "<h3 style='margin: 0'>";
        if (USER_ADMIN == $_SESSION['user_role'] || USER_SUPER == $_SESSION['user_role']) {
            echo "<input type='checkbox' name='sent_" . $Result[0] . "' title='Пометить отправленным'";
        ...
        echo "<table style='display: inline-block; border-spacing: 0;'><tr>
<th style='width: 115px; border:1px solid grey;'>Дата</th>
<th style='width: 150px; border:1px solid grey;'>Оператор</th>
<th style='width: 100px; border:1px solid grey;'>Статус</th>
<th style='width: 360px; border:1px solid grey;'>Примечание</th>
        ...
        foreach (GetData::$array_hist as $key => $value) {
            ...
            elseif (STATUS_CLINIC == $value['STATUS_ID']) {
                if (GetData::GetCallWriteClinic($Result[0]) > 0) {
                    foreach (GetData::$array_wc as $rec_wc){
                        $comment_cut = $comment_cl." (".$rec_wc['CLIENT_NAME']." ".$rec_wc['HOSPITAL_NAME']." (".$rec_wc['CITY'].")"." ".$rec_wc['CLIENT_DATE'].")";
                    }
                }
    ...
    if (GetData::GetUsers(FALSE, FALSE,"usr.ID = ".$_SESSION['login_id_med'], "FIO") > 0) {
        ...
        if (OCIExecute($query)) {
            while ($Result = OCI_Fetch_Row($query)) {
                if (GetData::GetCallHistory($Result[0]) > 0) {
                    $person = (strlen(trim($Result[1])) > 0 ? trim($Result[1]) : '---');
                    $mess .= "<h3 style='margin: 0'> ID звонка: " . $Result[0] . ". &nbsp;Представился: " . $person . ". &nbsp;Контактный телефон: " . $Result[2] . "<br/></h3>";
                    if ($Result[3])
