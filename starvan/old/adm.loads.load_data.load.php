...
continue;			
		...
		//проверка экстеншна (если несколько номеров, и есть экстеншн)
		if(count($ins_phones_arr)>0 and isset($phone_ext_field_id)) {
			if($str[$phone_ext_field_num]<>'' and count($ins_phones_arr)>1) {
...
continue;
	...
	if($phone_field_id<>'') { //если есть поле Телефон
		foreach($ins_phones_arr as $ord => $phone) {
			if(isset($phone_ext_field_id)) $ext=$str[$phone_ext_field_num]; else $ext='';
