...
while(OCIFetch($q_page)) {
	...
	while(OCIFetch($q_grp)) {
		...
		if($g_on_p==1) { //если первая группа на странице, то совмещаем в одной строке
			...
			echo "<tr data-type=page onMouseOver='sel_row(this)' onMouseOut='unsel_row(this)'>";
			if($i==1) echo "<th>$i</th>"; //запрещаем удалять первую страницу
			else 
			echo "<th style='cursor:pointer' title='Удалить (двойной щелчок)' onDblClick='del_old_page(\"".$page_id."\",\"".$grp_id."\");del_obj(this)'><img src='png/del.png'></img></th>";
			echo "<th style='cursor:pointer' title='Добавить ниже' onClick='add_obj(this)'><img src='png/plus.png'></img></th>";
			//echo "<th></th>";
			if($i==1) echo "<td onclick='click_row(this)'>"; //запрещаем перемещать первую страницу
			else 
			echo "<td style='cursor:s-resize' onMouseDown='fMD(this)' onMouseUp='fMU(this)' onclick='click_row(this)'>";
			...
			echo "</td>";
			echo "<td colspan=5 onclick='click_row(this)'>"; 
			echo "<textarea name=message[$i] onchange='notsaved()' style='width:100%'>".$page_mess."</textarea>";
			echo "</td>";
			echo "<td onclick='click_row(this)'><select name='order_type[$i]' style='width:100%' onchange='notsaved()'><option>".$grp_order_type."</option><option>По порядку</option><option>Случайно</option></select></td>";
			echo "</tr>";
			}
			//ГРУППЫ
			if($g_on_p>1) {$i++;
				//if($i==2) echo "<tr data-type=group>";
				echo "<tr data-type=group onMouseOver='sel_row(this)' onMouseOut='unsel_row(this)'>";
				//if($i==2) echo "<th></th>";
				echo "<th style='cursor:pointer' title='Удалить (двойной щелчок)' onDblClick='del_old_grp(\"".$grp_id."\");del_obj(this)'><img src='png/del.png'></img></th>";
				echo "<th style='cursor:pointer' title='Добавить ниже' onClick='add_obj(this)'><img src='png/plus.png'></img></th>";
				//echo "<th></th>";
				//if($i==2) echo "<td>";
				echo "<td style='cursor:s-resize' onMouseDown='fMD(this)' onMouseUp='fMU(this)' onclick='click_row(this)'>";
		...
		while(OCIFetch($q_obj)) {
			...
			echo "<tr data-type=object data-object_id='".$obj_id."' onMouseOver='sel_row(this)' onMouseOut='unsel_row(this)'>";
			echo "<th style='cursor:pointer' title='Удалить (двойной щелчок)' onDblClick='del_old_obj(\"".$obj_id."\");del_obj(this)'><img src='png/del.png'></img></th>";
			echo "<th style='cursor:pointer' title='Добавить ниже' onClick='add_obj(this)'><img src='png/plus.png'></img></th>";
			//вопросы одиночные
			if(substr($obj_type_id,0,5)=='q_sn_') {
				...
				echo "</td>";
				echo "<td onclick='click_row(this)'><textarea name=message[$i] onchange='notsaved()' style='width:100%'>".$message."</textarea></td>";
				echo "<td onclick='click_row(this)'><input style='width:100%' type=text name=text_name[$i] value='".$text_name."' onchange='notsaved()'></td>";
				echo "<td onclick='click_row(this)'><input style='width:100%' type=text name=code_name[$i] value='".$code_name."' onchange='notsaved()'></td>";
				echo "</td>";
				echo "<td onclick='click_row(this)'>об<input type=checkbox name=must[$i] ".($must<>''?' checked':NULL)." onchange='notsaved()'></td>";
				echo "<td onclick='click_row(this)'></td>";
				echo "<td onclick='click_row(this)'></td>";
				echo "</tr>";
			}
			//вопросы с выбором
			else if(substr($obj_type_id,0,5)=='q_ls_') {
				//echo "<th><a href='adm.ank.list.edit.php?obj_id=".$obj_id."' target='admAnkEditSecondFrame'><img src='gif/edit.gif'></img></a></th>";
			...
			//концы
				...
				echo "</td>";
				echo "<td colspan=6 onclick='click_row(this)'><textarea name=message[$i] onchange='notsaved()' style='width:100%'>".$message."</textarea></td>";
				echo "</tr>";	
			}
			//все остальное
			else {
				...
				echo $page_num.".".$grp_num.".".$obj_num." (".$obj_id.")"; 
				echo "</td>";
				echo "<td onclick='click_row(this)'><input type=hidden name=obj_type_id[$i] value='".$obj_type_id."'>".$obj_type_name."</td>";
				echo "<td colspan=6 onclick='click_row(this)'><textarea name=message[$i] onchange='notsaved()' style='width:100%'>".$message."</textarea></td>";
				echo "</tr>";		
			}			
		}
...
if($i==0) { 
	...
	echo "</td>";
	echo "<td onclick='click_row(this)'><select name='order_type[$i]' style='width:100%' onchange='notsaved()'><option>По порядку</option><option>Случайно</option></select></td>";
	echo "</tr>";
}
