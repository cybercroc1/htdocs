...
if(isset($frm_submit) and $frm_submit=='save') {
	...
	//сохранение независимых по исходным
	if($level=='src' and isset($src_index_id)) {
		...
		echo "Сохранены значения независимых квот по исходным полям<hr>";
	}
	//сохранение независимых по вопросам
	if($level=='qst' and isset($qst_index_id)) {
		...
		echo "Сохранены значения независимых квот по вопросам<hr>";		
	}
	OCICommit($c);
...
echo "<option value='src'".($level=='src'?' selected':NULL).">Независимые по исходным</option>";
echo "<option value='qst'".($level=='qst'?' selected':NULL).">Независимые по вопросам</option>";
echo "</select> | ";
...
<input type=submit name=import_from_file disabled value='Загрузить'><br>";	
echo "</form>";
...
where i.project_id=".$project_id." and qq.quote_level=".$lvl." and qq.index_id=i.id) q".$lvl." ";
	...
	$i=0; while(OCIFetch($q)) {$i++; 
		if($i==1) {
			echo "<tr><td></td>";
			if(count($src_fields)>0) echo "<th colspan=".($level==0?count($src_fields)+3:count($src_fields)).">И С Х О Д Н Ы Е  П О Л Я</th>";
