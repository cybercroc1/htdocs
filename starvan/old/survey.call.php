...
function show_base($project_id,$base_id) {
	...
	echo "ID: ".OCIResult($q,"ID")." | ".OCIResult($q,"STATUS")." | Местное время (мск<b>".OCIResult($q,"UTC_MSK")."</b>): <b>".OCIResult($q,"LOCAL_TIME")."</b>";
}
...
function show_phone($project_id,$base_id,$phone_id) {
	...
	echo "Тел.: <b>".segment_phone(OCIResult($q,"PHONE"))."</b>".(OCIResult($q,"EXT")<>''?' доб.: <b>'.OCIResult($q,"EXT").'</b>':NULL)." | ".OCIResult($q,"STATUS");	
}
function show_call_buttons($base_id,$phone_id,$num_src_fields,$num_phone_fields) {
	...
	echo "<input type=hidden name=set_call_status>";
	if($num_src_fields==0) {
		echo "<font color=red>В проекте нет исходных полей</font><hr>";
		echo "<input type=button style='background:green' value='Создать новую запись и начать опрос' onclick=frm_set_status.set_call_status.value='inwork';frm_set_status.submit()></input>";
	...
	elseif($num_phone_fields==0) {
		...
		echo "<input type=button style='background:green' value='Начать опрос без телефона' onclick=frm_set_status.set_call_status.value='inwork';frm_set_status.submit()></input>";
		echo "<input type=button style='background:red' value='Ошибка' onclick=frm_set_status.set_call_status.value='error';frm_set_status.submit()></input>";	
		echo "<input type=hidden name=base_id value='".$base_id."'>";
		echo "<input type=hidden name=phone_id value='".$phone_id."'>";
		show_ank();
	}	
	elseif($phone_id=='') {
		...
		echo "<input type=button style='background:green' name=inwork value='Начать опрос без телефона' onclick=frm_set_status.set_call_status.value='inwork';frm_set_status.submit()></input>";
		echo "<input type=button style='background:red' name=error value='Ошибка' onclick=frm_set_status.set_call_status.value='error';frm_set_status.submit()></input>";			
		echo "<input type=hidden name=phone_id value='".$phone_id."'>";
		show_ank();
	}
	elseif($base_id<>'' and $phone_id<>'') {
		echo "<input type=button style='background:green' name=inwork value='Дозвон/начать опрос' onclick=frm_set_status.set_call_status.value='inwork';frm_set_status.submit()></input>";
		echo "<input type=button style='background:blue' name=perez value='Перезвонить' onclick=frm_set_status.set_call_status.value='perez';frm_set_status.submit()></input>";
		echo "<input type=button style='background:orange' name=nedoz value='Недозвон' onclick=frm_set_status.set_call_status.value='nedoz';frm_set_status.submit()></input>";
		echo "<input type=button style='background:red' name=otkaz value='Отказ' onclick=frm_set_status.set_call_status.value='otkaz';frm_set_status.submit()></input>";
		echo "<input type=button style='background:red' name=error value='Ошибка' onclick=frm_set_status.set_call_status.value='error';frm_set_status.submit()></input>";				
		echo "<input type=hidden name=base_id value='".$base_id."'>";
		echo "<input type=hidden name=phone_id value='".$phone_id."'>";
		show_ank();
