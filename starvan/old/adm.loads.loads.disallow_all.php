...
if(isset($mark)) {
	...
	$q_upd_base=OCIParse($c,"update STC_BASE b set b.allow='' where b.project_id=".$_SESSION['adm']['project']['id']." and b.load_hist_id in (".$load_ids.") and b.allow='y'"); 
	
	$q_upd_phone=OCIParse($c,"update STC_PHONES p set allow='' where p.load_hist_id in (".$load_ids.") and p.project_id='".$_SESSION['adm']['project']['id']."' and p.allow='y'");
...
where h.project_id='".$_SESSION['adm']['project']['id']."' and h.id in (".$load_ids.")");
	...
	echo "Заблокировано записей: <b>$count_base</b>; телефонов: <b>$count_phones</b><br>";
