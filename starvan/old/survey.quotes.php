...
$i=0; while(OCIFetch($q)) {$i++; 
	if($i==1) {
		...
		echo "<tr".$tmp_class." onclick=sel_quote('auto')>";
		echo "<td></td>";
		echo "<td colspan=".$j."><b>Авто</th>";
		echo "<td>".OCIResult($q_prj,"QUOTE")."</td>";
		echo "<td>".OCIResult($q_prj,"STAT_END_NORM")." (".OCIResult($q_prj,"PROC").")</td>";
		echo "<td>".OCIResult($q_prj,"STAT_NEW")."</td>";
		echo "<td>".OCIResult($q_prj,"STAT_NEDOZ")."</td>";
		echo "<td>".OCIResult($q_prj,"STAT_PEREZ")."</td>";
		echo "<td>".OCIResult($q_prj,"STAT_INWORK")."</td>";
		echo "</tr>";
	}
	$rownum++;
	$color1='';
	...
	if(OCIResult($q,"QID")==$_SESSION['survey']['call']['quote_id'])
		echo "<tr class=clicked_row onclick='sel_quote(".OCIResult($q,"QID").")'>";
	else if(OCIResult($q,"LOCK_ROW")<>'')
		echo "<tr class=not_selectable_row>";
	else 	
		echo "<tr class=selectable_row onclick='sel_quote(".OCIResult($q,"QID").")'>";
