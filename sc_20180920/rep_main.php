	...
	//список форм
		...
		*/
		...
		$x=0; while(OCIFetch($q)) {$x++;
			...
			elseif(!isset($cdn_uniq[OCIResult($q,"CGPN")])) $cdn_uniq[OCIResult($q,"CGPN")]=OCIResult($q,"CGPN");
			
			if($tmp_form_id<>OCIResult($q,"FORM_ID")) {
				$forms_arr[OCIResult($q,"FORM_ID")]=OCIResult($q,"FORM_NAME");
			...
			if(OCIResult($q,"CGPN")=='') {
				...
				$cdn_arr[OCIResult($q,"FORM_ID")][]=OCIResult($q,"CGPN");
				$cdn_name[OCIResult($q,"FORM_ID")][]=OCIResult($q,"PHONE_NAME");
			}
		}
		asort($cdn_uniq);
