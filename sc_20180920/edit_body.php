...
$q=OCIParse($c,"select * from sc_forms where project_id='".$_SESSION['project']['id']."' /*and id not in (select form_id from sc_body
...
if($_SESSION['admin']==1) {
	$q=OCIParse($c,"select * from sc_forms where project_id='0' and id>0 and deleted is null");
	OCIExecute($q,OCI_DEFAULT);	
...
if (!isset($form_id)) $form_id="";
	...
	$ins=OCIParse($c,"insert into sc_body (id,head,body,type,ordering,project_id,punkt_id,general,form_id,shedule_id,txt_tag_before,txt_tag_after,txt_align,forw_list_id,colapsed,new_window,faq,out_prefix) 
	values ((select nvl(max(id),0)+1 from sc_body),'".$quest."',:body,'".$blog_type."',".$ordering."+1,'".$_SESSION['project']['id']."','".$new_punkt_id."','".$general."','".$form_id."','".$shedule_id."','".$txt_tag_before."','".$txt_tag_after."','".$txt_align."','".$forw_list_id."','".$colapsed."','".$new_window."','".$faq."','".$out_prefix."') returning id into :new_id");
	if ($answer=='') $answer=' ';
