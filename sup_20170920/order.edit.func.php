...
if($redirect=='y' or ($solution=='y' and $author_id==$_SESSION['user_id'])) {
	...
	if($solution=='y' and $author_id=$_SESSION['user_id']) {
	...
	if($solution=='y' and $from_user_id<>$_SESSION['user_id']) {
...
if($solution=='y' or $redirect=='y' or $eval=='y' or ($create_new=='y' and $author_id==$_SESSION['user_id'])) {
	if($solution=='y' and $from_user_id<>$_SESSION['user_id'] and $eval<>'y') $to_user_id_HTML.=" <nobr><input type=submit name=save style='background-color:green' value='Принять в работу'>";
	//else if($_SESSION['tmp'][$base_id]['solution']=='y' and $from_user_id==$_SESSION['user_id'] and $_SESSION['tmp'][$base_id]['eval']<>'y') $to_user_id_HTML.=" <nobr><input type=submit disabled name=save style='background-color:indigo' value='Сохранить'>";
